...
class syssafe_pub:
    ...
    __elist = None
    __last_pid_count = 0
    __name_number = re.compile(r"^\d+(\.(sh|py|exe|bash|cmd|run))?$")
    def check_main(self):
        pids = psutil.pids()
        pid_count = len(pids)
        ...
        try:
            for pid in pids:
                ...
                is_num_name = self.__name_number.match(name)
                if not is_num_name:
                    if self.check_white(name): continue
                try:
                    exe_file = '/proc/' + str(pid) + '/exe'
                    if not os.path.exists(exe_file): continue
                    exe_bin = os.readlink(exe_file)

                    if exe_bin.find("/tmp/") == -1 and not is_num_name: continue
