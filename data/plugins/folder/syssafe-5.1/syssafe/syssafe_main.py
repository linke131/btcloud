...
class syssafe_main:
    ...
    # 获取防护对象名称
    def get_key_name(self,s_key):
        data = self.__read_config()
        if not s_key in data: return s_key
        return data[s_key]['name']
    
    # 设置防护对象状态
    def set_safe_path(self, get):
        data = self.__read_config()
        status = get.status in ['1', 'true', 'True', 'on', 'On', 'ON']
        is_exists = False
        for m_path in data[get.s_key]['paths']:
        ...
        self.__write_config(data)
        if data['open'] and data[get.s_key]['open']: 
            for m_path in data[get.s_key]['paths']:
                if not 'open' in m_path: m_path['open'] = True
                self.__set_safe_state([m_path], m_path['open'])
        
        msg = u'已将[%s]-[%s]目录的防护状态设置为[%s]' % (data[get.s_key]['name'],get.path, self.__state[m_path['open']])
        public.WriteLog(self.__name, msg)
        return public.returnMsg(True, msg)
    ...
    # 是否锁定或解锁
    def is_lock(self,path):
        _list = self.get_keyfile_config()
        for i in _list:
            if path == i['fullpath']:
                return False
        return True
    ...
    def __lock_path(self, pathInfo):
        try:
            if 'open' in pathInfo and not pathInfo['open']: return False
    ...
    def __list_safe_state(self, paths):
        ...
        for i in range(len(paths)):
            ...
            if 'open' not in paths[i]: paths[i]['open'] = True
    ...
    def __set_safe_state(self, paths, lock=True):
        for path_info in paths:
            # if not self.is_lock(path_info['path']): continue
            if lock:
...
last -n 1000 -x -f {}|grep -v 127.0.0.1|grep -v " begins"'''.format('/var/log/' + get.log_name)
    ...
    def delete_linux_user(self,get):
        ...
        self.lock_file()
        if res[1]:
            if res[1].find('home directory') == -1: # 忽略主目录不存在错误
    ...
    def get_ssh_tmout(self,get):
        ...
        res = public.ExecShell("source /etc/profile &>/dev/null && echo $TMOUT")[0].strip()
        if not res: res = '0'
        return public.returnMsg(True,{
            "title": "SSH超时时间",
            "value": int(res),
