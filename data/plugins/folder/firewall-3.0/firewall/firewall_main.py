...
class firewall_main:
    ...
    _white_list = []
    __DEL_STATUS_TIPS = "请在防火墙开启状态删除规则。"
    __CREATE_STATUS_TIPS = "请在防火墙开启状态添加规则。"
    __MODIFY_STATUS_TIPS = "请在防火墙开启状态编辑规则。"
    ...
    def create_forward(self, get):
        ...
        if d_ip:
            ...
            if d_ip in ["127.0.0.1", "localhost"]:
                d_ip = ""
        if public.M('firewall_trans').where("start_port=?",(s_port,)).count() > 0: return public.returnMsg(False,'该端口已存在，请勿重复添加！')
        if self.__isUfw:
    ...
    def ufw_handle_add(self, s_port, d_port, d_ip, protocol):
        ...
        if d_ip == "":
            ...
            _string = "-A PREROUTING -p {0} --dport {1} -j DNAT --to-destination {2}:{3}\n".format(protocol,s_port,d_ip,d_port)+ "-A POSTROUTING -d {0} -j MASQUERADE".format(d_ip)
    ...
    def create_country(self, get):
        ...
        if ports:
            ...
            for port in port_list:
                ...
                if public.M('firewall_country').where("country=? and ports=?",(country, port)).count() > 0: return public.returnMsg(False,'该国家已添加过，请勿重复添加！')
        self.get_os_info()
        ...
        for r in result:
            if brief == r["brief"]:
                ...
                break
        if not ip_list:
            return public.returnMsg(True, "请输入正确的国家名称！")
        if self.__isUfw:
