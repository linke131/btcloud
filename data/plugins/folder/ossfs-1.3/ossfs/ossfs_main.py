...
class ossfs_main:
    ...
    _secret_key = None
    _aes_key = None
    _ossfs_path= '/www/server/panel'
    _conf_path = 'plugin/ossfs/mount.conf'
    _oss_path = os.path.join(_ossfs_path,'data/ossfs.conf')
    _set_path='/www/ossfs'
    _access_key_id=''
    _access_key_secret=''
    _bucket_name=''
    _app_id=''
    _endpoint=''
    _mount_file=os.path.join(_ossfs_path,'data/oss_mount.conf')

    def __init__(self):
        if not os.path.isdir(os.path.join(self._set_path,'alioss')):os.makedirs(os.path.join(self._set_path,'alioss'))
        if not os.path.isdir(os.path.join(self._set_path,'bos')):os.makedirs(os.path.join(self._set_path,'bos'))
        # self._access_key_id='LTAI4GG1Syr3Hv7faFoTDaUD'
        # self._access_key_secret='mFvRbpVO7Lr1axFOTrKsOt5YP9Dow3'
        # self._bucket_name='thisyearbase'
        # self._endpoint='oss-cn-shenzhen.aliyuncs.com'
    ...
    def uninstall_check(self,get=None):
        ...
        for item in conf:
            ...
            if len(mlist) > 0:
                ...
                break
        if has_mount:
            return {'status':True, 'code': -1,'msg': '检测到有云存储未卸载，为保证数据安全,请在保证没有读写的情况下卸载所有云存储后再卸载插件！' } 
        return {'status': False }
    ...
    def get_oss_info(self,get):
        ...
        if conf:
            for ali_info in conf:
                if 'type' not in ali_info or 'key_info' not in ali_info or 'access_key_id' not in ali_info['key_info'] or 'access_key_secret' not in ali_info['key_info']:continue
    ...
    def check_bucket_mount(self,bucket_name):
        ...
        path = os.path.join(add_path,self._bucket_name)
        if self.check_path_mount(path):return True
        else:return False
    ...
    def get_oss_mount_list(self,args=None,is_get=False):
        ...
        if is_get:return self.get_page(data, args) 
        else:return data        
    ...
    def get_oss_region(self,oss_type):
        region_list=[]
        if oss_type =='alioss':
            region_list =[{'value':'oss-ap-south-1','title':'印度（孟买）'},{'value':'oss-ap-southeast-7','title':'泰国（曼谷）'},{'value':'oss-ap-southeast-6','title':'菲律宾（马尼拉）'},{'value':'oss-me-east-1','title':'阿联酋（迪拜）'},{'value':'oss-us-east-1','title':'美国（弗吉尼亚）'},{'value':'oss-us-west-1','title':'美国（硅谷）'},{'value':'oss-eu-west-1','title':'英国（伦敦）'},{'value':'oss-eu-central-1','title':'德国（法兰克福）'},{'value':'oss-ap-northeast-1','title':'日本（东京）'},{'value':'oss-ap-southeast-5','title':'印度尼西亚（雅加达）'},{'value':'oss-ap-southeast-3','title':'马来西亚（吉隆坡）'},{'value':'oss-ap-northeast-2','title':'韩国（首尔）'},{'value':'oss-ap-southeast-1','title':'新加坡'},{'value':'oss-ap-southeast-2','title':'澳大利亚（悉尼）'},{'value':'oss-cn-hongkong','title':'中国香港'},{'value':'oss-cn-chengdu','title':'西南1（成都）'},{'value':'oss-cn-guangzhou','title':'华南3（广州）'},{'value':'oss-cn-heyuan','title':'华南（2）（河源）'},{'value':'oss-cn-shenzhen','title':'华南1（深圳）'},{'value':'oss-cn-wulanchabu','title':'华北6（乌兰察布）'},{'value':'oss-cn-huhehaote','title':'华北5（呼和浩特）'},{'value':'oss-cn-zhangjiakou','title':'华北3（张家口）'},{'value':'oss-cn-beijing','title':'华北2（北京）'},{'value':'oss-cn-qingdao','title':'华北1（青岛）'},{'value':'oss-cn-nanjing','title':'华东5（南京）'},{'value':'oss-cn-shanghai','title':'华东2（上海）'},{'value':'oss-cn-hangzhou','title':'华东1（杭州）'}]
        elif oss_type =='bos':
            region_list =[{'value':'bj','title':'华北-北京'},{'value':'bd','title':'华北-保定'},{'value':'su','title':'华东-苏州'},{'value':'gz','title':'华南-广州'},{'value':'fsh','title':'华东-上海 '},{'value':'fwh','title':'金融华中-武汉'},{'value':'hkg','title':'中国香港'},{'value':'sin','title':'新加坡'}]
        elif oss_type =='jdoss':
            region_list =[{'value':'cn-north-1','title':'华北-北京'},{'value':'cn-south-1','title':'华南-广州'},{'value':'cn-east-1','title':'华东-宿迁'},{'value':'cn-east-2','title':'华东-上海'}]
        elif oss_type =='obs':
            region_list =[{'value':'af-south-1','title':'非洲-约翰内斯堡'},{'value':'cn-north-4','title':'华北-北京四'},{'value':'cn-north-1','title':'华北-北京一'},{'value':'cn-north-9','title':'华北-乌兰察布一'},{'value':'cn-east-2','title':'华东-上海二'},{'value':'cn-east-3','title':'华东-上海一'},{'value':'cn-south-1','title':'华南-广州'},{'value':'la-north-2','title':'拉美-墨西哥城二'},{'value':'na-mexico-1','title':'拉美-墨西哥城一'},{'value':'sa-brazil-1','title':'拉美-圣保罗一'},{'value':'la-south-2','title':'拉美-圣地亚哥'},{'value':'cn-southwest-2','title':'西南-贵阳一'},{'value':'ap-southeast-2','title':'亚太-曼谷'},{'value':'ap-southeast-3','title':'亚太-新加坡'},{'value':'ap-southeast-1','title':'中国-香港'}]
        elif oss_type =='txcos':
            region_list =[{'value':'ap-chengdu','title':'成都（中国）'},{'value':'ap-shanghai','title':'上海（中国）'},{'value':'ap-nanjing','title':'南京（中国）'},{'value':'ap-beijing','title':'北京（中国）'},{'value':'ap-guangzhou','title':'广州（中国）'},{'value':'ap-chongqing','title':'重庆（中国）'},{'value':'ap-shenzhen-fsi','title':'深圳金融（中国）'},{'value':'ap-shanghai-fsi','title':'上海金融（中国）'},{'value':'ap-beijing-fsi','title':'北京金融（中国）'},{'value':'ap-hongkong','title':'中国香港（中国）'},{'value':'ap-singapore','title':'新加坡'},{'value':'ap-mumbai','title':'孟买'},{'value':'ap-jakarta','title':'雅加达'},{'value':'ap-seoul','title':'首尔'},{'value':'ap-bangkok','title':'曼谷'},{'value':'ap-tokyo','title':'东京'},{'value':'na-siliconvalley','title':'硅谷（美西）'},{'value':'na-ashburn','title':'弗吉尼亚（美东）'},{'value':'na-toronto','title':'多伦多'},{'value':'sa-saopaulo','title':'圣保罗'},{'value':'eu-frankfurt','title':'法兰克福'},{'value':'eu-moscow','title':'莫斯科'}]
        elif oss_type =='ks3':
            region_list =[{'value':'beijing','title':'中国（北京）'},{'value':'shanghai','title':'中国（上海）'},{'value':'guangzhou','title':'中国（广州）'},{'value':'hk-1','title':'中国（香港）'},{'value':'rus','title':'俄罗斯'},{'value':'sgp','title':'新加坡'}]
        elif oss_type == 'qiniu':
            region_list =[{'value': 'cn-east-1', 'title': '华东-浙江'}, {'value': 'cn-east-2', 'title': '华东-浙江2'}, {'value': 'cn-north-1', 'title': '华北-河北'}, {'value': 'cn-south-1', 'title': '华南-广东'}, {'value': 'us-north-1', 'title': '北美-洛杉矶'}, {'value': 'ap-southeast-1', 'title': '亚太-新加坡（原东南亚）'}]
        return region_list
    ...
    def oss_mount(self,get):
        ...
        #
        if get:
            if 'bucket_name' not in get or 'path' not in get or 'type' not in get or not get.bucket_name or not get.path or not get.type:return public.returnMsg(False, '错误的参数')
    ...
    def oss_exec_mount(self, args):
        ...
        # print(path)
        check_str = ''
        if get.type == 'bos':check_str='bosfs'
        elif get.type == 'txcos':check_str='cosfs'
        else:check_str='s3fs'
        if not self.check_ossfs_install(check_str):
            self.install_ossfs()
        if not self.check_ossfs_install(check_str):return public.returnMsg(False, '{}未成功安装，无法挂载！'.format(check_str))
        if self.check_path_mount(path):return public.returnMsg(False, '挂载失败！挂载目录被占用，请重新设置挂载目录！')
        if not self.check_path(path):return public.returnMsg(False, '不能设置系统关键目录为挂载目录，请重新设置挂载目录！')
        ...
        #兼容多种对象存储
        if get.type == 'obs':self._endpoint='https://obs.{}.myhuaweicloud.com'.format(region_name)
        elif get.type =='jdoss':self._endpoint="https://s3.{}.jdcloud-oss.com".format(region_name)
        elif get.type =='alioss':self._endpoint='https://{}.aliyuncs.com'.format(region_name)
        elif get.type =='obs':self._endpoint='https://obs.{}.myhuaweicloud.com'.format(region_name)
        elif get.type =='txcos':self._endpoint='http://cos.{}.myqcloud.com'.format(region_name)
        elif get.type =='bos':self._endpoint='http://{}.bcebos.com'.format(region_name)
        elif get.type =='qiniu':self._endpoint='https://s3-{}.qiniucs.com'.format(region_name)
        if get.type=='jdoss':
            ...
            exec_cmd = 's3fs {} {} -o passwd_file={} -o url={}'.format(self._bucket_name,path,conf_file,self._endpoint)
            print('exec_cmd')
            print(exec_cmd)
            ...
            exec_cmd = 'bosfs {} {} -o endpoint={} -o ak={} -o sk={} -o allow_other -o mount_umask=0000'.format(self._bucket_name,path,self._endpoint,self._access_key_id,self._access_key_secret)
            print(exec_cmd)
            public.ExecShell(exec_cmd)
            ...
            exec_cmd = 'echo {}:{}:{} > {}'.format(self._bucket_name,self._access_key_id,self._access_key_secret,conf_file)
            public.ExecShell(exec_cmd)
            public.ExecShell('chmod 600 {}'.format(conf_file))
            exec_cmd ='cosfs {} {} -opasswd_file={} -ourl={} -odbglevel=info -onoxattr -oallow_other'.format(self._bucket_name,path,conf_file,self._endpoint)
            public.ExecShell(exec_cmd)
        elif get.type == 'ks3':
            exec_cmd = 'echo {}:{}> {}'.format(self._access_key_id,self._access_key_secret,conf_file)
            public.ExecShell(exec_cmd)
            ...
            for region in region_list:
                ...
                else:self._endpoint='https://ks3-cn-{}.ksyuncs.com'.format(region['value'])
                print(self._endpoint)
                exec_cmd = 's3fs {} {} -o passwd_file={} -o url={}'.format(self._bucket_name,path,conf_file,self._endpoint)
                print(exec_cmd)
        ...
        alioss_int_info['key_info']={'access_key_id':self._access_key_id,'access_key_secret':self._access_key_secret}
        mount_list =[]
    ...
    def clear_mount(self,file_name,check_str,is_passwd=False,is_umount=False,oss_type=None,mount_str =None):
        ...
        with open(file_name, "w", encoding="utf-8") as f_w:
            ...
            for line in lines:
                ...
                if is_umount and line[0]!='#' and len(line.split())>4 and line.split()[1] == check_str and replace_two == oss_type and line.split()[0]==mount_str:continue
                f_w.write(line)
        self.remove_blank_lines(self._mount_file)
    ...
    def oss_exec_unmount(self, args):
        ...
        mount_str = ''
        if get.type == 'bos':mount_str ='bosfs'
        elif get.type == 'txcos':mount_str ='cosfs'
        else:mount_str ='s3fs'
...
'''
    ...
    def create_bucket(self,get=None):
        ...
        if get:
            if 'bucket_name' not in get or 'region_url' not in get or 'type' not in get or not get.bucket_name or not get.region_url or not get.type:return public.returnMsg(False, '错误的参数')
        if 'type' not in oss_info or 'key_info' not in oss_info or 'access_key_id' not in oss_info['key_info'] or 'access_key_secret' not in oss_info['key_info'] or not oss_info['key_info']['access_key_id'] or not oss_info['key_info']['access_key_secret']:return public.returnMsg(False, '请配置密钥信息后再尝试创建!')
        import re
    ...
    def get_oss_error(self,oss_type,e):
        ...
        res = '创建失败！{}'.format(e)
        if oss_type == 'alioss':
            if e.args[0]==409 and 'Code' in e.args[-1] and e.args[-1]['Code'] == 'BucketAlreadyExists':res='bucket名称已经存在！'
            elif e.args[0]==400 and 'Code' in e.args[-1] and e.args[-1]['Code'] == 'InvalidBucketName':res='bucket名称不合法！'
            else:return e
        elif oss_type == 'bos':
    ...
    def set_oss_config(self,get):
        ...
        if not get.get('access_key_id/str','') or not get.get('access_key_secret/str','') or not get.get('type/str',''):return public.returnMsg(False, 'access_key_id/access_key_secret/bucket_name/endpoint_url参数不能为空或者不合法！')
...
class jdoss_main(ossfs_main):
    ...
    def create_bucket(self, args):
        ...
        try:
            ...
            print(type(e))
            e = str(e)
            if e.find('The specified bucket is not valid.')!=-1 or e.find('BucketAlreadyExists')!=-1 or e.find('The requested bucket name is not available.')!=-1:
...
class qiniu_main(ossfs_main):
    ...
    def __init__(self):
        ...
        if conf:
            for item in conf:
                if 'type' not in item or 'key_info' not in item or 'access_key_id' not in item['key_info'] or 'access_key_secret' not in item['key_info']:continue
