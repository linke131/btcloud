...
#——————————————————————————————
    ...
    def attack(self, args=None, path_id=None):
        ...
        if not sys_test and not sys_user:
            return public.returnMsg(False, "无法添加测试用户，请确认相关文件是否已被锁定，当确认可以添加用户后再次测试。")
        err_res =  {"status": True, "msg":"未进行模拟攻击", "attack_msg": ["正在检查保护目录[%s]的状态..."% rule["path"],]}
        if not (bool(rule["status"]) and bool(config["status"])): 
            err_res["attack_msg"].append("【保护列表】->【%s】未开启防篡改，无法测试。" % rule["path"])
            return err_res
        res = self._check_white_dirs(rule["path"], rule["white_dirs"])
        if res:
            err_res["attack_msg"].append("【保护列表】->【%s】->【配置】->【目录白名单】中的%s匹配了当前保护目录，这导致该目录下所有文件均未加入保护，无法测试。" %(rule["path"], str(res)))
            return err_res
        acts = [
            "is_mkdir",   # ---> 所有情况都可以测试
            "is_create",  # ----> 需要有保护的黑名单
        ...
        attack_config = {"attack_process": random_name, "base_path": rule["path"], "sys_test": sys_test, "sys_user": sys_user}
        for i, act in enumerate(acts):
            ...
            err_res["attack_msg"].append("【保护列表】->【%s】->【配置】->【基础设置】中未开启任何保护规则，无法测试。" % rule["path"])
            return err_res
        ...
        if not file_path:
            err_res["attack_msg"].append("【保护列表】->【%s】->【配置】->【受保护的文件后缀】中没有设置任何受保护的后缀，无法测试。" % rule["path"])
            return err_res
    ...
    def _do_attack(self, attack_config, config):
        ...
        if res.find("攻击失败") != -1:
            ...
            del perm
            return public.returnMsg(False,"测试失败，可能原因是您的系统环境与测试文件不兼容，但这不意味着您的防篡改系统未生效。")
        else:
            flag = False
        ...
        return {"status": flag, "msg":"已进行模拟攻击", "attack_msg": self._show_attack(attack_config, flag)}
    
    def _show_attack(self, attack_config, flag):
        start = "正在检查保护目录[%s]的状态"% attack_config["base_path"]
        acts = {
            "is_mkdir":'创建文件夹',   
            "is_create":'创建文件',  
            "is_link":'创建软链接',   
        ...
        prep = "准备阶段：" + ("创建测试用户[{}]".format(attack_config["atk_user"]) if attack_config["atk_user"] not in ("www", "root" ) else "选取测试用户为[{}]".format(attack_config["atk_user"]))
        if attack_config["type"] not in ("is_mkdir", "is_create"):
            prep += "，创建测试文件夹[{}]和测试文件[{}]".format(attack_config["dir"], attack_config["file"])
        attack = "攻击内容：使用测试用户运行模拟攻击程序[进程名:{}]执行{}操作".format(attack_config["attack_process"], acts[attack_config["type"]])
        end = "攻击结果：" + ("该次攻击已被拦截，防御成功" if flag else "该次攻击未拦截成功，请尽快联系官方解决问题" )
        return start, act_type, prep, attack, "结束阶段：清除本次测试中的所有临时文件", end
