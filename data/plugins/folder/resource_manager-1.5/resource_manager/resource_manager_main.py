...
do
    ...
    def get_site_request(self, site_name):
        ...
        if os.path.exists(path):
            ...
            for hour, value in spdata.items():
                request_count += value.get('GET', 0) + value.get('POST', 0)
    ...
    def get_site_qps(self, site_name):
        req_sec_file = '/www/server/total/total/' + site_name + '/req_sec.json'
        qps = int(public.readFile(req_sec_file)) if os.path.exists(req_sec_file) and time.time() - os.stat(req_sec_file).st_mtime < 10 else 0
        return qps
