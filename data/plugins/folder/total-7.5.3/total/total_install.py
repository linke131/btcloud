...
class total_install():
    ...
    def install_total(self) -> Optional[bool]:
        ...
        # 获取当前服务器的内存大小，用于设置监控报表的内存字典阈值
        import psutil
        memory_info = psutil.virtual_memory()
        total_memory = memory_info.total // 1024 // 1024
        if total_memory < 1024:
            os.system("sed -i 's/lua_shared_dict.*/lua_shared_dict bt_total 30m;/g' {}/vhost/nginx/total.conf".format(panel_path))
        elif 1024 < total_memory < 2048:
            os.system("sed -i 's/lua_shared_dict.*/lua_shared_dict bt_total 60m;/g' {}/vhost/nginx/total.conf".format(panel_path))
        elif 2048 < total_memory < 4096:
            os.system("sed -i 's/lua_shared_dict.*/lua_shared_dict bt_total 100m;/g' {}/vhost/nginx/total.conf".format(panel_path))
        elif 4096 < total_memory < 8192:
            os.system("sed -i 's/lua_shared_dict.*/lua_shared_dict bt_total 180m;/g' {}/vhost/nginx/total.conf".format(panel_path))
        elif 8192 < total_memory < 16384:
            os.system("sed -i 's/lua_shared_dict.*/lua_shared_dict bt_total 360m;/g' {}/vhost/nginx/total.conf".format(panel_path))
        elif 16384 < total_memory < 32768:
            os.system("sed -i 's/lua_shared_dict.*/lua_shared_dict bt_total 600m;/g' {}/vhost/nginx/total.conf".format(panel_path))
        elif total_memory > 32768:
            os.system("sed -i 's/lua_shared_dict.*/lua_shared_dict bt_total 1024m;/g' {}/vhost/nginx/total.conf".format(panel_path))
