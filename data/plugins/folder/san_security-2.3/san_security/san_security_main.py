...
class san_security_main:
    ...
    def __init__(self):
        ...
        if not os.path.exists(self.config):
            resutl = {}
            public.WriteFile(self.config, json.dumps(resutl))  # json.dumps将一个Python数据结构转换为JSON
        if os.path.exists(self.repair_json):
            self.__repair = json.loads(public.ReadFile(self.repair_json))
            # json.load()是用来读取文件;json.loads()是用来读取字符串,即，可以把文件打开，用readline()读取一行，然后json.loads()一行
    ...
    def ssh_security(self):
        ...
        tmp_check = {
            ...
            "Suggestions": "在/etc/ssh/sshd_config 文件按如相下设置参数",
            "repair": "Protocol 2",
            "rule": [
                {
                    "re": "\nProtocol\\s*(\\d+)",
        ...
        ret = self.check_san_baseline(tmp_check)
        if not ret: result.append(tmp_check)
    ...
    def panel_security(self):
        ...
        if not self.get_limitip():
            ret1 = {
                ...
                "Suggestions": "如果你的IP存在固定IP建议添加到面板的授权IP",
            ...
            if not get_port_default:
                ret1 = {
                    ...
                    "type": "file",
                    "name": "宝塔面板登陆端口未修改",
                    "Suggestions": "修改默认端口,例如8989或56641",
            ...
            if not get_admin_path:
                ret1 = {
                    ...
                    "type": "file",
                    "name": "宝塔面板登陆未开启安全入口",
                    "Suggestions": "修改安全入口例如 /123456789",
        ...
        for i in panel_chome:
            if not self.check_san_baseline(i):
                ret1 = {
                    ...
                    "Suggestions": "%s 权限改为%s 所属用户为%s" % (i['file'], i['chmod'], i['user']),
                    "repair": "%s 权限改为%s 所属用户为%s" % (i['file'], i['chmod'], i['user']),
                }
                result.append(ret1)
                ...
                if os.path.isdir(php_path + i):
                    if os.path.exists(php_path + i + '/etc/php.ini'):
                        php_data = {
                            ...
                            "Suggestions": "在%s expose_php的值修改为Off中修改" % (php_path + i + '/etc/php.ini'),
                ...
                if os.path.isdir(php_path + i):
                    if os.path.exists(php_path + i + '/etc/php.ini'):
                        php_data = {
                            ...
                            "file": php_path + i + '/etc/php.ini',
                            "Suggestions": "在%s 中 disable_functions= 修改成如下:" % (php_path + i + '/etc/php.ini'),
    ...
    def php_security(self):
        ...
        php_path = '/www/server/php/'
        if not os.path.exists(php_path): return []
        php_list = os.listdir(php_path)  # 返回信息，如['70', '73']
        if len(php_list) >= 1:
            for i in php_list:
                if os.path.isdir(php_path + i):
                    if os.path.exists(php_path + i + '/etc/php.ini'):
    ...
    def redis_security(self):
        ...
        redis_server_ip = {
            ...
            "Suggestions": "在%s 中的监听IP设置为127.0.0.1 例如" % ('/www/server/redis/redis.conf'),
        ...
        redis_server_not_pass = {
            ...
            "name": "Redis 查看是否设置密码",
            "file": '/www/server/redis/redis.conf',
            "Suggestions": "在面板软件商店-redis插件-性能调整中设置大于8位并且同时包含大写字母、小写字母、数字、标点符号的复杂密码",
            "repair": "例如:adM1#@$544..",
            "rule": [
        ...
        try:
            rett = public.ExecShell("cat /www/server/redis/redis.conf | grep 'requirepass .*'|grep -v '#'")[0]
            ret_tmp = rett.replace('requirepass', '').strip()
        except:
            ret_tmp = ''
        if ret_tmp:
            if self.checklisttest(ret=ret_tmp) == False:
                ret.append(redis_server_pass)
        # 查看版本是否是低于最新版本
        if os.path.exists('/www/server/redis/version.pl'):
            re2t = public.ReadFile('/www/server/redis/version.pl').strip()
            if re2t != '6.2.6':
                ret2 = {
                    'id': 45,
                    "type": "password",
    ...
    def memcache_security(self):
        ...
        memcache_bind = {
            ...
            "Suggestions": "在%s 中的监听IP设置为127.0.0.1 例如" % ('/etc/init.d/memcached'),
            "repair": "IP=127.0.0.1",
            "rule": [
    ...
    # 检测mysql root密码是否包含标点符号
    def charscheck(self, ret):
        # ret = public.M('config').field('mysql_root').select()[0]['mysql_root']
        chars = string.punctuation
        charslist = []
        for i in ret:
            if i in chars:
                charslist.append('Ture')
        if charslist != []:
            return True
        else:
            return False

    # 检测mysql root密码是否包含大小写字母、数字、标点符号
    def checklisttest(self, ret):
        # ret = public.M('config').field('mysql_root').select()[0]['mysql_root']
        checklist = []
        lenchecklist = []
        stringdigits = re.compile(r'[0-9]')
        stringlowercase = re.compile(r'[a-z]')
        stringuppercase = re.compile(r'[A-Z]')
        if stringdigits.search(ret) != None:
            checklist.append('Ture')
        if stringlowercase.search(ret) != None:
            checklist.append('Ture')
        if stringuppercase.search(ret) != None:
            checklist.append('Ture')
        if self.charscheck(ret=ret) == True:
            checklist.append('Ture')
        if self.lencheck(ret=ret) == True:
            checklist.append('Ture')
        if len(checklist) == 5:
            return True
        else:
    ...
    def get_root_pass(self):
        ...
        if len(ret) < 8:
            return False
        if self.checklisttest(ret=ret) == False:
            return False
        return True
    ...
    def chekc_mysql_user(self):
        ...
        ret = public.M('con'
                       ''
                       'fig').field('mysql_root').select()[0]['mysql_root']
    ...
    def mysql_security(self):
        ...
        if not self.get_root_pass():
            ret = {
                ...
                "Suggestions": "使用大于8位并且同时包含大写字母、小写字母、数字、标点符号的强密码。注意：部分特殊字符，如$等，因考虑冲突，不能使用面板数据库管理添加。",
                "repair": "例如:dM1#@a544..",
            }
            result.append(ret)
        ...
        if public.M('firewall').where('port=?', ('3306',)).count():
            ret = {
                ...
                "name": "3306 端口对外开放",
                "file": '/etc/my.cnf',
                "Suggestions": "建议3306不对外开放，如果是特殊需求可以忽略这次记录",
                "repair": "关闭3306对外访问",
            }
            result.append(ret)
    ...
    # 检测是否开启sftp
    def isorsftp(self):
        myconf = public.readFile('/etc/ssh/sshd_config')
        rep = r".*Subsystem\s*"
        repone = r'#*'
        a = re.search(rep, myconf)
        if not a:
            return False
        c = a[0]
        b = re.match(repone, c)
    ...
    def checkftps(self):
        ...
        if len(ret) < 1:
            ...
            for i in range(0, len(ret)):
                ...
                if len(rett) > 8:
                    checklist.append('Ture')
                result = re.findall('[a-z]', rett)
                if len(result):
                    checklist.append('Ture')
                result = re.findall('[A-Z]', rett)
                if len(result):
                    checklist.append('Ture')
                result = re.findall('[0-9]', rett)
                if len(result):
                    checklist.append('Ture')
                if self.charsftpcheck(ret=rett):
                    checklist.append('Ture')
                if len(checklist) != 5:
                    tmpftplist.append(ret[i]['name'])
        return tmpftplist

    def sftp_security(self):
        if not os.path.exists('/www/server/pure-ftpd'):
            return True
        result = []
        tmpftplist = self.checkftps()
        if tmpftplist == []:
            return True
        else:
            ftp_list = ''
            if len(tmpftplist) > 1:
                for i in range(0, len(tmpftplist)):
                    if i == len(tmpftplist) - 1:
                        ftp_list += tmpftplist[i]
                    else:
                        ftp_list += tmpftplist[i] + '、'
            else:
                ftp_list = tmpftplist[0]
            ...
            ret = {
                ...
                "Suggestions": "%s  以上ftp账户的密码使用长度大于8位且同时包含大写字母、小写字母、数字、特殊字符组合" % ftp_list,
                "repair": "例如:adM1#@$544..",
            }
            result.append(ret)

        if not self.isorsftp():
            ret = {
                'id': 73,
                "type": "password",
                "harm": "高",
                ...
                "name": "Sftp 未开启监测",
                "file": '/etc/ssh/sshd_config',
                "Suggestions": "/etc/ssh/sshd_config文件打开'Subsystem sftp /usr/libexec/openssh/sftp-server'代码段",
                "repair": "/etc/ssh/sshd_config文件打'Subsystem sftp /usr/libexec/openssh/sftp-server'代码段",
            }
            result.append(ret)
    ...
    def user_security(self):
        ...
        if len(self.user_not_password()) >= 1:
            user_len = {
                ...
                "name": "系统存在空密码的用户",
                "file": "/etc/login.defs ",
                "Suggestions": "为如下%s这些用户添加密码" % self.user_not_password(),
                "repair": "（如果用户不用可以删除）",
            }
    ...
    def tasks_security(self):
        ...
        for i in f.readlines():
            ...
            if i3 == None: continue
            if i3[0][0] == '#': continue
            if len(i3) < 5: continue
            tmp_value.append(i)
        if tmp_value != []:
            for n in range(0, len(tmp_value)):
                i = tmp_value[n].strip().split()
                if i[5]:
                    if '/www/server/' not in i[5] and 'flock' not in i[5]:
    ...
    def site_security(self):
        ...
        for i in site_lists:
            ...
            if not os.path.exists(path):
                ...
                if not ssl:
                    site['Suggestions'] = '使用https为访问方式'
                    site['repair'] = 'https 强制模式'
                    site['ps'] = '%s该站点未启用SSL' % i['name']
                else:
                    if tls:
                        site['Suggestions'] = '建议使用TLS1.2及以上的安全协议'
                ...
                site = {
                    "Suggestions":'请为站点{}部署SSL证书'.format(i['name']),
                    "user_ini": True,
                    "level": 1,
                    "name": '%s该站点未启用SSL' % i['name'],
                    "ssl": ssl,
    ...
    def Nginx_Apache_security(self):
        ...
        Nginx_Get_version = {
            ...
            "Suggestions": "在%s expose_php的值修改为Off中修改" % ('/www/server/nginx/conf/nginx.conf'),
            "repair": "expose_php = Off",
            "rule": [
        ...
        if os.path.exists('/www/server/nginx/version.pl'):
            ret2 = public.ReadFile('/www/server/nginx/version.pl').strip()
            tmp_value = ret2.split('.')
            ret2 = tmp_value[0] + '.' + tmp_value[1]
            nginx_tmp_check = ['1.8', '1.12', '1.14', '1.15', '1.16', '1.17','1.21']
            if ret2 in nginx_tmp_check:
                Nginx_Get_version = {
                    'id': 71,
                    "type": "file",
                    'level': "1",
                    ...
                    "harm": "低",
                    "file": '/www/server/nginx/conf/nginx.conf',
                    "Suggestions": "做好磁盘快照后，面板软件商店找到nginx，升级至最新版的Nginx 软件",
                    "repair": "例如：Nignx1.21 或者Nginx1.20",
                }
                ret.append(Nginx_Get_version)
        if os.path.exists('/www/server/apache/version_check.pl'):
            ret3 = public.ReadFile('/www/server/apache/version_check.pl').strip()
            apache_check = ['2.4.47', '2.4.48', '2.4.49', '2.4.50', '2.4.51']
            if ret3 in apache_check:
                Apache_Get_version = {
                    "type": "file",
                    'level': "1",
                    "repaired": "0",
                    "name": "Apache 版本为漏洞版本",
                    "harm": "高",
                    "file": '/www/server/apache/conf/httpd.conf',
                    "Suggestions": "做好磁盘快照后，面板软件商店找到apache，升级至最新版的apache 软件",
                    "repair": "例如：Apache2.4.52 ",
                }
                ret.append(Apache_Get_version)

        return ret

    def get_user_passwd_result(self,user_list):
        ret = {
                "type": "password",
                "harm": "高",
                "repaired": "1",
                ...
                "name": "系统用户密码过期风险",
                "file": '/etc/passwd',
                "Suggestions": "请检测以下用户是否设置密码过期时间、密码保留有效天数是否小于90天：{}".format(user_list),
                "repair": "请检测以下用户是否设置密码过期时间、密码保留有效天数是否小于90天：{}".format(user_list),
            }
        return ret

    def system_user_security(self):
        """
        @name 系统用户密码过期检测
        """
        result = []
        user_str = public.ExecShell('cat /etc/passwd |cut -d: -f1')
        tmp_user_lists= user_str[0].split('\n')
        user_lists= []
        for user in tmp_user_lists:
            if not user:continue
            if len(user.split())>1:continue
            user_lists.append(user)
        add_user = []
        for tmp_user in user_lists:
            tmp_result = public.ExecShell('chage -l {}'.format(tmp_user))
            is_inactive =False
            is_expires=False
            is_maximum=False
            for check_str in tmp_result[0].split('\n'):
                if not check_str:continue
                if check_str.find('Password expires')!=-1 and check_str.find('never')!=-1:
                    is_expires=True
                #兼容中文
                if check_str.find('密码过期时间')!=-1 and check_str.find('从不')!=-1:
                    is_expires=True
                if check_str.find('密码失效时间') !=-1 and check_str.find('从不')!=-1: 
                    is_inactive=True
                if check_str.find('两次改变密码之间相距的最大天数') !=-1: 
                    tmp_check_str= check_str.replace('两次改变密码之间相距的最大天数','')
                    tmp_check_str= tmp_check_str.replace(':','')
                    tmp_check_str= tmp_check_str.replace('：','')
                    tmp_check_str= tmp_check_str.strip()
                    if int(tmp_check_str)>90:
                        is_maximum=True
                #兼容中文
                if check_str.find('Maximum number of days between password change') !=-1: 
                    tmp_check_str= check_str.replace('Maximum number of days between password change','')
                    tmp_check_str= tmp_check_str.replace(':','')
                    tmp_check_str= tmp_check_str.replace('：','')
                    tmp_check_str= tmp_check_str.strip()
                    if int(tmp_check_str)>90:
                        is_maximum=True
            if is_inactive and is_expires and is_maximum:
                add_user.append(tmp_user)
        if add_user:
            result.append(self.get_user_passwd_result(add_user))
        return result
    ...
    def San_Entrance(self):
        ...
        # 新增ftp扫描监测 hezhihong 20210923
        time.sleep(1)
        sftp_file = self.sftp_security()
        self.Write(name='Ftp安全监测', count=14)
        time.sleep(1)
        system_user_file = self.system_user_security()
        self.Write(name='系统用户密码监测', count=15)
        ...
        aa = {
            ...
            "sftp_file": sftp_file,
            "system_user_file":system_user_file
        }
    ...
    def repair(self, get):
        ...
        if id in self.__repair:
            if self.repair_san_baseline(self.__repair[id]):
                return public.returnMsg(True, '修复成功')
            else:
                return public.returnMsg(False, '自动修复失败,可以尝试手动修复')
        else:
            return public.returnMsg(False, '自动修复失败,建议根据提示进行修复')
    ...
    def repair_all(self, get):
        ...
        return public.returnMsg(True, '修复完成，未能修复的请根据详情提示建议手动处理下')

    def main(self, get):
        return self.San_Entrance()
