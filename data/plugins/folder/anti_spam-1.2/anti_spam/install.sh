...
set_amavisd_default_conf(){
  if [ -f '/etc/redhat-release' ];then
	...
	cp $pluginPath/conf/centos/amavis/amavisd.conf /etc/amavisd/amavisd.conf
	...
	cp $pluginPath/conf/ubuntu/amavis/amavis.zip /etc/amavis/conf.d/amavis.zip
    unzip amavis.zip
    rm -f amavis.zip
  fi
}
...
set_spamassassin_default_conf(){
  if [ ! -f '/etc/redhat-release' ];then
	...
	cp $pluginPath/conf/ubuntu/spamassassin/spamassassin /etc/default/spamassassin
    ...
    if [ ! -f '/var/lib/spamassassin/3.004002/updates_spamassassin_org/10_default_prefs.cf' ];then
	  cp $pluginPath/conf/ubuntu/spamassassin/update.tar.gz /var/lib/spamassassin/3.004002/updates_spamassassin_org/update.tar.gz
...
Install_spam_anti()
{
  if [ -f '/etc/redhat-release' ];then
    ...
    if [ "$?" -ne 0 ];then
      ...
      yum install perl -y
      rpm -ivh $pluginPath/perl-Digest-SHA1-2.13-23.el8.x86_64.rpm
      rpm -ivh $pluginPath/perl-IO-stringy-2.111-9.el8.noarch.rpm
      rpm -ivh $pluginPath/perl-common-sense-3.7.4-8.el8.x86_64.rpm
    fi
