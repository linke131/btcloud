...
class anti_spam_main:
    ...
    _parameters = {'max_servers': '最大工作进程数',
                  'local_domains_maps': '需要保护的邮件域名',
                  'inet_socket_bind': '监听的IP地址',
                  'inet_socket_port': '监听的端口',
                  'sa_tag_level_deflt': '当邮件分数小于它时，在邮件源添加邮件分数',
                  'sa_tag2_level_deflt': '当邮件分数小于它时，在邮件主题添加内容（默认添加 **Spam**)',
                  'final_virus_destiny': '检测到病毒时执行什么动作',
                  'final_banned_destiny': '检测到受禁止的内容时执行什么动作',
                  'final_spam_destiny': '检测到垃圾邮件、广告邮件（spam）时执行什么动作',
                  'final_bad_header_destiny': '检测到不良信件时执行什么动作',
                  'sa_spam_subject_tag': '检测到垃圾邮件时在邮件标题添加的内容',
                  'sa_mail_body_size_limit': '最大扫描多大的邮件'}
    ...
    def _init_syslog_centos_conf(self):
        ...
        modify at: 2020-06-06
        """
        public.ExecShell('yum install rsyslog -y && systemctl restart rsyslog')
    ...
    def _init_syslog_ubuntu_conf(self):
        public.ExecShell('apt install rsyslog -y && systemctl restart rsyslog')
    ...
    def get_service_status(self,get):
        ...
        return {'amavisd':{'status':amavisd,'ps':'用于调度邮件扫描'},
                'clamd@amavisd':{'status':clamd,'ps':'用于扫描邮件病毒'},
                'spamassassin':{'status':spamassassin,'ps':'用于扫描垃圾邮件'},
                'log_monitor':{'status':log_monitor,'ps':'用于记录拦截次数'}}
    ...
    def _check_args(self,get):
        ...
        if hasattr(get,'max_servers'):
            try:
                ...
                if get.max_servers[0] == '-':
                    return public.returnMsg(False, "max_servers 只能填入正整数")
            except:
                return public.returnMsg(False,"max_servers 只能填入正整数")
        if hasattr(get,'inet_socket_bind'):
            ...
            if not search(rep_ip, get.inet_socket_bind):
                return public.returnMsg(False,"inet_socket_bind 请输入正确的IP地址，如果你想监听所有IP请输入 0.0.0.0")
        ...
        if hasattr(get,'inet_socket_port'):
            try:
                if int(get.max_servers) < 1 or int(get.max_servers) > 65535:
                    return public.returnMsg(False, "inet_socket_port 只能填入 1-65535 之间的数字")
                if get.max_servers[0] == '-':
                    return public.returnMsg(False, "inet_socket_port 只能填入 1-65535 之间的数字")
            except:
                return public.returnMsg(False,"inet_socket_port 只能填入 1-65535 之间的数字")
        if hasattr(get,'sa_tag_level_deflt'):
            try:
                ...
                return public.returnMsg(False,"max_servers 只能填入数字")
    ...
    def __clamd_switch(self, act):
        ...
        try:
            ...
            return public.returnMsg(False, '没有找到对应的配置！')
  ...
  ['ClamAV-clamd',
    ...
    qr/^.*?: (?!Infected Archive)(.*) FOUND$/m ],
            ...
            return public.returnMsg(False, '不支持的操作！')
