...
if [ $? -eq 0 ];then
    ...
    if [ -f "/usr/bin/btpip" ];then
      ...
      if [ $tmp != '2.39.0' ];then
        ...
        btpip install google-api-python-client==2.39.0 -i https://pypi.Python.org/simple
      fi
      tmp=$(btpip list|grep google-auth-httplib2|awk '{print $2}')
      if [ $tmp != '0.1.0' ];then
        btpip uninstall google-auth-httplib2 -y
        btpip install google-auth-httplib2==0.1.0 -i https://pypi.Python.org/simple
      fi
      tmp=$(btpip list|grep google-auth-oauthlib|awk '{print $2}')
      if [ $tmp != '0.5.0' ];then
        btpip uninstall google-auth-oauthlib -y
        btpip install google-auth-oauthlib==0.5.0 -i https://pypi.Python.org/simple
      fi
      tmp=$(btpip list|grep -E '^httplib2'|awk '{print $2}')
      if [ $tmp != '0.18.1' ];then
        btpip uninstall httplib2 -y
        btpip install httplib2==0.18.1 -i https://pypi.Python.org/simple
      fi
    else
      pip install  pyOpenSSL
      pip install  google-api-python-client==2.39.0 google-auth-httplib2==0.1.0 google-auth-oauthlib==0.5.0 -i https://pypi.Python.org/simple
      pip install httplib2==0.18.1 -i https://pypi.Python.org/simple
    fi
