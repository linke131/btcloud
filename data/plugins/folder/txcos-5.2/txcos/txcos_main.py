    ...
    def get_config(self):
        ...
        try:
            ...
            if not os.path.exists(bak_config_path):
                add_str = data['secret_id']+'|'+data['secret_key']+'|'+data['region']+'|'+data['bucket_name']+'|'+data['backup_path']
    ...
    def upload_by_shell(self,path,to_path):
        ...
        with open(path, "rb") as fp:
            ...
            while left_size > 0:
                ...
                response =oss_client.upload_part(Bucket=self.__bucket_name,Key=object_name,Body=part ,UploadId=upload_id, PartNumber=part_number
                                            ) 
                ...
                if is_last:
                    ...
                    oss_client.complete_multipart_upload(Bucket=self.__bucket_name,Key=object_name,UploadId=upload_id, MultipartUpload={'Part':part_list})
...
class txcos_main:
    ...
    def set_config(self, get):
        try:
            ...
            if secret_id.find('*') !=-1 or secret_key.find('*') !=-1 or bucket_name.find('*') !=-1 or  region.find('*') !=-1:return public.returnMsg(False, self.__before_error_msg)
            if not backup_path:
