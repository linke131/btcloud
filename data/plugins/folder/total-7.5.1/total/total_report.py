...
def automatic_generate_week_report(site):
    ...
    # print(time.localtime(time.mktime((now.tm_year, now.tm_mon, 0, 0, 0, 0, week-1, 0,0))))
    base_time = now
    bcount = 10
    reports = []
    while bcount > 0:
        start, end = get_last_week(base_time=base_time)
        # print("{}年第{}周:".format(year, week))
        # print("Start: {}".format(time.strftime("%Y%m%d %H:%M:%S", start)))
        # print("End: {}".format(time.strftime("%Y%m%d %H:%M:%S", end)))
        report_info = generate_week_report(site, cycle_start=start, cycle_end=end)
        if report_info:
            reports.append(report_info)

        bcount -= 1
        base_time = time.localtime(time.mktime((start.tm_year, start.tm_mon, start.tm_mday, 0,0,0,0,0,0)))
        week = int(time.strftime("%W", start))
        year = time.strftime("%Y", start)
    return reports
        
def automatic_generate_report():
    sites = public.M('sites').field('name').order("addtime").select()
    config = json.loads(public.readFile("/www/server/total/config.json"))
    reports = []
    global_push_m = False
