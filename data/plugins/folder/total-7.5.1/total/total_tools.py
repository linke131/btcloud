...
def write_site_domains():
    ...
    for my_site in sites:
        tmp_domains = public.M('domain').where('pid=?',
                                               (my_site['id'],)).field('name,port').select()
        ...
        if normal_domains:
            ...
            tmp["port"] = normal_port
        if generic_domains:
            tmp["generic"] = generic_domains
            tmp["port"] = generic_port
