...
class node_admin_main:
    ...
    def create_node(self,args):
        '''
            ...
            @return dict
        '''
        args_names = ['title','address','group_id','connect_type','connect_info','ps']
        pdata = {}
    ...
    def get_sync_task_list(self,args):
        ...
        data = public.get_page(count,int(args.p),int(args.rows),args.callback)
        data['data'] = self._sql.table('node_task').limit(data['shift'] + ',' + data['row']).order('state asc').select()
    ...
    def run(self):
        ...
        '''
        task_list = self._sql.table('node_task').where('state=? or state=?',(0,-1)).select()
        if not task_list: self.echo_log("没有需要执行的任务!")
