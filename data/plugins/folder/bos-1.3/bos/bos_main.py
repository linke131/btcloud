...
class OSClient(object):
    ...
    _aes_status=os.path.join(_panel_path,'plugin/bos/aes_status')
...
class BOSClient(OSClient, object):
    _title = "百度云BOS"
    _name = "bos"
    __error_msg = "ERROR: 无法连接到百度云BOS服务器，请检查[" \
    ...
    def authorize(self):
        ...
        try:
            ...
            raise OsError("百度云BOS客户端连接异常, 请检查配置参数是否正确。")
    ...
    def upload_by_shell(self,path,to_path):
        ...
        bos_client = None
        try:
            bos_client = self.authorize()
            upload_id = bos_client.initiate_multipart_upload(self.__bucket_name, object_name).upload_id
        except Exception as e:
        ...
        self.echo_info('正在上传到百度云存储，请稍候...'.format(public.format_date()))
        ...
        while left_size > 0:
            ...
            response = bos_client.upload_part_from_file(
                self.__bucket_name, object_name, upload_id, part_number, part_size, path, offset)
            ...
            if is_last:
                ...
                bos_client.complete_multipart_upload(self.__bucket_name, object_name, upload_id, part_list)
    ...
    def get_list(self, path="/", delimiter="/"):
        ...
        response = bos_client.list_objects(self.__bucket_name, prefix=path,
                                           delimiter="/")
        # print("{} list files:".format(path))
    ...
    def generate_download_url(self, object_name):
        try:
            ...
            url = bos_client.generate_pre_signed_url(self.__bucket_name,
                                                     object_key)
            # 以下代码兼容python2环境
            # 获取url问题
            if is_py3:
    ...
    def create_dir(self, dir_name):
        ...
        try:
            ...
            bos_client = self.authorize()
            object_key = dir_name
            result = bos_client.put_object_from_file(bucket_name, object_key,
                                                     file_name)
            try:
                os.remove(file_name);
            except:
    ...
    def delete_object_by_os(self, object_name):
        ...
        # 注意：BOS SDK不论object name是否存在，删除请求状态都返回204。
        ...
        bos_client = self.authorize()
        object_key = object_name
        result = bos_client.delete_object(self.__bucket_name, object_key)
    ...
    def batch_delete(self, object_names):
        ...
        try:
            bucket_name = self.__bucket_name
            bos_client = self.authorize()
            key_list = object_names
            result = bos_client.delete_multiple_objects(bucket_name, key_list)
            if result.status == 200 or result.status == 204:
                return True
        except Exception as e:
...
class bos_main(object):
    ...
    def set_config(self, get):
        try:
            ...
            _client = BOSClient(load_config=False)
    ...
    def get_lib(self):
        ...
        info = {
            ...
            "type": "计划任务",
            "ps": "将网站或数据库打包备份到百度云BOS对象存储空间,百度云BOS提供100GB免费存储空间, "
                  "<a class='link' "
                  "href='https://cloud.baidu.com/product/bos.html?track=navigation20200904' target='_blank'>点击申请</a>",
            "status": 'false',
            "opt": "bos",
            "module": "bos",
            "script": "bos",
            "help": "http://www.bt.cn/bbs/thread-1061-1-1.html",
            "AccessKeyID": "AccessKeyID|请输入AccessKeyID|百度云的AccessKeyId",
            "SecretKey": "SecretKey|请输入SecretKey|百度云的AccessKeySecret ",
            "Bucket": "存储空间名称|百度云BOS中您创建的Bucket名称",
            "Domain": "百度云BOS地域域名，不包括Bucket名",
            "backup_path": "备份保存路径, 默认是/bt_backup",
            "check": [
                "/www/server/panel/plugin/bos",
            ]
        }
