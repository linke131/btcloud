...
class mail_sys_main:
    ...
    def check_rspamd_page(self):
        ...
        if not os.path.exists(rspamd_path+'rspamd/') and os.path.isfile(src_rspamd_path):
            public.ExecShell('unzip {} -d {}'.format(src_rspamd_path,rspamd_path))
    ...
    def check_mail_sys(self, args):
        if os.path.exists('/etc/postfix/sqlite_virtual_domains_maps.cf'):
            public.ExecShell('/sbin/postconf -e "message_size_limit = 102400000"')
            ...
            if not result:
                return public.returnMsg(False, "postfix配置文件中找不到mydestination配置项")
            result = result.group(1)
            if 'localhost' in result or '$myhostname' in result or '$mydomain' in result:
                public.ExecShell('/sbin/postconf -e "mydestination =" && systemctl restart postfix')
            # 修改dovecot配置
    ...
    def _check_a(self, hostname):
        ...
        try:
            ...
            if str(type(error_ip)).find("dns.rdtypes.IN.A") != -1:
                self._session[key] = {"status": 0, "v_time": now, "value": error_ip.to_text()}
            else:
    ...
    def repair_dovecot(self, args=None):
        ...
        return public.returnMsg(status,"修复{}！".format("成功" if status else "失败"))
...
systemctl restart  opendkim'''.format(domain=domain)
    ...
    def add_domain(self, args):
        ...
        if not self._check_a(a_record):
            return public.returnMsg(False, 'A记录解析失败 <br>域名：{}<br>IP  ：{}'.format(a_record,self._session['{}:A'.format(a_record)]['value']))
        ...
        try:
            ...
            return public.returnMsg(False, '邮局没有初始化成功！<br>'
                                           '请尝试重新初始化,<br>'
...
'''
        ...
        return public.returnMsg(True, '{0}执行{1}操作成功'.format(service_name, type))
    ...
    def get_sent_mails(self, args):
        ...
        try:
            ...
            print(public.get_error_info())
            return public.returnMsg(False, '获取已发送邮件失败,错误原因[{0}]'.format(str(e)))
    ...
    def set_postfix_ssl(self, csrpath, keypath, act):
        ...
        for i in [[main_conf, main_file], [master_conf, master_file]]:
            if not i[0]:
                return public.returnMsg(False, "找不到postfix配置文件 {}".format(i[1]))
    ...
    def set_dovecot_ssl(self, csrpath, keypath, act):
        ...
        if not ssl_conf:
            return public.returnMsg(False, "找不到dovecot配置文件 {}".format(ssl_file))
        if act == "0":
            csrpath = "/etc/pki/dovecot/certs/dovecot.pem"
            keypath = "/etc/pki/dovecot/private/dovecot.pem"
    ...
    def set_ssl(self, args):
        ...
        if hasattr(args, "act") and args.act == "1":
            if args.key.find('KEY') == -1: return public.returnMsg(False, '私钥错误，请检查!')
            if args.csr.find('CERTIFICATE') == -1: return public.returnMsg(False, '证书错误，请检查!')
            public.writeFile('/tmp/mail_cert.pl', str(args.csr))
            if not public.CheckCert('/tmp/mail_cert.pl'): return public.returnMsg(False, '证书错误，请以pem格式粘贴正确的证书!')
...
if header :contains "X-Spam-Flag" "YES" {
    ...
    def get_backup_task_status(self, get):
        c_id = public.M('crontab').where('name=?', u'[勿删]堡塔邮局-数据备份任务').getField('id')
...
/sbin/postconf -e "smtpd_tls_key_file = /etc/pki/dovecot/private/dovecot.pem"
    ...
    def enable_catchall(self, args):
        ...
        if catchall_exist:
            ...
            create_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
            self.M('alias').add('address,goto,domain,created,modified,active', (domain, email, args.domain.strip(), create_time, create_time, '1'))
