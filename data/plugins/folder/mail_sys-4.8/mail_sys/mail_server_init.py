...
'''
    ...
    def create_ssl(self, get=None):
        import OpenSSL
        import mail_sys_main
        mail_sys_main.mail_sys_main().back_file('/etc/pki/dovecot/certs/dovecot.pem')
        mail_sys_main.mail_sys_main().back_file('/etc/pki/dovecot/private/dovecot.pem')
        ...
        if len(cert_ca) > 100 and len(private_key) > 100:
            ...
            mail_sys_main.mail_sys_main().restore_file('/etc/pki/dovecot/certs/dovecot.pem')
            mail_sys_main.mail_sys_main().restore_file('/etc/pki/dovecot/private/dovecot.pem')
            return False
    ...
    def check_sqlite(self):
        if not public.ExecShell('postconf -m | grep sqlite')[0].strip():
            return public.returnMsg(False,"Postfix不支持Sqlite")
