...
class tamper_proof_main:
    ...
    def sim_test(self, get):
        ...
        result = public.ExecShell(cmdString, user='www')[0]
        time.sleep(0.5)
        if result:
            public.WriteLog("网站防篡改", result)
            return public.returnMsg(False, "模拟测试失败,命令执行异常,请查看面板日志")
        if os.path.exists(php_name):
            if os.path.exists(php_name):
                public.ExecShell("rm -rf %s" % php_name)
    ...
    def site_reload(self, siteInfo):
        site_path = siteInfo['path'].replace(' ', '\ ')
        cmd = "{} {} {}".format(public.get_python_bin(), self.__plugin_path + '/tamper_proof_service.py unlock', site_path)
        public.ExecShell(cmd)

        cmd = "{} {}".format(public.get_python_bin(), self.__plugin_path + '/tamper_proof_service.py reload')
        public.ExecShell(cmd)
    ...
    def ClearDayLog(self, get):
        ...
        if get.day in data['days']:
            log_path = path + "/" + get.day + "/logs.json"
            total_path = path + "/" + get.day + "/total.json"
            if not os.path.exists(log_path):
                return public.returnMsg(False, u'没有该日志文件!')
            public.ExecShell("echo ''> %s" % log_path)
            totalData = {"total": 0, "delete": 0, "create": 0, "modify": 0, "move": 0}
            public.writeFile(total_path, json.dumps(totalData))
            public.WriteLog('防篡改程序', log_path + " 防篡改日志清理成功")
