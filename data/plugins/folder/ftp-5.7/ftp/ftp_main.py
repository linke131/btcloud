...
def get_client(use_sftp=None, load_config=True, timeout=None):
    is_install = public.ExecShell(f"btpip list | grep paramiko")[0]
    if not is_install:  # 安装
        public.ExecShell(f"nohup btpip install paramiko >/dev/null 2>&1 &")
    elif is_install.find("2.12.0") == -1:  # 更新指定版本呢
        public.ExecShell(f"nohup btpip install -U paramiko==2.12.0 >/dev/null 2>&1 &")
