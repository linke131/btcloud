		...
		bt_tools.table({
			...
			column: [
				...
				},
				{
					...
					width: '100px',
					template: function (item) {
						return '<span>' + that.ip_country_str(item.ip_country) + '</span>';
					}
				},
				{
					fid: 'server_name',
				...
				$.each(_dynamic, function (index, item) {
					_dynamic_html +=
						'<div class="item">\
							...
							<span>' + that.ip_country_str(item.ip_country) +'</span>\
			...
			$.each(res.map.top10_ip, function (index, item) {
				html +=
					'<div class="item">\
						...
						<span>' + that.ip_country_str(item[2]) +'</span>\
						<span>' +item[1] +'次</span>\
					</div>';
			});
			...
			$.each(data, function (index, item) {
				$('#protectTableBody').append(
    				...
    				<td><a class="btlink ' +
							...
							'</a></td>\
    				' +
							(num == 0 ? '<td>' + that.ip_country_str(item[2]) + '</td>' : '') +
							'\
    				<td>' +
							item[1] +
							'</td>\
		...
		$.each(config, function (index, item) {
			$('#reportLogBody').append(
				...
				<td>' +
						item.server_name +
						'</td>\
				<td>' + that.ip_country_str(item.ip_country)
						 +
						'</td>\
				<td>' +
						that.escapeHTML(item.uri).substring(0, 30) +
	...
	create_dynamic_table: function (obj) {
		...
		$.each(_dynamic, function (index, item) {
			...
			<td>' +
						...
						'</td>\
			<td>' +
			that.ip_country_str(item.ip_country) +
						'</td>\
		...
		var _intercept = bt_waf_tools.table({
			...
			column: [
				...
				},
				{
					...
					width: '120px',
					template: function (row, index) {
						row.ip_country = row.ip_country == '台湾' || row.ip_country == '香港' || row.ip_country == '澳门' ? '中国' + row.ip_country : row.ip_country;
						if (row.ip_country == '内网地址') return '<span title="' + row.ip_country + '">' + row.ip_country + '</span>';
						return '<span title="' + row.ip_country + '">' + row.ip_country + '</span>';
					},
				},
				{
					fid: 'filter_rule',
