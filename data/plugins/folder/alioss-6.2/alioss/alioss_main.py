...
class OSClient(object):
    ...
    def set_config(self, conf):
        """写入配置文件"""
        aes_status ='/www/server/panel/alioss/aes_status'
        a_pass ='/www/server/panel/data/a_pass.pl'
        if not os.path.isfile(a_pass):
            public.writeFile(a_pass,'VE508prf'+public.GetRandomString(10))
        
        public.writeFile(aes_status,'True')
        path = self.get_config_file()
        
        aes_key = public.readFile(a_pass)
    ...
    def upload_to(self, file_name, object_name=None, *args, **kwargs):
        """按照数据类型上传文件
...
class OSSClient(OSClient, object):
    ...
    backup_path = None
    reload = False
    __aes_status='/www/server/panel/plugin/alioss/aes_status'
    ...
    def get_config(self):
        ...
        try:
            # path = self.get_config_file()
            path = os.path.join(public.get_plugin_path(),'alioss/config.conf')
            ...
            decrypt_key = public.readFile('/www/server/panel/data/a_pass.pl')
            ...
            if isinstance(conf, str) :
                try:
                    ...
                    if not os.path.isfile(self.__aes_status):public.writeFile(self.__aes_status,'True')
            ...
            if not os.path.exists(bak_config_path):
                add_str = data['access_key']+'|'+data['secret_key']+'|'+data['bucket_name']+'|'+data['bucket_domain']+'|'+data['backup_path']
    ...
    def upload_by_shell(self,path,to_path):
        ...
        error_message = '阿里云OSS客户端连接异常, 请检查配置参数是否正确。'
        offset = 0
        part_number = 1
        ...
        oss_client = None
        try:
            oss_client = self.get_bucket()
            # print('22222222')
            upload_id=oss_client.init_multipart_upload(object_name).upload_id
            # print('33333333333')
        except Exception as e:
            # print('1111111111111')
        ...
        self.echo_info('正在上传到阿里云存储，请稍候...'.format(public.format_date()))
        ...
        with open(path, 'rb') as fileobj:
            while left_size > 0:
                ...
                response =oss_client.upload_part(object_name, upload_id, part_number,
                                            SizedFileAdapter(fileobj, part_size)) 
                part_list.append(PartInfo(part_number, response.etag))
                left_size -= part_size
                offset += part_size
                ...
                if is_last:
                    ...
                    oss_client.complete_multipart_upload(object_name, upload_id, part_list)
