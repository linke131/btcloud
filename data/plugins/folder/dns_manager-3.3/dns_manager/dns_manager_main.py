...
"""
    ...
    def set_listen_ip(self,get):
        ...
        if "listen_ip" not in values:
            return public.ReturnMsg(False, "请输入监听地址")
        ip = values["listen_ip"]
...
""" % (domain, domain)
    ...
    def import_data2(self,get):
        ...
        if not data:
            return public.returnMsg(False, "导入文件为空或导入失败")
        data = json.loads(data)
        logfile = "{}import.log".format(self.path)
        public.writeFile(logfile,"")
        for i in data:
            if i in domain_list:
                public.writeFile(logfile,"域名已经存在，跳过 {} 导入\n".format(i),"a+")
                continue
            # 导入记录
        ...
        if hasattr(get,"ns1domain"):
            if not get.ns1domain:
                ...
                if re.search(rep_domain,get.ns1domain):
                    ...
                    return public.ReturnMsg(False, "请检查 NS1服务器域名格式是否正确")
        if hasattr(get,"ns2domain"):
            if not get.ns2domain:
                ...
                if re.search(rep_domain,get.ns2domain):
                    ...
                    return public.ReturnMsg(False, "请检查 NS2服务器域名格式是否正确")
        ...
        if hasattr(get,"soa"):
            if not get.soa:
                ...
                if re.search(rep_domain,get.soa):
                    ...
                    return public.ReturnMsg(False, "请检查SOA格式是否正确")
