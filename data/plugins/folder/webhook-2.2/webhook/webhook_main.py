...
class webhook_main:
    ...
    def GetList(self,get):
        ...
        data = sorted(data, key=lambda b: b['addtime'], reverse=True)

        if 'p' in get and 'limit' in get:
            import page
            # page, data_list = page.page_data(get, data)
            p = int(get.p)
            limit = int(get.limit)
            info = {
                "p": p,
            ...
            new_page = page.Page()
            # 获取分页数据
            page = new_page.GetPage(info)
            start_idx = (int(p) - 1) * limit
            end_idx = p * limit
            data_list = data[start_idx:end_idx]
