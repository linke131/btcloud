...
class nodejs_main:
    ...
    _node_url = 'https://cdn.npmmirror.com/binaries/node/'
    _node_index_url = 'https://cdn.npmmirror.com/binaries/node/index.json'
    ...
    def get_registry_url(self,get = None):
        ...
        if not os.path.exists(self._registry_file): return  'https://registry.npmmirror.com/' #'https://registry.npmjs.org/' #'https://registry.npm.taobao.org/'
...
'''
    ...
    def clear_nvm_env(self,get = None):
        ...
        return self.return_data(True,'清理成功!')
    ...
    def get_online_version_list(self,get = dict_obj):
        ...
        if not os.path.exists(self._list_file) or get['force'] == '1':
            try:
                result = public.httpGet(self._node_index_url,timeout=5)
                http_result = json.loads(result)
...
'''.format(prefix,registry,cache_path,init_module)
    ...
    def get_version_info(self,get = dict_obj):
        ...
        return self.return_data(False,error_msg='没有找到对应的版本')
    ...
    def install_nodejs(self,get):
        ...
        if version_info['setup']: return self.return_data(False,'',error_msg='已经安装了该版本')
        if not os.path.exists(self._nodejs_path): os.makedirs(self._nodejs_path, 384)
        fname = self._get_version_filename()
        namex = fname.replace('.tar.gz','')
        if not namex in version_info['files']:
            return self.return_data(False,'',error_msg='指定版本不兼容您的服务器操作系统')
        ...
        if not os.path.exists(save_file):
            return self.return_data(False,'',error_msg='文件下载失败，请稍候重试!')
        ...
        return self.return_data(True,'安装成功')
    ...
    def uninstall_nodejs(self,get):
        ...
        if not version_info['status']: return version_info
        version_info = version_info['data']
        if not version_info['setup']: return self.return_data(False,'',error_msg='该版本不需要卸载')
        re_path = '{}/{}'.format(self._nodejs_path,version)
        if not os.path.exists(re_path): return self.return_data(False,'',error_msg='该版本不存在')
    ...
    def get_modules(self,get):
        '''
            ...
            @return list
        '''
        if not 'version' in get: return public.returnMsg(False,'缺少参数!')
    ...
    def upgrade_module(self,get):
        ...
        if not 'version' in get or not 'module' in get: return self.return_data(False,'',error_msg='缺少参数!')
        version = get['version'].strip()
        module = get['module'].strip()
        if module == 'npm':
            if version[:2] in ['v4','v6']: return self.return_data(False,'',error_msg='当前已经是最新版，无需升级!')
        npm_bin = os.path.join(self._nodejs_path,version,'bin/npm')
        mod_path = os.path.join(self._nodejs_path,version,'lib/node_modules')
        if not os.path.exists(npm_bin): return self.return_data(False,'',error_msg='指定node版本未安装!')
        module = module.strip()
        if not self.module_exists(version,module):
            return self.return_data(False,'',error_msg='指定模块未安装!')
...
{} update -global {} &> {}'''.format(self.get_shell_env(version),npm_bin,module,self._exec_log)
        ...
        if not os.path.exists(mod_pack_file):
            return self.return_data(False,'',error_msg='升级失败!')
        mod_pack_info = json.loads(public.readFile(mod_pack_file))
        if old_mod_pack_info['version'] == mod_pack_info['version']: return self.return_data(False,'',error_msg='当前已经是最新版，无需升级!')

        return self.return_data(True,'已成功将{}模块升级到{}'.format(module,mod_pack_info['version']))
    ...
    def install_module(self,get):
        ...
        if self.module_exists(version,module):
            return self.return_data(False,'',error_msg='指定模块已经安装过了!')
...
{} install {} -g &> {}'''.format(self.get_shell_env(version),npm_bin,module,self._exec_log)
        res = public.ExecShell(exec_cmd)
#         if module in ['pm2']:
#             exec_cmd = '''{}
# {} install {} -g &> {}'''.format(self.get_shell_env(version),npm_bin,'yarn',self._exec_log)
#             public.ExecShell(exec_cmd)
        if not os.path.exists(mod_pack_file):
            return self.return_data(False,res,error_msg='安装失败!')
    ...
    def uninstall_module(self,get):
        ...
        module = get['module'].strip()
        if module in ['npm']:
            return self.return_data(False,'',error_msg='禁止卸载{}模块!'.format(module))
        npm_bin = os.path.join(self._nodejs_path,version,'bin/npm')
        mod_path = os.path.join(self._nodejs_path,version,'lib/node_modules')
        if not os.path.exists(npm_bin): return self.return_data(False,'',error_msg='指定node版本未安装!')
        module = module.strip()
...
{} uninstall {} -g &> {}'''.format(self.get_shell_env(version),npm_bin,module,self._exec_log)
        ...
        if os.path.exists(mod_pack_file):
            return self.return_data(False,'',error_msg='卸载失败!')
    ...
    def get_default_env(self,get):
        ...
        if not os.path.islink(env_node_bin):
            return self.return_data(False,'',error_msg='当前未设置默认Node.js版本!')
    ...
    def set_default_env(self,get):
        ...
        if get:
            # 前置检查
            if not 'version' in get: return self.return_data(False,'',error_msg='缺少参数!')
            version = get['version'].strip()
        ...
        if get:
            ...
            if not os.path.exists(env_node_bin):
                return self.return_data(False,'',error_msg='设置失败!')
            self.clear_nvm_env()
