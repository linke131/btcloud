	...
	for k,v in pairs(requires_data) do
		if type(v)=='string' then
		    ...
		    if  config['xss_injection'] and config['xss_injection']['open'] and  site_config[server_name] and site_config[server_name]['xss_injection'] and site_config[server_name]['xss_injection']['open'] then 
    			local isxss = btengine.xss_detected(tostring(v))
    			if isxss==1 then 
    			    is_type='XSS防御'
...
local function nday_detected(request_args)
		local nday_request_args=request_args
        local url_info ={}
        url_info['url']=request_uri
        url_info['method']=method
		for key,valu in pairs(ngx.req.get_headers(20)) do 
            if type(valu)=='string' then 
        ...
        local is_nday,mess=btengine.nday_detected(json.encode(nday_request_args),json.encode(url_info))
        if is_nday==1 then
            is_type="通用漏洞拦截"
            write_log("通用漏洞拦截","拦截"..mess)
        	return_html(config['get']['status'],get_html)
        end
end 
