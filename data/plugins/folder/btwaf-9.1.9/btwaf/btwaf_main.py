...
class btwaf_main:
    ...
    def get_total_all_overview(self, get):
        ...
        if os.path.exists("/www/server/btwaf/totla_db/totla_db.db"):
            day_24_data = self.M2('blocking_ip').field(
                'time,time_localtime,server_name,ip,ip_country,blocking_time,is_status').where(
            ...
            if len(day_24_data) == 0:
                day_24_data = self.M2('blocking_ip').field(
                    'time,time_localtime,server_name,ip,ip_country,blocking_time,is_status').limit("30").order('id desc').select()
        ...
        result['map']['24_day_count'] = 0
        if os.path.exists("/www/server/btwaf/totla_db/totla_db.db"):
            map_24_data = self.M2('totla_log').field('time,ip,ip_country').where(
                "time>=? and time<=?", (start_timeStamp,end_timeStamp)).order(
                'id desc').limit("10000").select()
            result['map']['24_day_count'] = len(
                self.M2('totla_log').field('time').where("time>=?", int(time.time()) - 86400).order(
                    'id desc').limit("10000").select())
            is_time = time.time()
    ...
    def get_safe_logs_sql(self, get):
        ...
        if os.path.exists("/www/server/btwaf/totla_db/totla_db.db"):
            ...
            data222 = self.M3('blocking_ip').field(
                'time,time_localtime,server_name,ip,ip_city,ip_country,ip_subdivisions,ip_continent,ip_longitude,ip_latitude,type,uri,user_agent,filter_rule,incoming_value,value_risk,http_log,http_log_path,blockade,blocking_time,is_status').order(
    ...
    def get_all_tu(self, get):
        ...
        if os.path.exists("/www/server/btwaf/totla_db/totla_db.db"):
            for i in range(0, 8):
                day = time_xianzai - (i * 86400)
                day2 = time_xianzai - ((i - 1) * 86400)
            ...
            dongtai = self.M2('totla_log').field('id,time,time_localtime,server_name,ip,ip_country,type,filter_rule').where(
                "time>=?", int(time.time()) - 86400).order('id desc').limit("20").select()
            if len(dongtai) == 0:
                dongtai = self.M2('totla_log').field('id,time,time_localtime,server_name,ip,ip_country,type,filter_rule').order(
    ...
    def add_cnip(self, get):
        ...
        iplist.insert(0, ipn)
        self.__write_rule('cn', self.cn_to_ip(iplist))
...
'''
    ...
    '''
    time_localtime,server_name,ip,ip_city,ip_country,ip_subdivisions,ip_continent,ip_longitude,ip_latitude,type,uri,user_agent,filter_rule,incoming_value,value_risk,http_log,http_log_path
    '''
    ...
    def get_safe_logs_sql2(self, get):
        ...
        if os.path.exists("/www/server/btwaf/totla_db/totla_db.db"):
            ...
            data22 = self.M3('totla_log').field(
                'time,time_localtime,server_name,ip,ip_city,ip_country,ip_subdivisions,ip_continent,ip_longitude,ip_latitude,type,uri,user_agent,filter_rule,incoming_value,value_risk,http_log,http_log_path').order(
    ...
    def gongji_map(self, get):
        ...
        if os.path.exists("/www/server/btwaf/totla_db/totla_db.db"):
            map_24_data = self.M2('totla_log').field(
                'ip,ip_country,ip_subdivisions,ip_continent,ip_longitude,ip_latitude').where("time>=?",
    ...
    def get_id_log(self, get):
        ...
        data22 = self.M3('totla_log').field(
            'id,time,time_localtime,server_name,ip,ip_city,ip_country,ip_subdivisions,ip_continent,ip_longitude,ip_latitude,type,uri,user_agent,filter_rule,incoming_value,value_risk,http_log,http_log_path').order(
            ...
            if len(result['uri_list'][url]['ip_list']) < 100:
                result['uri_list'][url]['ip_list'].append(
                    {"id": i["id"], "uri": i['uri'], "ip": i["ip"], "filter_rule": i["filter_rule"],
                     "server_name": i['server_name'], "time_localtime": i["time_localtime"],"ip_country":i["ip_country"]})
    ...
    def get_report_info(self, get):
        ...
        if 'server_name' in get:
            if os.path.exists("/www/server/btwaf/totla_db/totla_db.db"):
                map_24_data = self.M3('totla_log').field('id,time_localtime,ip,uri,server_name,type,filter_rule,ip_country').where(
                    "time>? and time<? and server_name=?",
    ...
    def get_search_logs(self, get):
        ...
        try:
            ...
            self.__OPT_FIELD = 'time,time_localtime,server_name,ip,ip_city,ip_country,ip_subdivisions,ip_continent,ip_longitude,ip_latitude,type,uri,user_agent,filter_rule,incoming_value,value_risk,http_log,http_log_path,blockade,blocking_time,is_status'
            count = "count(*)"
    ...
    def get_site_safe_logs(self, get):
        ...
        if os.path.exists("/www/server/btwaf/totla_db/totla_db.db"):
            import page
            page = page.Page()
            self.__OPT_FIELD = 'time,time_localtime,server_name,ip,ip_city,ip_country,ip_subdivisions,ip_continent,ip_longitude,ip_latitude,type,uri,user_agent,filter_rule,incoming_value,value_risk,http_log,http_log_path'
            count = "count(*)"
            select_data_sql = select_sql.format(count)
