					...
					$.get('/btwaf/static/js/world_fix.js', function () {
						...
						$.post('/btwaf/get_total_all_overview.json', function (res) {
							...
							if (res.map['24_day_count'] >= 100) {
								layer.tips('<span>注意：当天拦截的攻击超过100次！</span>', '.day_intercept', { tips: [2, '#FCB958'], time: 0 });
				...
				if (res.msg == []) {
					...
					switch (index) {
						case 0:
							...
							if (res.msg.ip && res.msg.ip.length > 14) {
						...
						case 1:
							...
							if (res.msg.uri && res.msg.uri.length > 14) {
								that.create_table_thead({
									el: '.ng-fixed',
									list: [
										['URI', 'width="250px"'],
		...
		layer.open({
			...
			content:
				'<div class="uri_logs_table">\
						...
						<div id="report_logs_table" class="divtable" style="width:100%;height:621px;overflow:auto;padding:0px 15px 15px 15px;margin-top:15px">\
						    <table class="table table-hover">\
								<thead>\
									<tr>\
										<th width="150px">攻击时间</th>\
					...
					function (alarm_data) {
						...
						<div class="line-form">` +
								...
								<label class="btswitch-btn" for="` +
									  index +
									  `_switch" style="margin-bottom: 0"></label>
							</div>`) +
	...
	new_render_overall_data: function () {
		...
		this.ajaxTask('get_config', function (res) {
			...
			$.each(that.new_config_arr, function (index, item) {
				...
				_cont.append(
					`<div class="global-set-item ` +
						index +
						`">
												<div class="global-set-item-title">` +
			...
			column: [
				...
				},
				{
					...
					template: function (row, index) {
						...
						return '<span title="' + row.ip_continent + ' ' + row.ip_country + ' ' + (row.ip_city || '') + '">' + row.ip_continent + ' ' + row.ip_country + ' ' + (row.ip_city || '') + '</span>';
					},
				},
				{
					fid: 'filter_rule',
		...
		var _history = bt_waf_tools.table({
			...
			column: [
				...
				},
				{
					...
					template: function (row, index) {
						if (row.ip_country == '内网地址') return '<span title="' + row.ip_country + '">' + row.ip_country + '</span>';
						return '<span title="' + row.ip_country + '">' + row.ip_continent + ' ' + row.ip_country + ' ' + (row.ip_city || '') + '</span>';
					},
				},
				{
					title: '操作',
