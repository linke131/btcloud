...
class tomcat_manager(object):
    ...
    __tomcat10_server = '/www/server/tomcat10/conf/server.xml'
    ...
    def add_tomcat_project(self, get):
        ...
        if get.stype == 'built':
            ...
            if webserver == 'tomcat10' or webserver == '10':
                if not os.path.exists(self.__tomcat10_server): return public.returnMsg(False, "tomcat10的配置文件不存在，请重新安装tomcat10")
                public.ExecShell('cp -r /www/server/tomcat10/* %s && chown -R www:www %s' % (self.__site_path + domain, self.__site_path + domain))
            # server.xml
    ...
    def Initialization2(self, version, site):
        if version == '7' or version == 'tomcat7':
            ...
            if self.Xml_init(self.__site_path + site + '/conf/server.xml'):
                ...
                return False
        elif version == '10' or version == 'tomcat10':
            if self.Xml_init(self.__site_path + site + '/conf/server.xml'):
                return True
            else:
                return False
...
bash daemon.sh $1''' % path
    ...
    def RestarTomcat(self, version):
        ...
        if version == '10': version = 'tomcat10'
    ...
    def get_tomcat_services(self):
        ...
        if os.path.exists(self.__tomcat9_server):
            ...
            ret.append(tmp)
        if os.path.exists(self.__tomcat10_server):
            tmp = {'service_name': 'tomcat10', 'webserver': 'tomcat10'}
            if not self.Initialization('tomcat10'):
                tmp['port'] = False
            else:
                tmp['port'] = self.GetPort()
            tmp['status'] = self.GetServer('tomcat10')
            tmp['conf'] = public.readFile(self.__tomcat10_server)
            tmp['jdk'] = self.get_jdk('tomcat10')
            tmp['stype'] = 'built'
            tmp['log'] = public.GetNumLines('/www/server/tomcat10/logs/catalina-daemon.out', 3000)
            ret.append(tmp)
        # 获取独立项目列表
