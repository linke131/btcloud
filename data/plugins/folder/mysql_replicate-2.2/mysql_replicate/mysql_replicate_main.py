...
# 从库执行调用
execute_slave_func = os.path.join(plugin_dir, "execute_func.py")
logs_dir = "/tmp"
logs_file = "/tmp/mysql_replicate.log"

# CHANGE MASTER TO
# MASTER_HOST='192.168.69.148',
# MASTER_USER='Am345aOu',
# MASTER_PASSWORD='QtQ9HvWsY6M9',
...
class mysql_replicate_main():
    ...
    def uninstall_check(self, get=None):
        ...
        for slave_ip, slave_info in conf["slave"].items():
            ...
            slave_host = slave_info.get("slave_host",slave_ip)
            # 删除从库信息
            self.del_slave_info(slave_ip)
            # 删除防火墙放行端口
            mysql_master_obj = mysql_master()
            master_port = mysql_master_obj.get_master_port()
            mysql_master_obj.delete_port_release(slave_host, master_port)
    ...
    def update_versions_2_1(self):
        ...
        try:
            ...
            for slave_info in conf["slave"].values():
                db_dict = {}
                for db in slave_info.get("replicate_do_db"):
                    db_dict[db] = []

                for tb in slave_info["replicate_do_table"]:
                    db, tb = str(tb).split(".")
                ...
                for db, table_list in db_dict.items():
                    ...
                    db_list.append(data)
                slave_info["replicate_db"] = db_list
                slave_info["replicate_wild_table"] = []
                slave_info["replicate_db_type"] = 0
                slave_info["replicate_table_type"] = 0

                replicate_status = []
                for status in slave_info["replicate_status"]:
                    ...
                    if status["api"] == "check_master_conf":
                        ...
                        for t_status in slave_info["replicate_status"]:
                            if t_status["api"] == "create_replicate_user":
                                replicate_status.append(t_status)
                    replicate_status.append(status)
                slave_info["replicate_status"] = replicate_status
                if slave_info.get("replicate_do_db") is not None: del slave_info["replicate_do_db"]
                if slave_info.get("replicate_ignore_db") is not None: del slave_info["replicate_ignore_db"]
                if slave_info.get("replicate_do_table") is not None: del slave_info["replicate_do_table"]
                if slave_info.get("replicate_ignore_table") is not None: del slave_info["replicate_ignore_table"]
                if slave_info.get("replicate_wild_do_table") is not None: del slave_info["replicate_wild_do_table"]
                if slave_info.get("replicate_wild_ignore_table") is not None: del slave_info["replicate_wild_ignore_table"]
                if slave_info.get("export_path") is not None: del slave_info["export_path"]
                if slave_info.get("export_dump_database_task_id") is not None: del slave_info["export_dump_database_task_id"]
                if slave_info.get("upload_database_file_task_id") is not None: del slave_info["upload_database_file_task_id"]
                if slave_info.get("import_dump_database_task_id") is not None: del slave_info["import_dump_database_task_id"]
            public.writeFile(plugin_conf_file, json.dumps(conf))
        except Exception as err:
            public.print_log(f">>更新 2.1 错误:{err}")

    def update_versions_2_2(self):
        """
        更新到 2.2
        @return:
        """
        try:
            # 初始化配置文件
            conf = self.get_conf()
            if conf["master"].get("version") == "2.2":
                return

            public.print_log(f"2.2 更新配置文件！")
            execute_slave = os.path.join(plugin_dir, "execute_slave.py")
            if os.path.isfile(execute_slave):
                os.remove(execute_slave)
            conf["master"]["version"] = "2.2"
            for slave_ip, slave_info in conf["slave"].items():
                mysql_slave_obj = mysql_slave(slave_ip)
                result = mysql_slave_obj.bt_api_obj.get_config()
                slave_host = result["panel"].get("address")
                if slave_host is None:
                    continue
                slave_info["slave_host"] = slave_host
            public.writeFile(plugin_conf_file, json.dumps(conf))
        except Exception as err:
            public.print_log(f">>更新 2.2 错误:{err}")
    ...
    def get_slave_info(cls, slave_ip: str):
        """
        获取主从信息
        @param slave_ip: 从库 ip 唯一标识，可以是 ip/域名
        @return:
        """
        conf = cls.get_conf()
    ...
    # 修改主从信息
    @classmethod
    def set_slave_info(cls, slave_ip:str, slave_info: dict):
        """
        修改主从信息
        @param slave_ip: 从库 ip 唯一标识，可以是 ip/域名
        @param slave_info: 从库字典信息
        @return:
        """
        conf = cls.get_conf()
    ...
    # 删除主从信息
    @classmethod
    def del_slave_info(cls, slave_ip: str) -> bool:
        """
        删除主从信息
        @param slave_ip: 从库 ip 唯一标识，可以是 ip/域名
        @return:
    ...
    def get_slave_list(self, get):
        ...
        for slave_ip, slave_info in conf["slave"].items():
            ...
            if slave_status_info is False:  # 请求错误
                ...
                status = 1
                slave_status = 0
            slave_host = slave_info.get("slave_host")
            if slave_host is None:
                mysql_slave_obj = mysql_slave(slave_ip)
                result = mysql_slave_obj.bt_api_obj.get_config()
                slave_host = result["panel"].get("address")
                if slave_host is not None:
                    slave_info["slave_host"] = slave_host
    ...
    def get_replicate_conf(self, get):
        """
        :param get.slave_ip: 从库 ip 唯一标识，可以是 ip/域名
        :return:
        """
    ...
    def get_database(self, get):
        ...
        if not isinstance(db_list, list):
            return public.returnMsg(False, "MySQL数据库异常！请确保MySQL数据库正常并且是开启状态")
        database_list = []
        for db in db_list:
            db_name = db[0]
            ...
            for tb in tb_list:
                ...
                if len(data) == 0:
                    ...
                    size = data[0][0]
                if not isinstance(size, int): size = 0
                table = {
                    "name": tb[0],
                    "size": size,
                }
    ...
    def check_slave(self, get):
        ...
        addr_obj = re.search("https?://(.*):\d+/?", get.panel_addr, re.I)
        if not addr_obj:
            return public.returnMsg(False, "请输入正确的面板地址！ 如：http://1.1.1.1:8888/ http://www.bt.cn:8888/")
        panel_addr = addr_obj.group(0)
        slave_ip = addr_obj.group(1)
        ...
        slave_host = result["panel"].get("address")
        if slave_host is None:
            return public.returnMsg(False, "主库请求从库错误,获取从库 host 失败！")
        
        conf = self.get_conf()
        for s_ip, s_info in conf.get("slave", {}).items():
            if slave_ip == s_ip:
                return public.returnMsg(False, f"从库【{slave_ip}】已经存在！")
            if slave_host == s_info.get("slave_host"):
                return public.returnMsg(False, f"从库【{slave_host}】已经存在！")
    ...
    def add_replicate_info(self, get):
        ...
        :param get.panel_addr: 从库面板地址
        :param get.panel_key: 从库面板key
        :param get.replicate_db: 选择的数据库信息
        ...
        public.set_module_logs("mysql_replicate", "add_replicate_info", 1)
    ...
    def replicate_main(self, get):
        """
        主从复制配置流程请求接口
        @param get.action_name: 配置流程 API 方法名
        @return:
        """
    ...
    def check_slave_database(self, get):
        ...
        for db in replicate_db:
            ...
            if db.get("is_exist", False) is False and slave_db_dict.get(db_name) is not None:
                exist_dict[db_name] = []
    ...
    def sync_database(self, get):
        ...
        if self.get_slave_satuts(slave_ip, "check_slave_database") is False:
            return public.returnMsg(False, "请先检查主从库数据库一致性！")
        ...
        if task_id is not None:
            ...
            if not task:
                if slave_info.get("export_dir") is not None:
                    export_dir = slave_info["export_dir"]
                    mysql_slave_obj.bt_api_obj.delete_dir(export_dir)
                    public.ExecShell(f"rm -rf {export_dir}")
                    del slave_info["export_dir"]
        ...
        task_id = task_obj.create_task('同步主从数据库', 0, f"btpython {execute_slave_func} --slave_ip={slave_ip} --api=sync_slave_database")
    ...
    def check_master_conf(self, get):
        """
        检查并设置主库配置信息
        @param get: 从库 ip 唯一标识，可以是 ip/域名
        @return:
        """
        if not hasattr(get, "slave_ip"):
            return public.returnMsg(False, f"缺少参数! slave_ip")
        ...
        if server_id is None:
            is_update = True
            server_id = self._new_generate_number(9)
    ...
    # 生成 server-id
    @classmethod
    def _new_generate_number(cls, num_len):
        """
        帮助方法 生成 server-id
        @param num_len: 生成 id 的长度
        @return:
        """
        import random
    ...
    def create_replicate_user(self, get):
        ...
        # 检查同步用户
        is_user = mysql_obj.query(f"select count(User) from mysql.user where User='{user}' and Host='{slave_host}';")
        if is_user[0][0] == 0:
            mysql_obj.execute(f"flush privileges;")
            mysql_obj.execute(f"create user '{user}'@'{slave_host}' identified by '{password}'")
            if mysql_master.version_compare(master_version, "8.0.0") != -1:
                mysql_obj.execute(f"grant replication slave on *.* to '{user}'@'{slave_host}'")
            else:
                mysql_obj.execute(f"grant replication slave on *.* to '{user}'@'{slave_host}' identified by '{password}'")
            mysql_obj.execute(f"flush privileges;")
        ...
        for info in slave_info["replicate_status"]:
            if info["api"] == "create_replicate_user":
                info["status"] = True
                info["msg_list"] = [
                    {"msg": f"创建用户：{user}，权限：主从复制（replication），允许访问ip：{slave_host}"},
                ]
        self.set_slave_info(slave_ip, slave_info)
...
binlog_format=mixed"""
        ...
        if slave_info.get("is_sysc_database", False) is True:
            ...
            del slave_info["is_sysc_database"]
    ...
    def check_firewall_set(self, get):
        ...
        if mysql_master_obj.check_port(slave_host, master_port) is False:
            resp = mysql_master_obj.add_port_release(slave_host, master_port)
            if resp is False:
                return public.returnMsg(True, f"防火墙设置失败！请手动放行 {slave_host}:{master_port}")

        for info in slave_info["replicate_status"]:
            if info["api"] == "check_firewall_set":
                info["status"] = True
                info["msg_list"] = [
                    {"msg": f"防火墙已放行：{slave_host} 端口：{master_port}"},
                ]
        self.set_slave_info(slave_ip, slave_info)
        return public.returnMsg(True, f"防火墙已放行 {slave_host} 访问 {master_port} 端口！")
    ...
    def remove_replicate_info(self,get):
        ...
        if hasattr(get, "type"):
            if str(get.type) == "1":
                ...
                mysql_master_obj = mysql_master()
                master_port = mysql_master_obj.get_master_port()
                resp = mysql_master_obj.delete_port_release(slave_host, master_port)
                if resp is False:
                    return public.returnMsg(False, f"删除防火墙规则失败！请手动删除防火墙规则 {slave_host}:{master_port}")
        ...
        conf = self.get_conf()
        slave_user = conf["master"].get("slave_user")
        panelMysql.panelMysql().query(f"drop user `{slave_user}`@`{slave_host}`")
        # 删除从库信息
        self.del_slave_info(slave_ip)
        # 删除防火墙放行端口
        mysql_master_obj = mysql_master()
    ...
    def get_set_database(self, get):
        ...
        for db in db_list:
            ...
            for tb in tb_list:
                ...
                # 后续新增表
                if addtime is not None and db_is_exist is True and replicate_table_type == 1 and tb_is_exist is False:
    ...
    def set_replicate_info(self, get):
        ...
        修改主从同步数据库
        @param slave_ip: 从库 ip 唯一标识，可以是 ip/域名
        @param get.replicate_db: 选择同步的数据库
        @param get.replicate_wild_table: 使用通配符来控制需要复制的表
        @param get.replicate_db_type: # 数据库是否选择 0 选择 1 排除
        @param get.replicate_table_type: # 表是否选择 0 选择 1 排除
        @return:
    ...
    def repair_replicate(self, get):
        ...
        if task_id is not None:
            ...
            if task.get("status", 0) != 1:
                ...
                if not os.path.exists(repair_replicate_log):
                    return {"status": True, "msg": "正在修复中请稍后...", "type": 1}
                export_log = public.readFile(repair_replicate_log)
                export_log = str(export_log).split("\n")
                export_log = "\n".join(export_log)
                return {"status": True, "msg": export_log, "type": 1}
        ...
        Last_IO_Error = slave_status_info["Last_IO_Error"]
        ...
        # 数据库同步配置丢失
        if Last_SQL_Errno == "1872" and str(Last_SQL_Error).lower().find("Slave failed to initialize relay log info structure from the repository".lower()) != -1:
            api = "reset_master_info"
        if Last_IO_Errno == "2003" or Last_IO_Errno == "1045": # 修复连接超时
            # 2003 防火墙未放行 1045 主从用户不存在
            api = "reset_connecting"
        # server-uuid 冲突
        if Last_IO_Errno == "13114" and str(Last_IO_Error).lower().find("A slave with the same server_uuid/server_id as this slave has connected to the master;".lower()) != -1:
            api = "repair_server_uuid"
        # server-uuid 冲突
        if Last_IO_Errno == "13117" and str(Last_IO_Error).lower().find("because master and slave have equal MySQL server UUIDs;".lower()) != -1:
            api = "repair_server_uuid"
        # server-id 重复
        if Last_IO_Errno == "13117" and str(Last_IO_Error).lower().find("master and slave have equal MySQL server ids".lower()) != -1:
            api = "repair_server_id"
        if api is not None:
            task_id = task_obj.create_task("修复主从", 0, f"btpython {execute_slave_func} --slave_ip={slave_ip} --api={api}")
            slave_info["repair_replicate_task_id"] = task_id  # 保存任务id
            self.set_slave_info(slave_ip, slave_info)
            return {"status": True, "msg": "正在修复中请稍后...", "type": 1}
        return public.returnMsg(False, "目前无法修复，请将同步详情提交给专业的技术人员！")
    ...
    def set_slave_addr_key(self, get):
        ...
        修改主从复制配置-从库密钥
        @param slave_ip: 从库 ip 唯一标识，可以是 ip/域名
        @param panel_addr: 从库面板地址
        @param panel_key: 从库面板key
        @return:
        """
        # 校验参数
        if not hasattr(get, "slave_ip"):
...
class mysql_master():
    ...
    def version_compare(cls, version_1: str, version_2: str) -> int:
        """
        版本号比较
        @param version_1: 版本号 1
        @param version_2: 版本号 2
    ...
    # 检查防火墙
    def check_port(self, slave_host: str, port: int):
        """
        检查防火墙是否放行
        @param slave_host: 从库主机 ip 地址
        @param port: 端口号
        @return:
        """
        ...
        if self.__is_firewalld:
            ...
            # if a:
            #     return True
            a, e = public.ExecShell(f"firewall-cmd --list-all|grep -E 'rule\s*family.*address=.?{slave_host}.?.*{port}.*accept'")
            if a:
                return True
            return False
        else:
            ...
            # a, e = public.ExecShell(f"ufw status|grep -E '{port}/tcp\s*ALLOW\s*Anywhere'")
            # if a:
            #     return True
            a, e = public.ExecShell(f"ufw status|grep -E '{port}/tcp\s*ALLOW\s*{slave_host}'")
            if a:
                return True
            return False
    ...
    # 添加防火墙放行端口
    def add_port_release(self, slave_host: str, port: int):
        """
        添加防火墙放行端口
        @param slave_host: 从库主机 ip 地址
        @param port: 端口号
        ...
        """
        if self.__is_ufw:
            public.ExecShell(f"ufw allow proto tcp from {slave_host} to any port {port}")
        else:
            public.ExecShell(f"firewall-cmd --permanent --add-rich-rule='rule family=ipv4 source address='{slave_host}' port protocol='tcp' port='{port}' accept'")
        self.firewall_reload()
        if self.check_port(slave_host, port) is True:
            public.M('firewall_new').add('ports,brief,protocol,address,types,addtime,domain,sid', (port, public.xsssec("MySQL 主从复制"), "tcp", slave_host, "accept", datetime.datetime.now().strftime('%Y-%m-%d %X'), "", 0))
            return True
        return False

    # 删除防火墙放行端口
    def delete_port_release(self, slave_host: str, port: int) -> bool:
        """
        删除防火墙放行端口
        @param slave_host: 从库主机 ip 地址
        @param port: 端口号
        ...
        """
        if self.__is_ufw:
            public.ExecShell(f"ufw delete allow proto tcp from {slave_host} to any port {port}")
        else:
            public.ExecShell(f"firewall-cmd --permanent --remove-rich-rule='rule family=ipv4 source address=\'{slave_host}\' port protocol=\'tcp\' port=\'{port}\' accept'")
        self.firewall_reload()
        if self.check_port(slave_host, port) is False:
            public.M('firewall_new').where("address=? and ports=? ", (slave_host, port,)).delete()
            return True
        return False
        
    # 获取主库读写状态
    def get_master_readonly(self) -> bool:
        """
        获取主库读写状态
        @return: 是否只读
        """
    ...
    def set_master_readonly_temp(self, status: int):
        """
        设置主库只读状态
        @param status: 0 读写 1 只读
        @return:
        """
        try:
            if self.get_master_readonly() is True and status == 0:  # 读写
    ...
    # 获取主库 gtid
    def get_master_gtid(self) -> Union[str, None]:
        """
        获取主库 gtid
        @return: 主库 gtid
        """
    ...
    # 获取主库 server-id
    def get_master_server_id(self) -> Union[str, None]:
        """
        获取主库 server-id
        @return: server-id
        """
        ...
        server_obj = re.search(r"server-id\s*=\s*(\d+)\n", conf)
        if server_obj:
            server_id = server_obj.group(1)
            return server_id
        else:
            return None

    # 设置主库 server-id
    def set_master_server_id(self, server_id: str) -> bool:
        """
        设置主库 server-id
    ...
    # 获取主库 gtid 开启状态
    def get_master_gtid_status(self) -> bool:
        """
        获取主库 gtid 开启状态
        @return: status
        """
...
gtid-mode=on"""
    ...
    # 获取数据库二进制日志是否已经开启
    def get_master_binlog(self) -> bool:
        """
        获取数据库二进制日志是否已经开启
        @return: 
        """
...
class mysql_slave:
    ...
    # 从库执行sql语句:
    def exec_shell_sql(self, sql: str, is_resp: bool = False) -> Union[dict, str]:
        """
        从库执行sql语句
        @param sql: sql 语句
        @param is_resp: 是否返回 resp 响应，默认返回 resp["msg"]
        @return: 
        """
        get_time = 2  # 超时间
    ...
    # 获取从库版本号
    def get_slave_version(self) -> str:
        """
        获取从库版本号
        @return: 
        """
    ...
    def sync_slave_database(self):
        ...
        if sync_database_log is None:
            sync_database_log = f"{logs_dir}/mysql_replicate_export_{int(time.time() * 1000_000)}.log"
        ...
        # 创建临时同步目录
        time_num = int(time.time() * 1000_000)
        export_name = f"mysql_replicate_export_tmp_{time_num}"
        export_dir = os.path.join(backup_path, export_name)
        if os.path.isdir(export_dir):
            public.ExecShell(f"rm -rf {export_dir}")
        os.makedirs(export_dir)
        try:
            slave_info["export_dir"] = export_dir
            mysql_replicate_main.set_slave_info(self.slave_ip, slave_info)

            password = public.M("config").where("id=?", (1,)).getField("mysql_root")
            mysqldump_bin = public.get_mysqldump_bin()
            # public.print_log(f">>开始同步")
            for db in replicate_db:
                db_name = db["name"]

                if db.get("is_exist", False) is False:  # 创建未存在的数据库
                    # public.print_log(f">>创建数据库：{db_name}")
                    # 创建数据库
                    public.writeFile(sync_database_log, f"\n1|正在创建数据库：{db_name}", "a")
                ...
                for tb in db["table_list"]:
                    ...
                    tb_name = tb["name"]
                    #
                    # if tb_name == "test_data_3":
                    #     return public.writeFile(sync_database_log, f"\n0|test_data_3！错误", "a")

                    # 导出数据表文件
                    table_file_name = f"{db_name}_{tb_name}.sql"
                    export_table_file_path = os.path.join(export_dir, table_file_name)
                    # public.print_log(f">>正在同步表：{db_name}:{tb_name}：{export_table_file_path}")

                    shell = f"{mysqldump_bin} -R -E --skip-lock-tables --set-gtid-purged=off --routines --events --triggers=false --single-transaction " \
                            f"-uroot -p{password} --force --tables {db_name} {tb_name} > {export_table_file_path}"

                    public.writeFile(sync_database_log, f"\n1|正在导出表：{db_name}.{tb_name}", "a")

                    public.ExecShell(shell)

                    # 上传数据库表文件
                    public.writeFile(sync_database_log, f"\n1|正在上传表：{db_name}.{tb_name}", "a")

                    upload_resp = self.bt_api_obj.upload_file(export_table_file_path)
                    # public.print_log(f">>upload_resp：{upload_resp}")
                    if upload_resp is not True:
                        self.bt_api_obj.delete_dir(export_dir)
                        public.ExecShell(f"rm -rf {export_dir}")
                        del slave_info["export_dir"]
                        mysql_replicate_main.set_slave_info(self.slave_ip, slave_info)
                        return public.writeFile(sync_database_log, f"\n0|上传表：{db_name}.{tb_name} 失败！错误：{upload_resp}", "a")

                    # 导入数据库表
                    upload_file_resp = self.bt_api_obj.upload_file_exists(export_table_file_path)
                    if upload_file_resp["status"] is False:
                        self.bt_api_obj.delete_dir(export_dir)
                        public.ExecShell(f"rm -rf {export_dir}")
                        del slave_info["export_dir"]
                        mysql_replicate_main.set_slave_info(self.slave_ip, slave_info)
                        return public.writeFile(sync_database_log, f"\n0|从库服务器导入数据库文件不存在！请重新同步！", "a")

                    public.writeFile(sync_database_log, f"\n1|正在导入表：{db_name}.{tb_name}", "a")

                    import_resp = self.bt_api_obj.import_sql(export_table_file_path, db_name)
                    # public.print_log(f">>import_resp：{import_resp}")
                    if import_resp["status"] is False:
                        self.bt_api_obj.delete_dir(export_dir)
                        public.ExecShell(f"rm -rf {export_dir}")
                        del slave_info["export_dir"]
                        mysql_replicate_main.set_slave_info(self.slave_ip, slave_info)
                        return public.writeFile(sync_database_log, f"\n0|导入：{db_name}.{tb_name} 表失败！错误:{import_resp['msg']}", "a")

                    os.remove(export_table_file_path)
                    self.bt_api_obj.delete_file(export_table_file_path)
                    ...
                    public.writeFile(sync_database_log, f"\n2|已同步：{db_name}.{tb_name}，耗时：{time_day}{time_hour}{time_minute}{time_second}秒", "a")

                    tb["is_exist"] = True  # 已导入表
                    mysql_replicate_main.set_slave_info(self.slave_ip, slave_info)
            ...
            public.writeFile(sync_database_log, f"\n2|同步完成！总耗时：{time_day}{time_hour}{time_minute}{time_second}秒", "a")
            slave_info["is_sysc_database"] = True
            del slave_info["export_dir"]
            mysql_replicate_main.set_slave_info(self.slave_ip, slave_info)
        except Exception as err:
            public.ExecShell(f"rm -rf {export_dir}")
    ...
    # 获取从库 server-id
    def get_slave_server_id(self) -> Union[str, None]:
        """
        获取从库 server-id
        @return:
        """
    ...
    # 获取远程数据库读写状态
    def get_mysql_readonly(self) -> bool:
        """
        获取远程数据库读写状态
        @return:
        """
    ...
    def repair_primary_key(self):
        ...
        if repair_replicate_log is None:
            repair_replicate_log = f"{logs_dir}/repair_replicate_{int(time.time() * 1000_000)}.log"
            slave_info["repair_replicate_log"] = repair_replicate_log
            mysql_replicate_main.set_slave_info(self.slave_ip, slave_info)

        max_repair_num = 1000
        public.writeFile(repair_replicate_log, f">> 发现从库【主键冲突】,开始删除从库冲突的主键（每次最多修复1000条）", "w")
        ...
        repair_num = max_repair_num
        while repair_num:
            applier_obj = re.search("executing transaction '(.*)'.*performance_schema.replication_applier_status_by_worker", last_sql_error)
            if applier_obj:
                err_gtid = applier_obj.group(1)
                err_msg = self.exec_shell_sql(f"select * from performance_schema.replication_applier_status_by_worker where LAST_ERROR_NUMBER={Last_SQL_Errno} and APPLYING_TRANSACTION='{err_gtid}' limit 1;")

                field_list, value_list = err_msg.split("\n")
                err_dict = {}
                for fileld, value in zip(field_list.split("\t"), value_list.split("\t")):
                    fileld = str(fileld).strip()
                    value = str(value).strip()
                    err_dict[fileld] = value

                last_sql_error = err_dict.get("LAST_ERROR_MESSAGE", None)
                if last_sql_error is None:
                    return public.writeFile(repair_replicate_log, f"\n暂未解析到可修复操作！请将同步详情提交给专业的技术人员!", "a")

            info_err_obj = re.search(r"entry\s'(\d+)'.*database:\s'([\d\w]+)'.*insert\sinto\s(`[\d\w]+`\.)?`([\d\w]+)`", last_sql_error, re.I)
            if not info_err_obj:
                return public.writeFile(repair_replicate_log, f"\n暂未解析到可修复操作！请将同步详情提交给专业的技术人员!", "a")
            primary = info_err_obj.group(1)
            db_name = info_err_obj.group(2)
            tb_name = info_err_obj.group(4)

            # 查询主键字段
            data_list = panelMysql.panelMysql().query(f"show index FROM `{db_name}`.`{tb_name}` WHERE Key_name = 'PRIMARY';")
            if len(data_list) == 0:
                public.writeFile(repair_replicate_log, f"\n未查询到：{db_name}.{tb_name} 表的主键字段，主键冲突冲突值：{primary}，已跳过!", "a")
                continue
            prikey = data_list[0][4]
            ...
            repair_num -= 1
            if repair_num == 0:
                public.writeFile(repair_replicate_log, f"\n本次修复已达上限，每次最多修复{max_repair_num}条！", "a")
            else:
                public.writeFile(repair_replicate_log, f"\n>> 发现新的【表丢失】，正在修复.", "a")
    ...
    def fix_table_not_exist(self):
        ...
        max_repair_num = 3
        public.writeFile(repair_replicate_log, f">> 发现从库【表丢失】,开始同步丢失表到从库（每次最多修复 {max_repair_num} 张表）", "w")
        ...
        while repair_num:
            ...
            try:
                ...
                for table_name in err_list:
                    ...
                    # public.print_log(f">>开始修复表丢失：{db_name}.{tb_name}")
                    ...
                    public.writeFile(repair_replicate_log, f"\n正在导出表：{db_name}.{tb_name}", "a")
                    public.ExecShell(shell)

                    # 上传数据库表文件
                    public.writeFile(repair_replicate_log, f"\n正在上传表：{db_name}.{tb_name}", "a")

                    resp = self.bt_api_obj.upload_file(export_table_file_path)
                    if resp is not True:
                        self.bt_api_obj.delete_dir(export_dir)
                        public.ExecShell(f"rm -rf {export_dir}")
                        del slave_info["export_dir"]
                        mysql_replicate_main.set_slave_info(self.slave_ip, slave_info)
                        return public.writeFile(repair_replicate_log, f"\n上传表：{db_name}.{tb_name} 失败！错误：{resp}", "a")

                    # 导入数据库表
                    upload_resp = self.bt_api_obj.upload_file_exists(export_table_file_path)
                    if upload_resp["status"] is False:
                        self.bt_api_obj.delete_dir(export_dir)
                        public.ExecShell(f"rm -rf {export_dir}")
                        del slave_info["export_dir"]
                        mysql_replicate_main.set_slave_info(self.slave_ip, slave_info)
                        return public.writeFile(repair_replicate_log, f"\n从库服务器导入数据库文件不存在！请重新同步！", "a")

                    public.writeFile(repair_replicate_log, f"\n正在导入表：{db_name}.{tb_name}", "a")
                    ...
                    if import_resp["status"] is False:
                        ...
                        del slave_info["export_dir"]
                        mysql_replicate_main.set_slave_info(self.slave_ip, slave_info)
                        return public.writeFile(repair_replicate_log, f"\n导入：{db_name}.{tb_name} 表失败！错误:{import_resp['msg']}", "a")

                    os.remove(export_table_file_path)
                    self.bt_api_obj.delete_file(export_table_file_path)

                    end_sync_time = time.time() - start_sync_time
                    time_second = round(end_sync_time % 60, 2)
                    time_minute = f"{int(end_sync_time // 60)}分" if int(end_sync_time // 60) != 0 else ""
                    time_hour = f"{int(end_sync_time // 60 // 60)}时" if int(end_sync_time // 60 // 60) != 0 else ""
                    time_day = f"{int(end_sync_time // 60 // 60 // 60 % 24)}天" if int(end_sync_time // 60 // 60 // 60 % 24) != 0 else ""
                    public.writeFile(repair_replicate_log, f"\n已同步：{db_name}.{tb_name}，耗时：{time_day}{time_hour}{time_minute}{time_second}秒", "a")
                ...
                time_day = f"{int(end_time // 60 // 60 // 60 % 24)}天" if int(end_time // 60 // 60 // 60 % 24) != 0 else ""
                public.writeFile(repair_replicate_log, f"\n同步完成！总耗时：{time_day}{time_hour}{time_minute}{time_second}秒", "a")
                ...
                repair_num -= 1
                if repair_num == 0:
                    public.writeFile(repair_replicate_log, f"\n本次修复已达上限，每次最多修复{max_repair_num}张表！", "a")
                else:
                    public.writeFile(repair_replicate_log, f"\n>> 发现新的【表丢失】，正在修复.", "a")
            except Exception as err:
                public.ExecShell(f"rm -rf {export_dir}")
    ...
    def reset_master_info(self):
        ...
        try:
            public.writeFile(repair_replicate_log, f">> 发现主从【数据库同步配置丢失】正在重新配置启动主从复制", "w")
            
            public.writeFile(repair_replicate_log, f"\n正在停止主从复制", "a")
            self.exec_shell_sql("stop slave;reset slave all;")
            
            data_list = panelMysql.panelMysql().query("show master status;")
            master_log_file = data_list[0][0]
            master_log_pos = data_list[0][1]
            
            public.writeFile(repair_replicate_log, f"\n正在重新初始化主从复制，设置同步位置 binlog_file={master_log_file} pos={master_log_pos}", "a")
            ...
            slave_status_info = self.get_slave_status()
            Last_SQL_Errno = slave_status_info["Last_SQL_Errno"]
            Last_SQL_Error = slave_status_info["Last_IO_Error"]
            if Last_SQL_Errno != "1872" and str(Last_SQL_Error).lower().find("Slave failed to initialize relay log info structure from the repository".lower()) == -1:  # 修复失败
                return public.writeFile(repair_replicate_log, f"\n修复成功!", "a")
            return public.writeFile(repair_replicate_log, f"\n修复失败!", "a")
        except Exception as err:
            return public.writeFile(repair_replicate_log, f"\n修复失败!错误：{err}", "a")
    ...
    def reset_connecting(self):
        ...
        try:
            public.writeFile(repair_replicate_log, f">> 发现主从【连接超时】正在检查连接配置并修复", "w")
            
            conf = mysql_replicate_main.get_conf()
            master = conf.get("master", {})
            if not master.get("slave_user"):
                master["slave_user"] = public.GetRandomString(8)
            if not master.get("password"):
                master["password"] = public.gen_password(12)
            slave_user = master["slave_user"]
            password = master["password"]
            
            slave_host = slave_info.get("slave_host", self.slave_ip)
            public.writeFile(repair_replicate_log, f"\n正在检查主从同步用户：{slave_user}@{slave_host}", "a")
            ...
            # 检查同步用户
            is_user = mysql_obj.query(f"select count(User) from mysql.user where User='{slave_user}' and Host='{slave_host}';")
            if is_user[0][0] == 0:
                public.writeFile(repair_replicate_log, f"\n主从用户不存在！正在创建主从同步用户：{slave_user}@{slave_host}", "a")
                mysql_obj.execute(f"flush privileges;")
                mysql_obj.execute(f"create user '{slave_user}'@'{slave_host}' identified by '{password}'")
                if mysql_master.version_compare(master_version, "8.0.0") != -1:
                    mysql_obj.execute(f"grant replication slave on *.* to '{slave_user}'@'{slave_host}'")
                else:
                    mysql_obj.execute(f"grant replication slave on *.* to '{slave_user}'@'{slave_host}' identified by '{password}'")
                mysql_obj.execute(f"flush privileges;")
            
            # 检查防火墙放行端口
            mysql_master_obj = mysql_master()
            master_port = mysql_master_obj.get_master_port()
            public.writeFile(repair_replicate_log, f"\n正在检查防火墙放行：{slave_host} 端口:{master_port}", "a")
            
            if mysql_master_obj.check_port(slave_host, master_port) is False:
                public.writeFile(repair_replicate_log, f"\n防火墙未放行！正在放行：{slave_host} 端口:{master_port}", "a")
                resp = mysql_master_obj.add_port_release(slave_host, master_port)
                if resp is False:
                    return public.writeFile(repair_replicate_log, f"\n防火墙设置失败！请手动放行 {slave_host}:{master_port}", "a")
            ...
            slave_status_info = self.get_slave_status()
            Last_IO_Errno = slave_status_info["Last_IO_Errno"]
            if Last_IO_Errno != "2003" and Last_IO_Errno != "1045":  # 修复失败
                return public.writeFile(repair_replicate_log, f"\n修复成功!", "a")
            return public.writeFile(repair_replicate_log, f"\n修复失败!", "a")
        except Exception as err:
            return public.writeFile(repair_replicate_log, f"\n修复失败!错误：{err}", "a")
    
    # 修复主从复制 server-uuid 冲突
    def repair_server_uuid(self):
        """
        修复主从复制 server-uuid 冲突
        @return:
        """
        ...
        try:
            public.writeFile(repair_replicate_log, f">> 发现主从【server-uuid 冲突】重新设置从库 server_uuid", "w")
            ...
            conf = resp["data"]
            # 获取数据目录
            server_obj = re.search(r"datadir\s*=\s*(.+)\n", conf)
            datadir = "/www/server/data"
            if server_obj:
                datadir = server_obj.group(1)
            auto_cnf = os.path.join(datadir, "auto.cnf")
            resp = self.bt_api_obj.get_slave_conf(path=auto_cnf)
            if resp["status"] is False:
                return public.writeFile(repair_replicate_log, f"\n请求从库错误！", "a")
            conf = resp["data"]
            # 获取数据目录
            server_obj = re.search(r"server-uuid\s*=\s*(.+)\n", conf)
            if server_obj:
                server_uuid = server_obj.group(1)
                public.writeFile(repair_replicate_log, f"\n当前从库 server-uuid={server_uuid}", "a")
            import uuid
            server_uuid = uuid.uuid4()
            public.writeFile(repair_replicate_log, f"\n生成新的 server-uuid={server_uuid}", "a")
            public.writeFile(repair_replicate_log, f"\n设置从库新的 server-uuid={server_uuid}", "a")
            
            conf = re.sub("server-uuid\s*=\s*(.*)\n", f"server-uuid={server_uuid}\n", conf)
            resp = self.bt_api_obj.save_slave_conf(path=auto_cnf, content=conf)
            if resp["status"] is False:
                return public.writeFile(repair_replicate_log, f"\n设置从库 server-uuid 失败! \n请手动设置:{self.slave_ip}\n路径：{auto_cnf}\nserver-uuid={server_uuid}", "a")
    ...
    def repair_server_id(self):
        ...
        try:
            public.writeFile(repair_replicate_log, f">> 发现主从【server-id 冲突】重新设置从库 server-id", "w")

            # 设置主库 server_id
            public.writeFile(repair_replicate_log, f"\n正在检查主库 server-id", "a")
            conf = mysql_replicate_main.get_conf()
            master_server_id = conf["master"].get("server-id", 1)

            mysql_master_obj = mysql_master()
            server_id = mysql_master_obj.get_master_server_id()
            if server_id is None:
                server_id = mysql_replicate_main._new_generate_number(9)
                public.writeFile(repair_replicate_log, f"\n正在设置主库 server-id:{server_id}", "a")
                mysql_master_obj.set_master_server_id(server_id)
                conf["master"]["server-id"] = server_id
                public.writeFile(plugin_conf_file, json.dumps(conf))
            elif server_id != master_server_id:
                conf["master"]["server-id"] = server_id
                public.writeFile(plugin_conf_file, json.dumps(conf))
            
            public.writeFile(repair_replicate_log, f"\n正在检查从库 server-id", "a")
            server_id_list = [server_id]  # 获取所有 server-id
            for ip, slave in conf["slave"].items():
                if ip == self.slave_ip: continue
                server_id_list.append(slave.get("server-id", 0))

            server_id = self.get_slave_server_id()
            if server_id is None or server_id in server_id_list:  # 生成新的 server-id
                while server_id in server_id_list:
                    server_id = mysql_replicate_main._new_generate_number(9)
                public.writeFile(repair_replicate_log, f"\n设置从库 server-id={server_id}", "a")
                self.set_slave_server_id(server_id)
                slave_info["server-id"] = server_id
                self.bt_api_obj.control_mysqld_service()
                mysql_replicate_main.set_slave_info(self.slave_ip, slave_info)
            ...
            Last_IO_Errno = slave_status_info["Last_IO_Errno"]
            Last_IO_Error = slave_status_info["Last_IO_Error"]
            if Last_IO_Errno != "13117" and str(Last_IO_Error).lower().find("master and slave have equal MySQL server ids".lower()) == -1:  # 修复失败
                return public.writeFile(repair_replicate_log, f"\n修复成功!", "a")
            return public.writeFile(repair_replicate_log, f"\n修复失败!", "a")
        except Exception as err:
            return public.writeFile(repair_replicate_log, f"\n修复失败!错误：{err}", "a")
...
class bt_api():
    ...
    def http_post_cookie(self, url:str, pdata: dict, timeout: int=1800):
        ...
        @param pdata: POST参数，可以是字符串或字典(必需)
    ...
    def get_slave_conf(self, path: str = None) -> dict:
        """
        获取从库文件内容
        @param path: path 获取文件路径， 默认获取 MySQL my.cnf 配置文件
        @return:
        """
        try:
    ...
    def save_slave_conf(self, content: str, path: str = None) -> dict:
        ...
        保存从库文件
        @param content: 保存内容
        @param path: 保存文件路径，默认 MySQL my.cnf 配置文件
        @return:
        """
        try:
