...
class supervisor_main:
    ...
    def __init__(self):
        ...
        if not self.proxy_client:
            self.proxy_client = client.ServerProxy('http://localhost',
                                                   transport=UnixStreamTransport(self.supervisord_sock))
    ...
    def GetServerStatus(self, get):
        ...
        if os.path.exists(self.supervisord_pid) and os.path.exists(self.supervisord_sock)\
                and ps_process[0]: return public.ReturnMsg(True, "服务运行正常")
    ...
    def __ServerOption(self, get):
        ...
        if get["cmd"] == "stop":
            os.remove(self.supervisord_pid)
            os.remove(self.supervisord_sock)
    ...
    def __KillPid(self):
        ...
        os.system("pid=`ps -ef|grep supervisord|grep -v grep|awk '{print $2}'` && [[ $pid != \"\" ]] && for i in $pid;do kill -9 $i;done")
    ...
    def GetProcessList(self, get):
        ...
        # print("="*50)
        # print("="*50)
        # for obj in config_infos:
        #     for key, val in obj.items():
        #         print(key, "=", val)
        #     print("*"*20)

        # for obj in process_infos:
        #     for key, val in obj.items():
        #         print(key, "=", val)
        #     print("*"*20)
        new_info = []
        ...
        for i in range(0, len(process_infos)):
            ...
            if pinfo["name"][under_inx+1:] != "00":
                ...
                #     if proc_id_desc:
                #         proc_id_desc += "</br>"
                #     proc_id_desc += pinfo["name"] + ":" + str(pinfo["pid"])
                continue
