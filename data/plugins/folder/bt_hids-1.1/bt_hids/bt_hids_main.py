...
class bt_hids_main:
        ...
        with open(self.__PIPE_PATH, "rb", buffering=0) as f:       # 不对文件进行缓冲，直接从文件中读取数据而不缓存到内存中
            ...
            while True:
                ...
                for i in yaml_list:
                    ...
                    #     print(result)
                    # if data_type=="42" and 'dip' in result and result["dip"]=="192.168.221.132":
                    #     print(result)
                    if data_type == i["detect_id"]:
                        ...
                        if count == len(i["rule"]):
                            ...
                            risk_dict["other"] = []
                            if data_type == '59':
                                risk_dict["other"].append({"key": "socket_argv", "value": result["socket_argv"]})  # 外联进程命令行
                                ...
                                if result["argv"].startswith("/usr/libexec/pk-command-not-found "):
                                    risk_dict["msg"] = i["msg"].replace("bt_ip", result["dip"]).replace("bt_cmd", result["argv"].replace(
                                                                                "/usr/libexec/pk-command-not-found ",""))  # 告警描述
                                else:
                                    risk_dict["msg"] = i["msg"].replace("bt_ip", result["dip"]).replace("bt_cmd", result["argv"])  # 告警描述
                            elif data_type == '611':
                            ...
                            self.send_news(risk_dict["vulnname"], risk_dict["level"], risk_dict["type"], risk_dict["time"])
                            break
    ...
    def set_process(self, get):
        ...
        if not new_pid:
            ...
            hids_p = subprocess.Popen(['btpython', '{}/load_hids.py'.format(self.__plugin_path)], stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=False)
    ...
    def get_result(self, get):
        ...
        data['count'] = {'serious':overview['serious'],'high_risk':overview['high_risk'],'medium_risk':overview['medium_risk'],'low_risk':overview['low_risk']}
    ...
    def set_white(self, args):
        ...
        if white_dict in white_list:
            ...
            try:
                ...
                public.M('white').dbfile("bt_hids").where("vulnname=? and ps=? and rules=? and time=?", (white_dict["vulnname"], white_dict["ps"], white_dict["rules"], white_dict["time"])).delete()
        ...
        try:
            ...
            public.M('risk').dbfile('bt_hids').where('vulnname=? and other=?',(white_dict['vulnname'], white_dict['rules'],)).select()
            # public.WriteFile(self.__white_file, json.dumps(white_list))
    ...
    def send_news(self, name, risk, type, time):
        ...
        if risk == 'serious':
            ...
            risk = "高风险"
            p_risk = '<span style="color:#FF5A5A;">高风险</span>'
        elif risk == 'medium':
            risk = "中风险"
            p_risk = '<span style="color:#FF9900;">中风险</span>'
        else:
            risk = "低风险"
            p_risk = '<span style="color:#DDC400;">低风险</span>'
    ...
    def get_log(self, get):
        ...
        data = {}
        data['page'] = page.GetPage(info, '1,2,3,4,5,8')
        data['log'] = public.M('log').dbfile('bt_hids').order('id desc').limit(str(page.SHIFT) + ',' + str(page.ROW)).select()
        return data
