...
class backup_main:
    ...
    def __init__(self):
        ...
        if not self.__backup_path:
            if os.path.exists(self.__backup_path):
                ...
                os.system('chmod -R 600 %s' % self.__backup_path)
    ...
    def BackupSiteConfig(self, name):
        ...
        sslDstPath = '/www/server/panel/vhost/ssl/' + name + '/'
        sslFiles = ['privkey.pem', 'fullchain.pem', 'partnerOrderId', 'README','info.json']
    ...
    def RecoverySSl(self, path, site):
        ...
        sslDstPathtwo = '/www/server/panel/vhost/ssl/' + site + '/'
        os.system('mkdir -p ' + sslDstPath)
        sslFilesone = ['privkey.pem', 'fullchain.pem']
        sslFilestwo = ['privkey.pem', 'fullchain.pem','info.json']
        for sslFileson in sslFilesone:
            if os.path.exists(sslSrcPath + sslFile):
                conf = public.readFile(sslSrcPath + sslFile)
                public.writeFile(sslDstPathone + sslFile, conf)
        for sslFilestwo in sslDstPathtwo:
            if os.path.exists(sslSrcPath + sslFile):
                conf = public.readFile(sslSrcPath + sslFile)
                public.writeFile(sslDstPathtwo + sslFile, conf)
