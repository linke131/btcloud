...
class App:
    ...
    def merge_environment_args(self, app_args=None):
        """合并环境配置运行参数"""
        tool = Tool()
        e_args_arr = []
        args_arr = []
        ...
        if not app_args:
            app_args = self.args
        if app_args:
            args_arr = tool.parse_str_args(app_args)
    ...
    def start(self, waitsecs=3):
        ...
        try:
            ...
            self.change_status(AppStatus.Starting)
            env_path = ""
            environment_args = []
            ...
            # global_logger.info("proc return code: {}".format(proc.returncode))
            end_time = time.time()
            pid = -1
            if not started:
                # 没有启动起来
                self.change_status(AppStatus.Startfailed)
    ...
    def stop(self, waitsecs=5, status=None):
        ...
        if not self.is_running():
            ...
            self.change_status(AppStatus.Stopping)

            # 优先处理终止进程无效的服务
            if self.args.find("octane:start") != -1:
                # 先执行laravel终止服务
                tool = Tool()
                cmd = ""
                env_path = ""
                environment_args = []
                args = self.args.replace("start", "stop")
                app_args = self.merge_environment_args(app_args=args)
                if self.environment:
                    env_path = self.environment.get_path()
                    environment_args = tool.parse_str_args(
                        self.environment.environment_args)
                ...
                if env_path:
                    ...
                    cmd = [self.main] + app_args
                # print("执行停止命令: {}".format(" ".join(cmd)))
                public.ExecShell(" ".join(cmd))
                pid = int(pid)
            else:
                pid = int(pid)
    ...
    def get_status(self):
        res = public.M("app").dbfile(NAME).field("status").\
            where("id=?", (self.id)).find()
        if "status" in res:
            s = res["status"]
            if not s:
                s = AppStatus.Stopped
    ...
    def get_load_info(self):
        ...
        if pid_content and pid_content.find(",") != -1:
            ...
            now = time.time()
            self.running_day = int((now - self.start_at) // 86400)

    def get_port_by_psutil(self, pid):
        """通过psutil获取端口号
    ...
    def get_port(self):
        ...
        try:
            ...
            # global_logger.error("获取端口调试命令: {}".format(cmd))
            ...
            if res:
                ...
                port = self.get_port_by_psutil(self.pid)
        except Exception as e:
            # global_logger.error("获取端口信息错误: {}".format(e))
            port = self.get_port_by_psutil(self.pid)
        self.port = port
...
class AppManager:
    ...
    def find_app(self, id, name=None, only_data=False):
        """查找应用"""
        db = public.M("app").dbfile(NAME)
        sql = db
        if id:
    ...
    def list_apps(self, pager=True, page=1, page_size=20, only_data=False, 
                  **kwargs):
        """应用列表
    ...
    def run_server(self):
        ...
        global_logger.info("正在开启后台服务线程。。。")
        # from threading import Thread
        # backgroud_thread = Thread(target=self.monitor)
        # backgroud_thread.start()
        self.monitor()
        # global_logger.info("后台服务线程开启。进程ID: {}。".format(os.getpid()))
    ...
    def recovery_app(self, id, max_retry=10):
        """循环重试启动app
        
        通过id生成一个recovery文件，标识当前正处在对该id的应用进行启动恢复。
        一个应用同时只能一个线程在恢复启动。
        
        recovery文件在后台进程启动时，会进程一次清理。
        单次启动最大等待时间为max_retry*sleep_interval，超过这个时间自动退出。
        每次启动后以应用状态为Startfailed或Abnormal作为启动失败的检测点。
        """
        ...
        try:
            ...
            if not os.path.exists(pid_dir):
                os.makedirs(pid_dir)
            recovery_file = os.path.join(pid_dir, str(id)+".recovery")
            if os.path.exists(recovery_file):
                return

            public.writeFile(recovery_file, str(time.time()))
            recovered = False
            to_start = False
            sleep_count = 0
            sleep_interval = 3
            max_sleep = max_retry * sleep_interval 
            while max_retry > 0:
                try:
                    app = self.find_app(id)
                    name = app.name
                    if app.is_running():
                        if name:
                            global_logger.info("{} 应用已启动。".format(name))
                        recovered = True
                        break
                    else:
                        if not to_start:
                            ...
                            max_retry -= 1
                            sleep_count = 0
                        elif app.status in [AppStatus.Startfailed, AppStatus.Abnormal]:
                            self.start_app(id)
                            max_retry -= 1
                            sleep_count = 0
                        # else:
                        #     global_logger.info("{} 循环启动状态检测： {}".format(name, app.status))
                    time.sleep(sleep_interval)
                    sleep_count += sleep_interval
                    if sleep_count > max_sleep:
                        global_logger.info("{} 启动时间过长。".format(name))
                        break
                except Exception as e:
                    global_logger.error("{} 重新启动异常: {}".format(name, e))
                    break
            if not recovered:
                global_logger.error("{} {}次尝试启动后失败。".format(name, max_retry))
                app.change_status(AppStatus.Stopped)
        except Exception as e:
            global_logger.error("{} 重新启动异常: {}".format(name, e))
        finally:
            if recovery_file and os.path.isfile(recovery_file):
    ...
    def monitor(self):
        ...
        while True:
            gc = GlobalConfig()
            gc.init_global_logger()
            apps, _page, _total = self.list_apps(pager=False)
            # if DEBUG:
            #     process = psutil.Process(os.getpid())
            #     global_logger.info(
            #         "监控进程：{}|{} 正在守护应用...".format(os.getpid(), process.name()))
            for app in apps:
                if not app.daemon: continue
                status = app.status
                app_id = app.id
                
                if status == AppStatus.Running or status == AppStatus.Shutdown:
                    if not app.is_running():
                        # Recovery 
                        pid_dir = os.path.join(PLUGIN_DIR, "pids")
                        recovery_file = os.path.join(pid_dir, str(app_id)+".recovery")
                        if not os.path.exists(recovery_file):
                            max_retry = app.max_retry
                            recovery_thread = threading.Thread(target=self.recovery_app, args=(app_id, max_retry,))
...
class btappmanager_main:
    ...
    def run_server(self, get):
        ...
        pids_dir = os.path.join(PLUGIN_DIR, "pids")
        if os.path.exists(pids_dir):
            for rf in os.listdir(pids_dir):
                if rf.endswith(".recovery"):
                    os.remove(os.path.join(pids_dir, rf))
    ...
    def add_app(self, get):
        ...
        if "environment_variables" in get:
            ...
            if environment_variables.find("=") == -1:
                return public.returnMsg(False, "环境变量参数必须包含等号。")
        daemon = True
        if "daemon" in get:
