log_by_lua_block {
    ...
    local function statistics_request()
        ...
        if not is_spider and method == 'GET' and tonumber(status_code) == 200 and body_length > 512 then
            local ua = ''
            if request_header['user-agent'] then
                ...
                if string_find(out_header['content-type'], 'text/html', 1, true) then
                    ...
                    if request_header['user-agent'] then
                        if string_find(ua, 'mozilla') then
                            ...
                            --debug("ip------: ".. tostring(ip))
                            local uv_token = ngx.md5(ip .. request_header['user-agent'] .. today2)
                            --cache:delete(uv_token)
                            --debug("uv_token------: ".. tostring(uv_token))
    ...
    local function update_stat(update_server_name, db, stat_table, key, columns)
        -- 根据指定表名，更新统计数据
        --debug("columns"..tostring(columns))
        if not columns then
            return
        end
        ...
        --debug("res"..tostring(res))
        --debug("err"..tostring(err))
        stmt:finalize()
        --debug("stat_table"..tostring(stat_table))
        ...
        --debug("status---------:  "..tostring(status))
        --debug("errorString---------:  "..tostring(errorString))
    end
    ...
    local function store_logs(store_server_name)
        ...
        --debug("Open db:logs_db"..tostring(logs_db))
        --debug("Open db:total_db"..tostring(total_db))
        if logs_db ~= nil then
        ...
        if tostring(res) ~= "5" then
            ...
            --debug("Commit restotal_db:"..tostring(res))
            --debug("Commit errtotal_db:"..tostring(err))
            local res, err = logs_db:execute([[COMMIT]])
            --debug("Commit reslogs_db:"..tostring(res))
            --debug("Commit errlogs_db:"..tostring(err))
    ...
    function _M.cache_logs()
        ...
        --debug(require("cjson").encode(config))
        ...
        --debug(require("cjson").encode(logline))
    ...
    local function log_write_logs()
        ...
        if  upstream_connect_time == nil or upstream_header_time == nil or upstream_response_time == nil then
        ...
        if ngx.var.upstream_cache_status == "HIT" then
            ...
            upstream_bytes_received = ngx.var.body_bytes_sent
            upstream_response_length = ngx.var.content_length or ngx.var.body_bytes_sent
        else
        ...
        local logline = {
            ...
            tostring(upstream_addr), request_time,
            tostring(upstream_response_length), tostring(upstream_bytes_received),
            tostring(upstream_status), tostring(ngx.var.upstream_cache_status)
        }
        local path = log_path .. '/logs/' .. ngx.var.site_name
        if not log_check_dir(path) then
            log_create_dir(path)
        end
