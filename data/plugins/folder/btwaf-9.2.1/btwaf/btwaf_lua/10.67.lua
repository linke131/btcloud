...
local xss_path = "/www/server/btwaf/xss_parser/"
if not package.path:find(xss_path) then
	package.path = xss_path .. "?.lua;" .. package.path
end
		...
		if continue_key(k) then
			if type(v)=='string' then
				...
				if config['post_is_xss'] then
					local isxss = xss_engine.xss(tostring(v))
					if isxss then
						...
						if ngx.shared.btwaf_data:get(ip..'xss')>=post_is_xss_count then
							error_rule = '语义分析分析出xss跨站攻击' .. ' >> ' .. tostring(k)..'='..tostring(v)..' >> '..tostring(v)..' 如需调整请在【全局设置-HTTP请求过滤-语义分析-POST传参XSS防御】调整阈值'
							is_type="XSS攻击"
							return true
						end
					end
...
local function post_urlencoded(request_args)
	if ngx.shared.btwaf_data:get(ip..'post') then 
	 ...
	 if is_ngx_match_urlencoded(post_rules,request_args,'post',disable,25) then 
	    write_log('post','POST拦截')
	    return_html(config['post']['status'],post_html)
	 end 
	 
	 if ngx.shared.btwaf_data:get(ip) and  ngx.shared.btwaf_data:get(ip) >1000 then 
	    is_type="ip攻击次数多被拦截"
	    lan_ip('post','ip攻击次数多被拦截')
	 end
...
local function args_urlencoded(request_args)
    if is_ngx_match2(args_rules,request_args,'args') then 
        write_log('args','ip攻击次数多被拦截')
		return_html(config['get']['status'],get_html)
    end 
    if ngx.shared.btwaf_data:get(ip) >1000 then 
		return lan_ip('post','ip攻击次数多被拦截')
    end 
	               ...
	               if type(part_body)=='string' then
                    ...
                    if type(part_body) =='string' and  part_body ~=nil then 
					    if ngx.re.find(part_body,[["php"]],'ijo') or ngx.re.find(part_body,[['php']],'ijo') or  ngx.re.find(part_body,[[<\?]],'ijo') or  ngx.re.find(part_body,[[phpinfo\(]],'ijo') or ngx.re.find(part_body,[[\$_SERVER]],'ijo') or ngx.re.find(part_body,[[<\?php]],'ijo') or ngx.re.find(part_body,[[fputs]],'ijo') or ngx.re.find(part_body,[[file_put_contents]],'ijo') or ngx.re.find(part_body,[[file_get_contents]],'ijo') or ngx.re.find(part_body,[[eval\(]],'ijo') or ngx.re.find(part_body,[[\$_POST]],'ijo')  or ngx.re.find(part_body,[[\$_GET]],'ijo') or ngx.re.find(part_body,[[base64_decode\(]],'ijo') or ngx.re.find(part_body,[[\$_REQUEST]],'ijo') or ngx.re.find(part_body,[[assert\(]],'ijo') or ngx.re.find(part_body,[[copy\(]],'ijo') or ngx.re.find(part_body,[[create_function\(]],'ijo') or ngx.re.find(part_body,[[preg_replace\(]],'ijo') or ngx.re.find(part_body,[[preg_filter\(]],'ijo') or ngx.re.find(part_body,[[system\(]],'ijo') or ngx.re.find(part_body,[[header_register_callback\(]],'ijo') or ngx.re.find(part_body,[[curl_init\(]],'ijo') or ngx.re.find(part_body,[[curl_error\(]],'ijo') or ngx.re.find(part_body,[[fopen\(]],'ijo')  or ngx.re.find(part_body,[[stream_context_create\(]],'ijo') or ngx.re.find(part_body,[[fsockopen\(]],'ijo')  then
					        local php_version=""
					        if  site_config[server_name]['php_version']~=nil then 
					            php_version= site_config[server_name]['php_version']
					        else
					            php_version="php"
					            if not check_dir("/usr/bin/php") then 
					                return false
					            end 
					        end 
					        local random_id =""
                        	...
                        	if not ok then
                        	    random_id=getuuid(0)..".txt"
                        	else
                        	     random_id=uuid.generate()..".txt"
                        	end
                        	if random_id ~="" then 
    					        write_file("/dev/shm/"..random_id,part_body)
                                local t =io.popen("cd /www/wwwlogs && "..php_version.." -l /dev/shm/"..random_id)
                                local a = t:read("*all")
                                if a and  ngx.re.find(a,"No syntax errors detected in /dev/shm/") then 
                                    os.remove("/dev/shm/"..random_id)
                                    is_type='webshell防御'
...
function get_is_webshell()
    if config['webshell_opens'] then 
        local webshell_info,lens=get_webshell()
		local url_infos=split2(request_uri,'?')
        local url_data=url_infos[1]
        if lens>=1 then 
            if  url_data and url_data then 
                local paths =ngx.var.document_root..url_data
                if webshell_info[paths] then 
                    if not ngx.shared.btwaf_data:get(paths) then 
                        ngx.shared.btwaf_data:set(paths,1,7800)
                    else
                        ngx.shared.btwaf_data:incr(paths,1)
                    end 
                    if ngx.shared.btwaf_data:get(paths)>10 then 
                        ngx.exit(404)
                    else
                         lan_ip('webshell','访问的URL为webshell,当前IP已经被封锁,如想关闭此功能如下操作--【全局设置-木马查杀】')
                    end 
                end 
            end 
        end 
