...
local function binary_search(data, value)
...
local function drop_abroad()
	...
	if binary_search(cnlist,ipn2) then return false end
...
local function post()
	...
	if count_sieze(request_args)>=8000 then
		is_type='参数超过8000拦截'
		error_rule = '参数太多POST传递的参数数量超过3000,拒绝访问,如有误报请点击误报'
		write_log('sql','参数太多POST传递的参数数量超过8000,拒绝访问,如有误报请点击误报')
		return_html_data('网站防火墙','您的请求带有不合法参数，已被网站管理员设置拦截','网站防火墙提醒您POST传递的参数数量超过8000,拒绝访问','点击误报')
	end
...
function get_country()
    ...
    if ip_postion["country"]["country"]==nil then return false end 
    ngx.shared.btwaf:set("get_country"..ip,ip_postion["country"]["country"],3600)
    return ip_postion["country"]["country"]
end 
