        ...
        ReaderTable.prototype = {
            ...
            */
            $custom_page: function (total, number) {
                ...
                var html = '<div clas="page">', config = this.config.page, page = Math.ceil(total / config.number);
                ...
                if (page > 1 && config.page < page) {
                    html += '<a class="Pstart" href="p=' + (config.page + 1) + '">下一页</a><a class="Pstart" href="p=' + page + '">尾页</a>';
                }
    ...
    form: function (config) {
        ...
        ReaderForm.prototype = {
            ...
            $reader_form_find: function (item) {
                ...
                switch (item.type) {
                    ...
                    case 'number': // 数字
                        ...
                        break;
                    case 'textarea':
                        html += '<textarea class="bt-input-text"' + _that.$verify(style, 'style') + attribute + ' >' + (that.data[item.name] || '') + '</textarea>';
            ...
            $reader_select: function (item, style, attribute, index) {
                ...
                return '<div class="bt_select_updown mr10 ' + (item.disabled ? 'bt-disabled' : '') + ' ' + _that.$verify(item.hide, 'hide', true) + _that.$verify(item.class) + '" ' + _that.$verify(style, 'style') + ' data-name="' + item.name + '">' +
                    '<span class="bt_select_value"><span class="bt_select_content">' + (active.title || '获取数据中...') + '</span><span class="glyphicon glyphicon-triangle-bottom ml5"></span></span>' +
            ...
            */
            $submit: function (param, callback, tips) {
                ...
                if (typeof this.config.url == "undefined") {
                    bt_waf_tools.msg('请求提交地址不能为空！', false);
                    return false;
                }
                bt_waf_tools.send({
                    url: this.config.url,
