...
Install_socket()
{
	...
	CentosStream8Check=$(cat /etc/redhat-release|grep -i "Centos Stream"|grep 8)
	if [ "${Centos8Check}" ] || [ "${CentosStream8Check}" ];then
		if [ ! -f /usr/share/lua/5.3/socket.lua ];then
		    cd $pluginPath &&  unzip $pluginPath/luasocket-master.zip && cd $pluginPath/luasocket-master && make && make install
        	mkdir -p /usr/share/lua/5.3/
            rm -rf /usr/share/lua/5.3/mime.lua /usr/share/lua/5.3/socket.lua /usr/share/lua/5.3/socket /usr/share/lua/5.3/mime
    		cp -a -r  /usr/local/lib/lua/5.1/mime /usr/lib64/lua/5.3/
            cp -a -r  /usr/local/lib/lua/5.1/socket /usr/lib64/lua/5.3/
            cp -a -r  /usr/local/share/lua/5.1/socket/* /usr/share/lua/5.3/socket/
            cp -a -r  /usr/local/share/lua/5.1/*.lua /usr/share/lua/5.3
		fi
	fi
