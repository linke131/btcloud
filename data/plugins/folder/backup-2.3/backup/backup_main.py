...
class backup_main:
    ...
    # 备份redirectconf文件 
    def RedirectConf(self,path):   
        data = public.M('redirect').field('sitename,redirectname,tourl,redirectdomain,redirectpath,redirecttype,type,domainorpath,holdpath').select()
        if len(data) >=  1:
            redirect_conf = path + '/redirect.conf'
            redirectConf = '/www/server/panel/data/redirect.conf'
            if os.path.exists(redirectConf):
                public.ExecShell('\cp -r %s %s' % (redirectConf,redirect_conf))
                print('备份完成，共' + str(len(data)) + '条日志!')
                return redirect_conf

    # 备份proxyfile_json文件
    def ProxyFileConfig(self,path):
        data = public.M('proxyfile').field('proxyname,sitename,proxydir,proxysite,todomain,type,subfilter,advanced,cachetime').select()
        if len(data) >= 1:
            proxy_conf = path + '/proxyfile.json'
            proxyConf = '/www/server/panel/data/proxyfile.json'
            if os.path.exists(proxyConf):
                public.ExecShell('\cp -r %s %s' % (proxyConf,proxy_conf))
                print('备份完成，共' + str(len(data)) + '条日志!')
                return proxy_conf
    ...
    def BackupSiteConfig(self, name, path):
        ...
        os.system('mkdir -p ' + bt_conf + '/redirect')
        os.system('mkdir -p ' + bt_conf + '/proxy')
        print("备份Nginx配置文件")
        ...
        # 备份Nginx重定向目录
        php_NginxRedirect = bt_conf + '/redirect/'
        phpNginxRedirect = '/www/server/panel/vhost/nginx/redirect/%s/' %name 
        if os.path.exists(phpNginxRedirect):
            public.ExecShell('\cp -r %s %s' % (phpNginxRedirect,php_NginxRedirect))	

        # 备份Nginx反向代理目录
        php_NginxProxy = bt_conf + '/proxy/'
        phpNginxProxy = '/www/server/panel/vhost/nginx/proxy/%s/' %name 
        if os.path.exists(phpNginxProxy):
            public.ExecShell('\cp -r %s %s' % (phpNginxProxy,php_NginxProxy))
        ...
        # 备份apache重定向目录
        php_ApacheRedirect = bt_conf + '/redirect/' 
        phpApacheRedirect = '/www/server/panel/vhost/apache/redirect/%s/' %name 
        if os.path.exists(phpApacheRedirect):
            public.ExecShell('\cp -r %s %s' % (phpApacheRedirect,php_ApacheRedirect))   

        # 备份apache反向代理目录
        php_ApacheProxy = bt_conf + '/proxy/' 
        phpApacherProxy = '/www/server/panel/vhost/apache/proxy/%s/' %name 
        if os.path.exists(phpApacherProxy):
            public.ExecShell('\cp -r %s %s' % (phpApacherProxy,php_ApacheProxy)) 
    ...
    def Backup_all(self, get):
        ...
        self.RedirectConf(backup_path[2])
        self.ProxyFileConfig(backup_path[2])
    ...
    def ImportData(self, get):
        ...
        if os.path.exists(path):
            ...
            redirect = self.RecoveryRedirectConf(path)
            proxy = self.RecoveryProxyConf(path)
            ...
            ret = {'log': log, 'firewalld': fire, 'ftp': ftp,'redirect':redirect,'proxy':proxy,'ssls': ssls, 'site': site, 'cron': cron, 'plugin': True, 'system': system}
            if ret:
                if type != 'local':
    ...
    # 恢复redirectconf文件
    def RecoveryRedirectConf(self,path):
        log_path =  path + '/redirect.conf'
        redirectConf = '/www/server/panel/data/redirect.conf'
        if os.path.exists(log_path):
            public.ExecShell('\cp -r %s %s' % (log_path,redirectConf))
        else:
            return False

    # 恢复proxyfile_json文件
    def RecoveryProxyConf(self,path):
        log_path =  path + '/proxyfile.json'
        proxyConf = '/www/server/panel/data/proxyfile.json'
        if os.path.exists(log_path):
            public.ExecShell('\cp -r %s %s' % (log_path,proxyConf))
        else:
            return False
    ...
    def RecoveryCron(self, path):
        ...
        if os.path.exists(cron_path):
            ...
            crontab.crontab().GetCrontab(get)
    ...
    def RecoverySSl(self, path, site):
        ...
        # 恢复nginx重定向目录
        php_NginxRedirect = ssl_path + '/bt_conf/redirect/' + site + '/'
        phpNginxRedirect = '/www/server/panel/vhost/nginx/redirect/%s/' %site 
        if os.path.exists(php_NginxRedirect):
            public.ExecShell('\cp -r %s %s' % (php_NginxRedirect,phpNginxRedirect))

        # 恢复nginx反向代理目录
        php_NginxProxy = ssl_path + '/bt_conf/proxy/' + site + '/'
        phpNginxProxy = '/www/server/panel/vhost/nginx/proxy/%s/' %site 
        if os.path.exists(php_NginxProxy):
            public.ExecShell('\cp -r %s %s' % (php_NginxProxy,phpNginxProxy))	
        ...
        # 恢复apache重定向目录
        php_ApacheRedirect = ssl_path + '/bt_conf/redirect/' + site + '/'
        phpApacheRedirect = '/www/server/panel/vhost/apache/redirect/%s/' %site 
        if os.path.exists(php_ApacheRedirect):
            public.ExecShell('\cp -r %s %s' % (php_ApacheRedirect,phpApacheRedirect))

        # 恢复apache反向代理目录
        php_ApacheProxy = ssl_path + '/bt_conf/proxy/' + site + '/'
        phpApacheProxy = '/www/server/panel/vhost/apache/proxy/%s/' %site 
        if os.path.exists(php_ApacheProxy):
            public.ExecShell('\cp -r %s %s' % (php_ApacheProxy,phpApacheProxy))
