...
class resource_manager_main:
    ...
    def get_process_ps(self, name, pid):
        processPs = {'mysqld': 'MySQL服务', 'php-fpm': 'PHP子进程', 'php-cgi': 'PHP-CGI进程',
                     ...
                     'gunicorn': "宝塔面板", "BT-Panel": '宝塔面板', 'baota_coll': "堡塔云控-主控端",
    ...
    def get_io_write(self, pid, io_write):
        ...
        if 'io_write' not in self.disk_write_old_info[pid]: self.disk_write_old_info[pid]['io_write'] = \
            self.disk_write_new_info[pid]['io_write']
    ...
    def get_io_read(self, pid, io_read):
        ...
        if 'io_read' not in self.disk_read_old_info[pid]: self.disk_read_old_info[pid]['io_read'] = \
            self.disk_read_new_info[pid]['io_read']
