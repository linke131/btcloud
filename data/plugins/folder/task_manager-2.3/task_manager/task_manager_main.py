...
class task_manager_main:
    ...
    def get_process_list(self, get):
        ...
        # data['process_list'] = self.__pro_s_s(data['process_list'])
        return data
    ...
    def cpu_status(self, get: Dict) -> Dict:
        try:
            get = {'sortx': 'cpu_percent',
                   'reverse': True}
            get = public.to_dict_obj(get)
            data = self.get_process_list(get)

            return data['process_list'][:5]
        except:
            public.print_log(traceback.format_exc())

    def mem_status(self, get: Dict) -> Dict:
        try:
            get = {'sortx': 'memory_used',
                   'reverse': True}
            get = public.to_dict_obj(get)
            data = self.get_process_list(get)
