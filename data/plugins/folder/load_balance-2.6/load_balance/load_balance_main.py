...
class UpStream(KeyObject):
    ...
    def save_to_config(self) -> Tuple[bool, str]:
        ...
        nodes = "\n".join([node.to_config_str() for node in self.nodes])  # type: node:Node
        up_body = 'upstream %s {\n' % self.name + alg + nodes + '\n}'
...
class Node(KeyObject):
    ...
    def to_config_str(self) -> str:
        return '    server {s}:{p} max_fails={mf} fail_timeout={ft}s weight={w};'.format(
            s=self.server, p=self.port, mf=self.max_fails, ft=self.fail_timeout, w=self.weight)
