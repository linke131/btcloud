...
class firewall_main:
    ...
    def get_firewall_status(self, get):
        if self.__isUfw:
            result = public.ExecShell('/usr/sbin/ufw status | grep "Status"')[
                0].replace("\n", "")
        ...
        if self.__isFirewalld:
            ...
            if result.find(
                'iptables: Firewall is not running') != -1: return False
            return True
        ...
        if hasattr(args, 'query'):
            if args.query:
                ...
                for rule in rule_list:
                    if rule['ports'].find(args.query) != -1: tmpList.append(
                        rule)
                rule_list = tmpList
    ...
    def modify_rules(self, get):
        ...
        public.M('firewall_new').where('id=?', id).update(
            {'address': address, 'protocol': protocol, 'ports': ports,
             'types': types, 'brief': brief, 'addtime': addtime})
        try:
            if int(ports) == 22:
                self.delete_service()
        except:
        ...
        if hasattr(args, 'query'):
            if args.query:
                ...
                for data in data_list:
                    if data['start_port'].find(
                        args.query) != -1: tmpList.append(data)
                data_list = tmpList
