...
class SendMail:
    ...
    def sendMail(self, receiveUsers):
        ...
        try:
            self.mailServer.sendmail(self.mailUser, receiveUsers,
                                     self.msg.as_string())
            # 保存邮件到发件箱
            local_part, domain = self.mailUser.split('@')
            dir_path = '/www/vmail/{0}/{1}/.Sent/cur'.format(
                domain, local_part)
            if not os.path.isdir(dir_path):
                os.makedirs(dir_path)
            file_name = public.GetRandomString(36)
            if file_name in [
                    item.split(':')[0] for item in os.listdir(dir_path)
            ]:
                file_name = public.GetRandomString(54)
            public.writeFile(os.path.join(dir_path, file_name),
                             self.msg.as_string())
...
class mail_sys_main:
    ...
    def __init__(self):
        ...
        self.check_rspamd_page()

        create_table_str = self.M('sqlite_master').where(
            'type=? AND name=?', ('table', 'domain')).getField('sql')
        if create_table_str and 'a_record' not in create_table_str:
            self.M('domain').execute(
                'ALTER TABLE `domain` ADD COLUMN `a_record` Text default "";')

    def check_rspamd_page(self):
        """
        @name 检测垃圾箱页面并修复
        @author hezhihong
        """
        plugin_path = public.get_plugin_path()
        src_rspamd_path = os.path.join(plugin_path, 'mail_sys/rspamd.zip')
        rspamd_path = '/usr/share/rspamd/www/'
        if not os.path.exists(rspamd_path +
                              'rspamd/') and os.path.isfile(src_rspamd_path):
            public.ExecShell('unzip {} -d {}'.format(src_rspamd_path,
                                                     rspamd_path))
    ...
    def check_mail_sys(self, args):
        if os.path.exists('/etc/postfix/sqlite_virtual_domains_maps.cf'):
            ...
            if not result:
                return public.returnMsg(False, "找不到postfix配置文件")
            ...
            if not dovecot_conf or not re.search(r"\n*protocol\s*imap",
                                                 dovecot_conf):
                return public.returnMsg(False, '配置dovecot失败')
    ...
    def _check_smtp_port(self):
        ...
        host_list = [
            'mx1.qq.com', 'mx2.qq.com', 'mx3.qq.com', 'smtp.gmail.com'
        ]
    ...
    def repair_dovecot(self, args=None):
        ...
        if os.path.exists('/etc/dovecot/conf.d/10-ssl.conf'):
            ...
            shutil.move('/etc/dovecot/conf.d/10-ssl.conf',
                        '/tmp/10-ssl.conf_aap_bak')
        if self.sys_v == 'centos7':
            ...
            status = True
        if os.path.exists('/tmp/10-ssl.conf_aap_bak') and os.path.exists(
                '/etc/dovecot/conf.d'):
            if os.path.exists("/etc/dovecot/conf.d/10-ssl.conf"):
                os.remove('/etc/dovecot/conf.d/10-ssl.conf')
            shutil.move('/tmp/10-ssl.conf_aap_bak',
                        '/etc/dovecot/conf.d/10-ssl.conf')
        return public.returnMsg(status,
    ...
    def repair_service_conf(self, args=None):
        ...
        if service_name.lower() not in ['postfix', 'dovecot', 'rspamd']:
            return public.returnMsg(False, '服务名不正确')
    ...
    def get_service_status(self, args=None):
        data = {}
        data[
            'change_rspamd'] = True if "smtpd_milters = inet:127.0.0.1:11332" not in public.readFile(
                "/etc/postfix/main.cf") else False
        data['postfix'] = public.process_exists('master',
                                                '/usr/libexec/postfix/master')
        data['dovecot'] = public.process_exists('dovecot', '/usr/sbin/dovecot')
        data['rspamd'] = public.process_exists('rspamd', '/usr/bin/rspamd')
        data['opendkim'] = public.process_exists('opendkim',
                                                 '/usr/sbin/opendkim')
        if "ubuntu" in self.sys_v:
            data['postfix'] = public.process_exists(
                'master', '/usr/lib/postfix/sbin/master')
        return data
    ...
    def flush_domain_record(self, args):
        ...
        if args.domain == 'all':
            ...
            for item in data_list:
                try:
                    ...
                    return public.returnMsg(False, '请检查Rspamd服务器是否已经启动！')
        ...
        return public.returnMsg(True, '刷新成功！')

    def get_record_in_cache(self, item):
        try:
            item['mx_status'] = self._session['{0}:{1}'.format(
                item['domain'], 'MX')]["status"]
            item['spf_status'] = self._session['{0}:{1}'.format(
                item['domain'], 'TXT')]["status"]
            ...
            item['a_status'] = self._session['{0}:{1}'.format(
                item['a_record'], 'A')]["status"]
        except:
            self._gevent_jobs(item['domain'], item['a_record'])
    ...
    def get_domains(self, args):
        ...
        data_list = self.M('domain').order('created desc').limit(
            page_data['shift'] + ',' +
            page_data['row']).field('domain,a_record,created,active').select()
        for item in data_list:
            try:
                if os.path.exists("/usr/bin/rspamd"):
                    self.set_rspamd_dkim_key(item['domain'])
                ...
                return public.returnMsg(False, '请检查rspamd服务状态是否正常')
            ...
            item[
                'dmarc_value'] = 'v=DMARC1;p=quarantine;rua=mailto:admin@{0}'.format(
                    item['domain'])
            item['mx_record'] = item['a_record']
            item['ssl_status'] = self._get_multiple_certificate_domain_status(
                item['domain'])
...
#BT_DOMAIN_DKIM_END
    ...
    def _gen_dkim_key(self, domain):
        ...
        if not os.path.exists(
                '/etc/opendkim/keys/{0}/default.private'.format(domain)):
            dkim_shell = '''
...
systemctl restart  opendkim'''.format(domain=domain)
            keytable = "default._domainkey.{domain} {domain}:default:/etc/opendkim/keys/{domain}/default.private".format(
                domain=domain)
            sigingtable = "*@{domain} default._domainkey.{domain}".format(
                domain=domain)
            ...
            if sigingtable_conf:
                if sigingtable not in sigingtable_conf:
                    ...
                    public.writeFile("/etc/opendkim/SigningTable",
                                     sigingtable_conf)
            public.ExecShell(dkim_shell)
    ...
    def _get_dkim_value(self, domain):
        ...
        if os.path.exists(
                opendkim_pub_file) and not os.path.exists(rspamd_pub_file):
    ...
    def _check_mx(self, domain):
        ...
        try:
            ...
            if not a_record:
                a_record = value
                self.M('domain').where('domain=?',
                                       domain).save('a_record', (a_record, ))
            if value == a_record:
                self._session[key] = {
                    "status": 1,
                    "v_time": now,
    ...
    def get_mx_txt_cache(self, args):
        ...
        dkim_key = '{0}:{1}'.format('default._domainkey.{0}'.format(domain),
                                    'TXT')
    ...
    def delete_mx_txt_cache(self, args):
        ...
        return public.returnMsg(True,
                                '删除域名({})在session中的缓存记录成功'.format(domain))
    ...
    def add_domain(self, args):
        ...
        if not a_record.endswith(domain):
            return public.returnMsg(False, 'A记录 [{}] 不属于该域名'.format(a_record))
        ...
        return public.returnMsg(True, '新增域名[{0}]成功!'.format(domain))
    ...
    def edit_domain_record(self, args):
        ...
        a_record = args.a_record
        if self.M('domain').where('domain=?', domain).count() == 0:
            return public.returnMsg(False, '该域名不存在')
        self.M('domain').where('domain=?',
                               domain).save('a_record', (a_record, ))
        return public.returnMsg(True, '修改域名[{0}]A记录成功!'.format(domain))
    ...
    def get_mailboxs(self, args):
        ...
        if 'domain' in args:
            ...
            data_list = self.M('mailbox').order('created desc').limit(
                page_data['shift'] + ',' +
                page_data['row']).where('domain=?', domain).field(
                    'full_name,username,quota,created,modified,active,is_admin'
                ).select()
            # 返回数据到前端
    ...
    def get_all_user(self, args):
        if 'domain' in args:
            data_list = self.M('mailbox').where(
                'domain=? AND active=?', (args.domain, 1)).field(
                    'full_name,username,quota,created,modified,active,is_admin'
                ).select()
        else:
    ...
    def _check_passwd(self, password):
        return True if re.search(r"^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).*$",
                                 password) and len(password) >= 8 else False

    def _check_email_address(self, email_address):
        return True if re.match(r"^\w+([.-]?\w+)*@.*",
    ...
    def __create_mail_box_mulitiple(self, info, args):
        ...
        return {
            ...
            True,
            'msg':
            "Create the mailbox [ {} ] successfully".format(
    ...
    def add_mailbox_multiple(self, args):
        ...
        info = [
            dict(zip(key, i))
            for i in [i.strip().split('|') for i in json.loads(args.content)]
        ]
        if not info:
            return public.returnMsg(False, '参数为空, 请检查密码强度！')
    ...
    def add_mailbox(self, args):
        ...
        if not self._check_email_address(username):
            return public.returnMsg(False, '邮箱地址格式不正确')
        if not username.islower():
            return public.returnMsg(False, '邮箱地址不能有大写字母！')
        ...
        if domain not in domain_list:
            return public.returnMsg(False, '域名列表不存在域名{}'.format(domain))
        ...
        self.M('mailbox').add(
            ...
            (args.full_name, is_admin, username, password_encrypt,
             password_encode, args.username + '/', quota, local_part, domain,
             cur_time, cur_time))
        ...
        public.ExecShell('chown -R vmail:mail /www/vmail/{0}/{1}'.format(
            domain, local_part))
        self.create_mail_box(username, args.password)
        return public.returnMsg(True, '增加邮箱用户[{0}]成功!'.format(username))
    ...
    def update_mailbox(self, args):
        ...
        if 'password' in args and args.password != '':
            ...
            self.M('mailbox').where('username=?', args.username).save(
                ...
                (password_encrypt, password_encode, args.full_name, quota,
                 cur_time, args.active, args.is_admin))
        else:
            self.M('mailbox').where('username=?', args.username).save(
                'full_name,quota,modified,active,is_admin',
    ...
    def delete_mailbox(self, args):
        ...
        # 在虚拟用户家目录删除对应邮箱的目录
        public.ExecShell('rm -rf /www/vmail/{0}/{1}'.format(
            domain, local_part))
    ...
    def send_mail(self, args):
        ...
        if not service_status['postfix']:
            return public.returnMsg(False, '无法发送邮件, 错误原因: 部分服务未启动,请查看服务状态')
        if not self._check_smtp_port():
            return public.returnMsg(
                False, '部分云厂商(如：阿里云，腾讯云)默认关闭25端口，需联系厂商开通25端口后才能正常使用邮局服务')

        mail_from = args.mail_from
        data = self.M('mailbox').where(
            'username=?', mail_from).field('password_encode,full_name').find()
        ...
        try:
            ...
            send_mail_client.setMailInfo(data['full_name'], subject, content,
                                         subtype, files)
    ...
    def send_mail_http(self, args):
        ...
        try:
            data = self.M('mailbox').where(
                'username=?', mail_from).field('full_name').find()
            send_mail_client = SendMail(mail_from, password, 'localhost')
            send_mail_client.setMailInfo(data['full_name'], subject, content,
                                         subtype, files)
            result = send_mail_client.sendMail(mail_to)
    ...
    def delete_mail(self, args):
        ...
        if not os.path.exists(path):
            return public.returnMsg(False, '邮件路径不存在')
        os.remove(path)
        return public.returnMsg(True, '删除邮件成功')
    ...
    def service_admin(self, args):
        ...
        if type.lower() not in ['start', 'stop', 'restart', 'reload']:
            return public.returnMsg(False, '操作不正确')
...
'''
        ...
        try:
            ...
            if os.path.isdir(dir_path):
                ...
                return {
                    ...
                    'page':
                    page_data['page'].replace(
                        '/plugin?action=a&name=mail_sys&s=get_sent_mails&p=',
                        '')
                }
            else:
                page_data = public.get_page(0, int(args.p), 10)
    ...
    def set_postfix_ssl(self, csrpath, keypath, act):
        ...
        main_conf = re.sub(main_rep,
                           "smtpd_tls_cert_file = {}".format(csrpath),
                           main_conf)
        main_rep = r"smtpd_tls_key_file\s*=\s*.+"
        main_conf = re.sub(main_rep, "smtpd_tls_key_file = {}".format(keypath),
                           main_conf)
    ...
    def get_dovecot_version(self, args=None):
        data = public.ExecShell(
            "dpkg -l|grep dovecot-core|awk -F':' '{print $2}'")[0]
        if os.path.exists('/etc/redhat-release'):
            data = public.ExecShell(
                'rpm -qa | grep dovecot | grep -v pigeonhole')[0].split('-')[1]
        return data
    ...
    def set_dovecot_ssl(self, csrpath, keypath, act):
        ...
        if dovecot_version.startswith('2.3'):
            if act == '1':
                if not os.path.exists(
                        '/etc/dovecot/dh.pem') or os.path.getsize(
                            '/etc/dovecot/dh.pem') < 300:
                    public.ExecShell(
    ...
    def set_mail_forward(self, args):
        ...
        if self.M('alias').where('address=?', args.user).count() > 0:
            return public.returnMsg(False, '被转发用户已经存在')
        # 检查域名是否存在
        if self.M('domain').where('domain=?', args.domain).count() <= 0:
            return public.returnMsg(False, '邮件服务器中不存在该域名')
        # 检查被转发用户是否存在邮局内
        if self.M('mailbox').where(
                'username=?', args.user).count() <= 0 and args.user[0] != '@':
            return public.returnMsg(False, '邮件服务器中不存在被转发用户')
        # 换行符替换为逗号
        ...
        self.M('alias').add('address,goto,domain,created,modified,active',
                            (args.user, forward_users, args.domain,
                             create_time, create_time, args.active))
        return public.returnMsg(True, '邮件转发添加成功')

    def edit_mail_forward(self, args):
        if self.M('alias').where('address=?', args.user).count() == 0:
            return public.returnMsg(False, '被转发用户不存在')
        # 换行符替换为逗号
        tmp = args.forward_user.split('\\n')
        if "\n" in tmp[0]:
            tmp = tmp[0].split('\n')
        ...
        self.M('alias').where('address=?', args.user).save(
            'goto,modified,active',
            (forward_users, modified_time, args.active))
        return public.returnMsg(True, '邮件转发编辑成功')

    def delete_mail_forward(self, args):
        if self.M('alias').where('address=?', args.user).count() == 0:
            return public.returnMsg(False, '转发用户不存在')
        self.M('alias').where('address=?', args.user).delete()
        return public.returnMsg(True, '邮件转发删除成功')
    ...
    def set_mail_bcc(self, args):
        ...
        data[args.type].append({
            "domain": args.domain,
            "user": args.user,
            "forward_user": args.forward_user
        })
...
""".format(smtphost=smtphost, port=port, username=username, passwd=passwd)
    ...
    def cancel_smtp_relay(self, args):
        ...
        if not tmp:
            return public.returnMsg(False, "smtp中继配置不存在")
        conf = re.sub(reg, "", conf)
        public.writeFile(self.postfix_main_cf, conf)
        return public.returnMsg(True, "取消邮件中继成功")
...
"""
        ...
        if args.spam_server_ip == 'localhost':
            ...
            anti_spam_conf = anti_spam_conf.format(
                spam_server_info['spam_server_ip'],
                spam_server_info['spam_server_port'])
            public.writeFile(self.postfix_main_cf, conf + anti_spam_conf)
        else:
            anti_spam_conf = anti_spam_conf.format(args.spam_server_ip,
                                                   args.spam_server_port)
            public.writeFile(self.postfix_main_cf, conf + anti_spam_conf)
...
if header :contains "X-Spam-Flag" "YES" {
    ...
    def turn_on_anti_spam(self, args):
        if args.spam_server_ip != 'localhost':
            return public.returnMsg(False, '目前还不支持远程扫描，正在测试该功能')
        if args.spam_server_ip == 'localhost' and not os.path.exists(
                '/www/server/panel/plugin/anti_spam'):
            return public.returnMsg(False, '请先安装[反垃圾邮件网关]插件,并将要监听的域名添加到配置里面')
    ...
    def get_backup_task_status(self, get):
        ...
        if not c_id: return public.returnMsg(False, '定时任务不存在!')
    ...
    def open_backup_task(self, get):
        ...
        if c_id:
            ...
            data['sBody'] = ''
            data[
                'backupTo'] = get.backupTo if 'backupTo' in get else 'localhost'
            data['sType'] = 'path'
    ...
    def restore(self, get):
        ...
        return public.returnMsg(True, '恢复数据完成')
    ...
    def _set_dovecot_cert_global(self, cert_file, cert_key, conf):
        ...
        if not re.search(default_cert_file, conf):
            return conf
        conf = re.sub(default_cert_file, "ssl_cert = <{}".format(cert_file),
                      conf)
    ...
    def _set_dovecot_certificate(self, args, a_record, cert_file, cert_key):
        ...
        if args.act == 'add' and not re.search(reg_cert, ssl_conf):
            ssl_conf = self._set_dovecot_cert_global(cert_file, cert_key,
                                                     ssl_conf)
            domain_ssl_conf = """
#DOMAIN_SSL_BEGIN_%s
local_name %s {
...
#DOMAIN_SSL_END_%s""" % (a_record, a_record, cert_file, cert_key, a_record)
    ...
    def _check_postfix_conf(self):
        ...
        if 'ubuntu 2' in self._get_ubuntu_version(
        ) or 'debian gnu/linux 10' in self._get_ubuntu_version():
            result = public.process_exists('master',
    ...
    def _modify_old_ssl_perameter(self, conf):
        if not os.path.exists('/etc/postfix/vmail_ssl.map'):
            ...
            if '#smtpd_tls_cert_file' not in conf:
                conf = conf.replace('smtpd_tls_cert_file',
                                    '#smtpd_tls_cert_file')
                conf = conf.replace('smtpd_tls_key_file',
                                    '#smtpd_tls_key_file')
            # 以前设置的获取证书路径
            old_cert_info = self._get_old_certificate_path(conf)
            # 设置新的证书配置和默认TLS配置
            if 'tls_server_sni_maps' not in conf:
                conf = self._set_new_certificate_conf(
                    conf, old_cert_info['cert_file'],
                    old_cert_info['cert_key'])
        public.writeFile(self.postfix_main_cf, conf)
    ...
    def set_mail_certificate_multiple(self, args):
        ...
        if not os.path.exists(
                '/etc/redhat-release'
        ) and 'debian gnu/linux 10' not in self._get_ubuntu_version():
            if 'ubuntu 2' not in self._get_ubuntu_version():
                if args.act == 'add':
                    ...
                    args.act = "0"
                pstr = """
/sbin/postconf -e "smtpd_tls_cert_file = /etc/pki/dovecot/certs/dovecot.pem"
/sbin/postconf -e "smtpd_tls_key_file = /etc/pki/dovecot/private/dovecot.pem"
    """
        ...
        # 获取域名的A记录
        arecord = self.M('domain').where(
            'domain=?', domain).field('a_record').find()['a_record']
        if arecord == '':
            return public.returnMsg(False, '所设置的域名不存在')
        # 验证域名证书是否有效
        if args.csr != '':
            verify_result = self._verify_certificate(args, cert_path,
                                                     cert_file, cert_key)
            if verify_result:
        ...
        #         os.remove(key)
        public.ExecShell(
            'postmap -F hash:/etc/postfix/vmail_ssl.map && systemctl restart postfix'
        )
        if not self._check_postfix_conf():
            self.restore_file(self.postfix_main_cf)
            return public.returnMsg(False, '设置失败，恢复配置文件')
        return public.returnMsg(True, '设置成功')
    ...
    def get_multiple_certificate(self, args):
        ...
        if not os.path.exists(csrpath):
            return public.returnMsg(False, '尚未为此域名设置SSL')
    ...
    def _get_multiple_certificate_domain_status(self, domain):
        path = '/www/server/panel/plugin/mail_sys/cert/{}/fullchain.pem'.format(
            domain)
    ...
    def enable_catchall(self, args):
        ...
        if catchall_exist:
            ...
            create_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
            self.M('alias').add('address,goto,domain,created,modified,active',
                                (domain, email, args.domain.strip(),
                                 create_time, create_time, '1'))
            new_conf = 'virtual_alias_maps= sqlite:/etc/postfix/btrule.cf'
            if public.FileMd5('/etc/postfix/btrule.cf'
                              ) != 'c96897b9285db8b53f2e1e2358918264':
                public.ExecShell(
                    'wget -O /etc/postfix/btrule.cf {}/mail_sys/postfix/btrule.cf -T 10'
                    .format(public.get_url()))
    ...
    def get_junk_mails(self, args):
        ...
        mail_list = []
        try:
            dir_path = '/www/vmail/{0}/{1}/.Junk/cur'.format(
                domain, local_part)
            if os.path.isdir(dir_path):
                # 先将new文件夹的邮件移动到cur文件夹
                new_path = '/www/vmail/{0}/{1}/.Junk/new'.format(
                    domain, local_part)
                if os.path.isdir(new_path):
                    for file in os.listdir(new_path):
                        src = os.path.join(new_path, file)
                ...
                for d in files:
                    ...
                    with open(mail_file, 'rb') as fp:
                        try:
                            ...
                            mailInfo = receive_mail_client.getMailInfo(
                                msg=message)
                            mailInfo['path'] = mail_file
                            mail_list.append(mailInfo)
                        except:
                ...
                return {
                    ...
                    'page':
                    page_data['page'].replace(
                        '/plugin?action=a&name=mail_sys&s=get_junk_mails&p=',
                        '')
                }
            else:
                page_data = public.get_page(0, int(args.p), 10)
            ...
            print(public.get_error_info())
            return public.returnMsg(False, '获取失败,错误原因[{0}]'.format(str(e)))
    ...
    def move_to_junk(self, get):
        ...
        src = get.path.strip()
        if not os.path.exists('/www/vmail/{0}/{1}/.Junk'.format(
                domain, local_part)):
            data = self.M('mailbox').where(
                'username=?',
                username).field('password_encode,full_name').find()
            password = self._decode(data['password_encode'])
        ...
        shutil.move(src, dst)
        return public.returnMsg(True, '标记成功')
    ...
    def get_ssl_info(self, domain):
        try:
            import data
            fullchain_file = '/www/server/panel/plugin/mail_sys/cert/{}/fullchain.pem'.format(
                domain)
            os.chown(fullchain_file, 0, 0)
            os.chmod(fullchain_file, 0o600)
            privkey_file = '/www/server/panel/plugin/mail_sys/cert/{}/privkey.pem'.format(
                domain)
            ...
            ssl_info['endtime'] = int(
                int(
                    time.mktime(time.strptime(ssl_info['notAfter'],
                                              "%Y-%m-%d")) - time.time()) /
                86400)
            return ssl_info
        except:
    ...
    def apply_cert(self, args):
        ...
        apply_cert = apply_cert_module.apply_cert(domains,
                                                  'dns',
                                                  args.auth_to,
                                                  auto_wildcard=1)
        return apply_cert
    ...
    def change_hostname(args):
        ...
        if not re.search(rep_domain, hostname):
            return public.returnMsg(False, "请输入完整域名，例如 mail.bt.com),")
        public.ExecShell(
            'hostnamectl set-hostname --static {}'.format(hostname))
    ...
    def check_confile_completeness(self):
        file_list = public.readFile("{}/services_file.txt".format(
            self.__setupPath))
        if not file_list:
            return [
                "%s/services_file.txt|{download_conf_url}/mail_sys" %
                self.__setupPath
            ]
    ...
    def check_smtp_port(args):
        ...
        if not re.search(rep_domain, domain):
            return public.returnMsg(False, "请输入完整域名，例如 smtp.qq.com)")
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        result = sock.connect_ex((domain, 25))
        if result == 0:
            return public.returnMsg(True, "25端口通信正常")
        return public.returnMsg(False, "25端口通信失败，请联系你的主机提供商进行放行")

    def download_file(self, args):
        filename = args.filename
        tmp = filename.split('|')
        local_file = tmp[0]
        remote_file = tmp[1].format(
            download_conf_url="http://node.aapanel.com")
        public.ExecShell(
            "wget {remote_file} -O {local_file} -T 10 --no-check-certificate".
            format(remote_file=remote_file, local_file=local_file))
        return public.returnMsg(True, "重新下载成功！")
