...
local function get_client_ip_bylog()
    ...
    if cdn == true then
        ...
        do
            if request_header[_] ~= nil and request_header[_] ~= "" then
                client_ip = split_bylog(httpd.headers_in[_],',')[1]
                break;
            end
        end
    end
...
local function statistics_request()
    ...
    local pvc = 0
    local uvc = 0
    if (method == 'GET' and tonumber(httpd.status) == 200) or
        (method == 'GET' and body_length > 200) then
        local ua = ''
        if httpd.headers_in['user-agent'] then
        ...
        if httpd.headers_out['content-type'] then
            if httpd.headers_out['content-type']:find("text/html") then
