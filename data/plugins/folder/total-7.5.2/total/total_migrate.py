...
class total_migrate:
    ...
    def init_site_db(self, server_name, target_data_dir=None, db_name="logs.db"):
        ...
        self.init_uri_stat_db(server_name)
    ...
    def init_uri_stat_db(self, site_name):
        '''
        检查并创建URI独立统计相关数据表
        @param site_name:
        @return:
        ...
        db_path = "/www/server/total/logs/{}/total.db".format(site_name)
        ts = tsqlite()
        ts.dbfile(db_path)
        if not ts:
            print("初始化uri独立统计数据库异常,数据库连接失败!")
            return
        ...
        if "uri_access" not in table_names:
            ts.query('''CREATE TABLE IF NOT EXISTS uri_access (
                id          INTEGER PRIMARY KEY,
                uri         TEXT,
                date        TEXT
            )
            ''')
        if "uri_count_stat_{}".format(current_year_month) not in table_names:
            ts.query('''CREATE TABLE IF NOT EXISTS uri_count_stat_{} (
                id       INTEGER PRIMARY KEY,
                uri_id   INTEGER,
                date       INTEGER,
                count_type TEXT,
        ...
        if "uri_client_stat_{}".format(current_year_month) not in table_names:
            ts.query('''CREATE TABLE IF NOT EXISTS uri_client_stat_{} (
                id       INTEGER PRIMARY KEY,
                uri_id   INTEGER,
                date       INTEGER,
                count_type TEXT,
            ...
            '''.format(current_year_month))
        if "uri_spider_stat_{}".format(current_year_month) not in table_names:
            ts.query('''CREATE TABLE IF NOT EXISTS uri_spider_stat_{} (
                id       INTEGER PRIMARY KEY,
                uri_id   INTEGER,
                date       INTEGER,
                count_type TEXT,
            ...
            '''.format(current_year_month))
        if "uri_flow_stat_{}".format(current_year_month) not in table_names:
            ts.query('''CREATE TABLE IF NOT EXISTS uri_flow_stat_{} (
                id       INTEGER PRIMARY KEY,
                uri_id   INTEGER,
                date       INTEGER,
                count_type TEXT,
            ...
            '''.format(current_year_month))
        if "uri_ip_stat_{}".format(current_year_month) not in table_names:
            ts.query('''CREATE TABLE IF NOT EXISTS uri_ip_stat_{} (
                id       INTEGER PRIMARY KEY,
                uri_id   INTEGER,
                date       INTEGER,
                ip         TEXT,
    ...
    def migrate_total(self):
        ...
        from total_migrate6x import site_logs_migrate
        logs_migrate = site_logs_migrate()
        for site in sites:
            site_name = site["name"]
            logs_migrate.migrate_run(site_name)
