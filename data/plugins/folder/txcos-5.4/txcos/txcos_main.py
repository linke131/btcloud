...
class OSClient(object):
    ...
    _aes_status = os.path.join(_panel_path, 'plugin/txcos/aes_status')
...
class COSClient(OSClient, object):
    ...
    __region     : 腾讯云COS 的地区
    __bucket_name     : COS 的Bucket
    __error_msg   ： 错误信息
    '''

    def __init__(self, load_config=True, config_file=None):
        super(COSClient, self).__init__(
    ...
    def init_config(self, data):
        ...
        self.__secret_id = keys.get("secret_id").strip()
        self.__secret_key = keys.get("secret_key").strip()
        self.__region = keys.get("region").strip()
        self.__bucket_name = keys.get("bucket_name").strip()

        # 设置存储路径和兼容旧版本
        if "backup_path" in keys.keys():
            bp = keys.get("backup_path").strip()
            if bp != "/":
                bp = self.get_path(bp)
            if bp:
                self.backup_path = bp
    ...
    def get_config(self):
        ...
        try:
            ...
            if not os.path.exists(bak_config_path):
                add_str = data['secret_id'] + '|' + data['secret_key'] + '|' + data['region'] + '|' + data[
                    'bucket_name'] + '|' + data['backup_path'] + data['pattern']
    ...
    def get_decrypt_config(self):
        ...
        if not conf['secret_id'] or not conf['secret_key'] or not conf['bucket_name'] or not conf['region']: return conf
        conf['secret_id'] = conf['secret_id'][:5] + '*' * 10 + conf['secret_id'][-5:]
        conf['secret_key'] = conf['secret_key'][:5] + '*' * 10 + conf['secret_key'][-5:]
        conf['bucket_name'] = conf['bucket_name'][:2] + '*' * 10 + conf['bucket_name'][-2:]
        conf['region'] = conf['region'][:2] + '*' * 10 + conf['region'][-2:]
        return conf
    ...
    def authorize(self):
        try:
            ...
            raise OsError("腾讯COS客户端连接异常, 请检查配置参数是否正确。")
    ...
    def upload_by_shell(self, path, to_path):
        ...
        error_message = '腾讯云存储客户端连接异常, 请检查配置参数是否正确。'
        offset = 0
        part_number = 1
        ...
        try:
            ...
            # print('22222222')
            response = oss_client.create_multipart_upload(Bucket=self.__bucket_name, Key=object_name)
        ...
        self.echo_info('正在上传到腾讯云COS，请稍候...'.format(public.format_date()))
        ...
        with open(path, "rb") as fp:
            ...
            while left_size > 0:
                ...
                response = oss_client.upload_part(Bucket=self.__bucket_name, Key=object_name, Body=part,
                                                  UploadId=upload_id, PartNumber=part_number
                                                  )
                part_value = {'ETag': response['ETag'], 'PartNumber': part_number}
                part_list.append(part_value)
                left_size -= part_size
                offset += part_size
                ...
                if echo_msg not in echo_msg_list:
                    ...
                    self.echo_info('进度：{} 速率：{}  耗时：{}  已完成：{} 总：{}'.format(upload_info[0], upload_info[1],
                                                                                      upload_info[2],
                                                                                      public.to_size(upload_info[3]),
                                                                                      public.to_size(upload_info[4])))
                ...
                if is_last:
                    ...
                    oss_client.complete_multipart_upload(Bucket=self.__bucket_name, Key=object_name, UploadId=upload_id,
    ...
    def resumable_upload(self,
        ...
        try:
            ...
            max_threads = 5
            metadata = {
                'x-cos-storage-class': self.get_config()['pattern']  # 设置自定义属性
            }
        ...
        if self.error_msg:
            self.error_msg += r"\n"
        self.error_msg += "文件{}上传出现错误。".format(object_name)
        return False
    ...
    def get_lib(self):
        ...
        list = {
            ...
            "check": ["/usr/lib/python2.6/site-packages/qcloud_cos/cos_auth.py",
...
class txcos_main:
    ...
    def get_config(self, get):
        data = self.client.get_decrypt_config()
        # data['pattern_list'] = {
        #     'StandardL': '标准存储',
        #     'Standard_IA': '低频存储',
        #     'ARCHIVE': '归档存储',
    ...
    def set_config(self, get):
        try:
            ...
            if hasattr(get, 'pattern') and get.backup_path != '':
            ...
            if secret_id.find('*') != -1 or secret_key.find('*') != -1 or bucket_name.find('*') != -1 or region.find(
            ...
            _client = COSClient(load_config=False)
    ...
    def set_save_path_pattern(self, get):
        if hasattr(get, 'backup_path') or hasattr(get, 'pattern'):
            try:
                conf = self.client.get_config()

                if hasattr(get, 'backup_path'):
                    conf['backup_path'] = get['backup_path']

                if hasattr(get, 'pattern'):
                    conf['pattern'] = get['pattern']

                self.set_config(public.to_dict_obj(conf))
                return public.returnMsg(True, '修改成功！')
            except:
                return traceback.format_exc()
        else:
            return public.returnMsg(False, '请检查修改参数是否正确')

    def get_pattern_list(self, get=None):
        try:
            data = {
                'pattern_list': {
                    'StandardL': '标准存储',
    ...
    def delete_file(self, get):
        try:
            ...
            if file_name[-1] == "/":
                return public.returnMsg(False, "暂时不支持删除自己创建的目录！")
    ...
    def get_lib(self):
        ...
        info = {
            ...
            "check": [
                "/usr/lib/python2.7/site-packages/qcloud_cos/cos_auth.py",
                "/www/server/panel/pyenv/bin/python3.7/site-packages"
