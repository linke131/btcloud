    ...
    def check_data(self) -> Tuple[bool, str]:
        ...
        for p in self.error_path_list:
            if p.startswith(self._data["source"]):
                return False, "指定源文件不能是【{}】或【{}】".format(*self.error_path_list)
        ...
        try:
            ...
            if not (0 <= int(self._data["bwlimit"]) < 1024 * 800):
...
class Receiver(object):
    ...
    def check_data(self, not_verify_conn: bool = False) -> Tuple[bool, Union[str, dict]]:
        ...
        if not self._data["name"]:
            return False, "缺少名称"
        elif any([self._data["name"].find(i) != -1 for i in "^\|/:*?'\"<>$)(]["]):
            return False, "名称中包含特殊字符，无法使用"
        # 有id 编辑的时候不用改
    ...
    def _verify_rsync(self, password: str) -> Tuple[bool, Union[str, dict]]:
        ...
        elif result[1].find('Unknown module') != -1:
            return False, '请检查服务器IP,端口,用户和密码是否正确'
        elif result[1].find('auth failed on') != -1:
            return False, '请检查服务器IP,端口,用户和密码是否正确'
    ...
    def parser_secret_key(secret_key: str, is_ssh: bool = False) -> Tuple[Optional[dict], Optional[str]]:
        ...
        if not is_ssh:
            ...
            for i in key_list:
                ...
                if secret_key.find('-----BEGIN DSA PRIVATE KEY-----') != -1:
                    return None, '暂不支持DSA格式的密钥，您可以在目标机器上【安全-SSH管理】中重启密钥登录，并更改加密方式'
    ...
    def modify_receiver_data(self, data: dict):
        ...
        if "bwlimit" in data and int(data["bwlimit"]) != self._data["bwlimit"]:
            if not (0 <= int(data["bwlimit"]) < 1024 * 800):
        ...
        if "password" in data and data["password"] != self._data["password"]:
            self._data["password"] = data["password"]
    ...
    def _parse_target_dir(data: str) -> dict:
        rep = re.compile(r"Number\s*of\s*files:\s*(?P<total>\d+)(\s*\(.*dir:\s*(?P<dir>\d+).*\))?")
        res = rep.search(data)
...
class rsync_main:
    ...
    def add_module(self, get):
        ...
        if open_port and firewall_tool.can_use_firewall():
            rsync_port = cfg.config["global"]["port"]
    ...
    def add_send_task(self, get):
        ...
        return public.returnMsg(True, '添加成功! ' + ("同步指令执行中..." if not later else ""))
    ...
    def run_later_task(self, get):
        ...
        if "send_id" in get:
            ...
            if isinstance(send_id, str) and send_id.isdigit():
                send_id = int(send_id)
        else:
            return public.returnMsg(False, '参数错误')
        ...
        for sender_conf in data:
            ...
            for recv_conf in sender_conf["target_list"]:
                ...
                if later_status is None:
                    recv_conf["last_status"] = ErrMsgParser.parse_status_msg(self._get_recv_status(recv_conf["name"]))
                else:
            ...
            while file_size and time_str is None:
                ...
                for log_line in read_str.split("\n")[::-1]:
                    ...
                    if res:
                        ...
                        break
                    if len(last_msg) <= 3000:  # 只取3000行数据
    ...
    def add_filter_template(self, get: public.dict_obj):
        ...
        if template_name in filters and not force:
...
class FirewallTool(object):
    ...
    def can_use_firewall(self) -> bool:
        return self._firewall_main is not None

    def get_port_status(self, port) -> Optional[bool]:
        if not self._firewall_main:
            return None
        if self._idx == 0:
    ...
    def _get_port_status_by_model(self, port) -> bool:
        res_data = self._get_rule_by_model(port)
        if len(res_data) == 0:
            return False
    ...
    def _get_rule_by_model(self, port) -> List[Dict]:
        get_obj = public.dict_obj()
        get_obj.p = 1
        get_obj.limit = 99
        get_obj.query = str(port)
        return self._firewall_main.get_rules_list(get_obj)
    ...
    def _set_port_status_by_model(self, port):
        ...
        for i in res_data:
            get_obj.id = i.get("id")
            self._firewall_main.modify_rules(get_obj)
...
class ErrMsgParser:
    _STATUS_MSG_LIST: Tuple[Tuple[str, Callable, Callable]] = (
        (
            r"@ERROR: auth failed on module (?P<m>.*)\n",
            lambda x: "在验证任务:[{}]的密码时出错了".format(x.group("m")),
            lambda x: "请检查任务:[{}]的密码是否已经被修改".format(x.group("m")),
        ),
        (
            r"@ERROR: Unknown module '(?P<m>.*)'",
            lambda x: "在远程服务器查找任务:[{}]时出错了".format(x.group("m")),
            lambda x: "请检查任务:[{}]是否已经被删除".format(x.group("m")),
        ),
        (
            r"failed to connect to (?P<t>.*?) \(",
            lambda x: "在连接远程服务器:[{}]时出错了".format(x.group("t")),
            lambda x: "请检查服务器网络状况",
        ),
    )

    @classmethod
    def parse_status_msg(cls, data_msg: dict) -> dict:
        if data_msg["status"] is True:
            return data_msg
        add_msg: List[str] = []
        msg = data_msg["err_msg"]
        for (rep, errmsg_func, tip_msg_func) in cls._STATUS_MSG_LIST:
            x = re.search(rep, msg)
            public.print_log(x)
            public.print_log(msg)
            if x:
                add_msg.append("ERROR => " + errmsg_func(x))
                add_msg.append("提示  => " + tip_msg_func(x))
        if add_msg:
            msg += "\n" + "* " * 20 + "\n" + "\n".join(add_msg)
        data_msg["err_msg"] = msg
        return data_msg
