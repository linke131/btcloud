...
class rsync_push(base_push):
    ...
    def set_push_config(self, get: public.dict_obj):
        ...
        try:
            ...
            return public.returnMsg(False, '没有配置文件同步任务，无法设置文件同步告警')
        if not ("senders" in lsyncd_conf and bool(lsyncd_conf['senders'])):
            return public.returnMsg(False, '没有配置文件同步任务，无法设置文件同步告警')

        conf_data = self.__push._get_conf()
        pdata["title"] = "文件同步告警"
        if "rsync_push" not in conf_data or len(conf_data["rsync_push"]) == 0:
    ...
    def check_self_plugin():
        if os.path.exists("{}/plugin/rsync/rsync_main.py".format(public.get_panel_path())):
            return True
        return False
