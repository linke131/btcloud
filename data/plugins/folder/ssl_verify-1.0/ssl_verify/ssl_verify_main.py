...
class ssl_verify_main:
  ...
  `addtime` INTEGER
    ...
    def get_ssl_list(self,get):
        ...
        data = _sql.where(where,()).field('id,company,client,status,day,last_modify,addtime').select()
        return data
...
SSLCACertificateFile "/www/server/panel/vhost/apache/bt_ca/{}/ca.cert"'''.format(siteName,siteName)
    ...
    def down_domain_pfx(self,get):
        ...
        self._write_pfx("{}/domain.pfx".format(spath),base64.b64decode(find['pfx']))
...
如默认浏览器无法正常使用，请使用谷歌浏览器进行配置""")
        ...
        for item in flist:
            f.write(item,item.replace(spath,''))       
        f.close()
    ...
    def down_client_pfx(self,get):
        ...
        self._write_pfx("{}/{}.pfx".format(spath,find['client']),base64.b64decode(find['pfx']))
        public.writeFile("{}/password.txt".format(spath),find['password'])
        public.writeFile("{}/使用说明.txt".format(spath),"""Windows用户使用说明：
1、双击当前目录的pfx文件
2、选择下一页直至输入密码页面
