                ...
                for table_name, kcolumns in keep_columns.items():
                    ...
                    if table_name == "spider_stat":
                        for column_name in columns:
                            ...
                            alter_sql = "ALTER TABLE {} ADD COLUMN {} INTEGER DEFAULT 0".format("spider_stat",
                                                                                                flow_column)
                            ts.execute(alter_sql)
                try:
                    conn.execute("COMMIT;")
...
def change_http_info(new_cur, table, field, value):
    insert_sql = "insert into {table}({field}) select '{value}' where not exists (select {field} from {table} where {field}='{value}');".format(
        table=table, field=field, value=value)
    new_cur.execute(insert_sql)
    query_sql = 'select id from {} where {}="{}";'.format(table, field, value)
    selector = new_cur.execute(query_sql)
    res = selector.fetchone()
    return res[0] if res[0] else ''


def site_log_cursor(new_cur, table, field, value, cursor):
    insert_sql = "insert into {table}({field}) select '{value}' where not exists (select {field} from {table} where {field}='{value}');".format(
        table=table, field=field, value=value)
    new_cur.execute(insert_sql)
    stat_sql = 'update {table} set cursor=cursor || ";{cursor}" where {field}="{value}";'.format(table=table,
                                                                                                 cursor=cursor,
                                                                                                 field=field,
                                                                                                 value=value)
    new_cur.execute(stat_sql)


def change_field_name(data):
    res = dict()
    res['a'] = data.get('domain')
    res['b'] = data.get('client_port')
    res['c'] = data.get('body_length')
    res['d'] = data.get('ip')
    res['e'] = data.get('uri')
    res['f'] = data.get('ip_list')
    res['g'] = data.get('request_time')
    res['h'] = data.get('user_agent')
    res['i'] = data.get('protocol')
    res['j'] = data.get('method')
    res['k'] = data.get('time')
    res['l'] = data.get('is_spider')
    res['m'] = data.get('status_code')
    res['n'] = data.get('server_name')
    res['o'] = data.get('referer')
    res['p'] = data.get('request_headers')
    return res


def move_site_logs(ori_db, new_file, new_db):
    import uuid
    global migrate_data_max
    """迁移网站日志数据到新存储文件"""
    ori_cur = None
    ori_conn = None
    new_conn = None
    ...
    try:
        ...
        new_cur = new_conn.cursor()
        new_cur.execute("BEGIN TRANSACTION")
        total_sql = "select count(*) from site_logs;"
        selector = ori_cur.execute(total_sql)
        total = selector.fetchone()
        total = total[0] if total else 0
        if total > migrate_data_max:
            offset = total - migrate_data_max
        data_sql = "select * from site_logs limit {} offset {};".format(migrate_data_max, offset)
        selector = ori_cur.execute(data_sql)
        data_line = selector.fetchone()
        field_list = ['time', 'ip', 'domain', 'server_name', 'method', 'status_code', 'uri', 'body_length', 'referer',
                      'user_agent', 'is_spider', 'protocol', 'request_time', 'request_headers', 'ip_list',
                      'client_port']
        f = None
        limit_total = 0
        flag = 0
        ...
        while data_line:
            ...
            num = len(data_line)
            for i in range(num):
                data[field_list[i]] = data_line[i]
            request_uri = data['uri'].split('?')[0]

            data['user_agent'] = change_http_info(new_cur, 'user_agent', 'user_agent', data['user_agent'])
            data['uri'] = change_http_info(new_cur, 'request_uri', 'request_uri', data['uri'].replace('"', '').replace("'", ''))
            if data['request_headers']:
                header_id = uuid.uuid4().hex
                insert_sql = "INSERT INTO request_headers(id, request_headers) VALUES('{}', '{}');".format(header_id, data['request_headers'])
                new_cur.execute(insert_sql)
                data['request_headers'] = header_id

            if data['referer']:
                referer_id = uuid.uuid4().hex
                insert_sql = "INSERT INTO referer(id, referer) VALUES('{}', '{}');".format(referer_id, data['referer'].replace('"', '').replace("'", ''))
                new_cur.execute(insert_sql)
                data['referer'] = referer_id

            f.seek(0, 2)
            size = f.tell()
            data = change_field_name(data)
            ...
            if lenth > 399:
                ...
                data_line = selector.fetchone()
                continue
            for _ in range(399 - lenth):
                json_data += '0'
            f.write(json_data + '\n')

            if data['m'] >= 400:
                insert_sql = "insert into error_logs(status_code) select {status_code} where not exists (select status_code from error_logs where status_code={status_code});".format(
                    status_code=data['m'])
                new_cur.execute(insert_sql)
                stat_sql = 'update error_logs set cursor=cursor || ";{}" where status_code={}'.format(size, data['m'])
                new_cur.execute(stat_sql)
            site_log_cursor(new_cur, 'uri_cursor', 'uri', request_uri, size)
            site_log_cursor(new_cur, 'ip_cursor', 'ip_list', data['f'], size)

            data_line = selector.fetchone()
            limit_total += 1
            if limit_total % 100 == 0:
                new_conn.commit()
            # 超过5万条数据退出迁移
            if limit_total >= migrate_data_max:
                break
        new_conn.commit()
        f.close()

        migrate_data_max = migrate_data_max - limit_total
        # print("统计数据: {} 已迁移完成。".format(table))
    except Exception as e:
        print("数据: {} 迁移失败: {}".format(ori_db, e))
        return False
    finally:
        ori_cur and ori_cur.close()
        ori_conn and ori_conn.close()
...
def upgrade_db_storage(site_name):
    ...
    new_file_name = time.strftime("%Y%m%d", now) + ".txt"
    logs_db_file = tool.get_log_db_path(site_name, db_name=today_db_name)
    db_path = "/www/server/total/logs/{}".format(site_name)
    new_file_name = os.path.join(db_path, new_file_name)
    cursor_name = os.path.join(db_path, 'cursor_log')
    new_db_name = cursor_name + '/' + today_db_name
    if "migrate_data" in log_info:
    ...
    if "brush_data" not in log_info:
        ...
        if logs_db_size == 0:
            ...
            log_info["brush_data"] = True
        elif os.path.exists(new_db_name):
            print("{}:新网站日志文件已存在，无法自动完成迁移数据。".format(site_name))
            log_info["brush_data"] = False
        else:
            if not os.path.exists(cursor_name):
                os.makedirs(cursor_name)
            # 迁移网站日志,只迁移当天的网站日志数据
            from total_migrate import total_migrate
            tm = total_migrate()
            tm.init_site_db(site_name)
            if os.path.exists(logs_db_file):
                split_res = move_site_logs(logs_db_file, new_file_name, new_db_name)
