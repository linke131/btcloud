...
local server_name, ip, today, day, body_length, method, config, cache_count, hour_str
local status_code
local json, sqlite3
...
local function debug(msg, server_name)
    ...
    local fp = io_open('/www/server/total/debug.log', 'ab')
    if fp == nil then
        return nil
    end
...
local function get_config(site)
    ...
    local conf = nil
    if config_data["global"] == nil then
        return nil
    end
...
local function get_server_name(c_name)
    ...
    do
        ...
        -- if v["generic"] then
        -- 	regx_domain = nil
        -- 	for d_name,_ in pairs(v['generic'])
        -- 	do
        -- 		ngx.log(ngx.ERR, d_name.." vs "..c_name)
        -- 		regx_domain = string_gsub(d_name, '*', '.*')
        --     	if ngx_re_find(c_name, regx_domain) then
        -- 			cache:set(c_name,site_name,cache_timeout)
        -- 			return site_name
        --     	end
        -- 	end
...
local function split_bylog(str, reps)
    local resultStrList = {}
    string_gsub(str, '[^' .. reps .. ']+', function(w)
...
local function init_cursor_db(log_dir, server_name, db_name)
    -- 初始化游标日志数据库
    local path = log_dir .. "/" .. server_name .. "/" .. "cursor_log"
    ...
    local stmt = db:prepare("SELECT COUNT(*) FROM sqlite_master where type='table'")
    local rows = 0
    if stmt ~= nil then
        stmt:step()
    ...
    if stmt ~= nil and rows > 7 then
       ...
       status, errorString = db:exec([[
        ...
        status, errorString = db:exec([[CREATE UNIQUE INDEX user_agent_inx on user_agent(user_agent))]])

        status, errorString = db:exec([[
			CREATE TABLE request_uri(
				id INTEGER PRIMARY KEY,
				request_uri TEXT DEFAULT ""
			)]])

        status, errorString = db:exec([[CREATE UNIQUE INDEX request_uri_inx on request_uri(request_uri))]])
...
local function save_data_on(filename, data)
    ...
    local extime = 0;
    if string_find(filename, "%d+-%d+-%d+") then
        extime = 88400
    end
...
function check_spider(name, ip)
    ...
    -- debug(worker_id.."/check spider:"..name.."/"..ip)
    local spiders_key = "SPIDERS_" .. name
    local sp_key = ip_spider_prefix .. ip
    ...
    local update_spider_tag = "UPDATEING_SPIDER_" .. name
    if cache:get(update_spider_tag) then
        return true
    end
    cache:set(update_spider_tag, true, 60)
    local data = get_spider_json(name, false)
    if not data then
        return true
    end
    ...
    if not ok then
        -- ngx.log(ngx.ERR, "IP库数据加载失败。")
        return true
    end
    for _, k in ipairs(zhizhu_list_data)
    do
        cache:set(ip_spider_prefix .. k, name, ONE_DAY_TIMEOUT)
    end
    cache:set(spiders_key, "1", ONE_DAY_TIMEOUT)
    if cache:get(sp_key) then
        return true
    end
    cache:delete(update_spider_tag)
    return false
end
            ...
            if string_find(out_header['content-type'], 'text/html', 1, true) then
                ...
                if request_header['user-agent'] then
                    if string_find(ua, 'mozilla') then
                        ...
                        local uv_token = ngx.md5(ip .. request_header['user-agent'] .. today2)
...
local function exclude_status(res_status_code)
    if not config['exclude_status'] then
        return false
    end
...
local function statistics_uri(db, stat_server_name, uri, uri_md5, body_length, is_spider)
    ...
    if not open_statistics_uri then
        return true
    end
...
local function statistics_ip(db, stat_server_name, ip, body_length)
    ...
    if not open_statistics_ip then
        return true
    end
...
local function site_log_cursor(db, cursor_table, field, value, cursor)
    local stat_sql = nil
    stat_sql = "INSERT INTO " .. cursor_table .. "(" .. field .. ") SELECT \"" .. value .. "\" WHERE NOT EXISTS (SELECT " .. field .. " FROM " .. cursor_table .. " WHERE " .. field .. "=\"" .. value .. "\");"
    db:exec(stat_sql)
    stat_sql = "UPDATE " .. cursor_table .. " SET cursor=cursor || \";" .. cursor .. "\" WHERE " .. field .. "=\"" .. value .. "\";"
    db:exec(stat_sql)
    return true
end

local function site_error_log_cursor(db, code, cursor)
    local stat_sql = nil
    stat_sql = "INSERT INTO error_logs(status_code) SELECT \"" .. code .. "\" WHERE NOT EXISTS (SELECT status_code FROM error_logs WHERE status_code=\"" .. code .. "\");"
    db:exec(stat_sql)
    stat_sql = "UPDATE error_logs SET cursor=cursor || \";" .. cursor .. "\" WHERE status_code=\"" .. code .. "\";"
    db:exec(stat_sql)
    return true
end
...
local function change_http_info(db, db_name, field, value)
    local res = nil
    local stat_sql = nil
    if value == nil then
        value = ''
    end
    stat_sql = "INSERT INTO " .. db_name .. "(" .. field .. ") SELECT \"" .. value .. "\" WHERE NOT EXISTS (SELECT " .. field .. " FROM " .. db_name .. " WHERE " .. field .. "=\"" .. value .. "\");"
    db:exec(stat_sql)
    local stmt = db:prepare("SELECT id FROM " .. db_name .. " WHERE " .. field .. "=\"" .. value .. "\";")
    if stmt ~= nil then
        local code, _ = stmt:step()
        if code == 100 then
...
local function save_headers(db, table_name, data_uuid, value)
    local stat_sql = ''
    stat_sql = string.format("INSERT INTO "..table_name.."(id, %s) VALUES('%s', '%s');", table_name, data_uuid, value)
    db:exec(stat_sql)
    return true
end

local function write_logs(data, cursor_db)
    local request_uri = Split(data['e'], '?')[1]

    data['h'] = change_http_info(cursor_db, 'user_agent', 'user_agent', data['h'])
    data['e'] = change_http_info(cursor_db, 'request_uri', 'request_uri', data['e'])

    --存储request_headers
    if data['p'] ~= nil and data['p'] ~= '' then
        local header_id = string.gsub(uuid.generate(), '-', '')

        local ok, err = pcall(function()
            return save_headers(cursor_db, 'request_headers', header_id, data['p'])
        end)
    ...
    --存储referer
    if data['o'] ~= nil and data['o'] ~= '' then
        local referer_id = string.gsub(uuid.generate(), '-', '')

        local ok, err = pcall(function()
            return save_headers(cursor_db, 'referer', referer_id, data['o'])
        end)
    ...
    if string.len(json_data) > 399 then
        return false
    end

    --补齐数据长度，不足400字节的填充0
    for _ = 1, 399 - string.len(json_data) do
        json_data = json_data .. '0'
    end

    log_data = log_data..json_data .. '\n'

    --存储日志游标
    site_log_cursor(cursor_db, 'uri_cursor', 'uri', request_uri, start_addr)
    site_log_cursor(cursor_db, 'ip_cursor', 'ip_list', data['f'], start_addr)

    --存储错误日志游标
    if data['m'] >= 400 then
        site_error_log_cursor(cursor_db, data['m'], start_addr)
    end

     start_addr = start_addr + 400

    return true
end

-- 存储单行日志
local function store_line(total_db, cursor_db, store_server_name, lineno)
    ...
    --精简json数据key,减少日志文件大小
    local file_data = {}
    file_data['a'] = domain
    file_data['b'] = client_port
    file_data['c'] = body_length
    file_data['d'] = ip
    file_data['e'] = request_uri
    file_data['f'] = ip_list
    file_data['g'] = request_time
    file_data['h'] = user_agent
    file_data['i'] = protocol
    file_data['j'] = method
    file_data['k'] = time
    file_data['l'] = is_spider
    file_data['m'] = status_code
    file_data['n'] = server_name
    file_data['o'] = referer
    file_data['p'] = request_headers

    --请求日志写入文件存储
    local flag = write_logs(file_data, cursor_db)
    if not flag then
        debug('数据日志过长,已过滤...')
    end
...
local function store_logs(store_server_name)
    ...
    local cursor_log_db_path = base_path .. "cursor_log" .. "/" .. today .. '.db'
    local logs_db_name = today .. ".txt"
    local logs_db_path = base_path .. logs_db_name

    local total_init_res = true
    local cursor_init_res = true
    ...
    if not cache:get(cursor_log_db_path) then
        cursor_init_res = init_cursor_db(log_dir, store_server_name, today .. '.db')
    end
    if not file_exists(cursor_log_db_path) then
        cache:set(cursor_log_db_path, nil)
        cursor_init_res = init_cursor_db(log_dir, store_server_name, today .. '.db')
    end

    if not cursor_init_res or not total_init_res then
    ...
    local total_db, err1 = sqlite3.open(total_db_path)
    local cursor_db, err2 = sqlite3.open(cursor_log_db_path)

    if total_db == nil or cursor_db == nil then
        -- debug("网站监控报表数据库连接异常。")
        cache:set(storing_key, false)
        ...
        if cursor_db and cursor_db:isopen() then
            cursor_db:close()
        end
        return true
    end
    ...
    log_data = ""
    file = io.open(logs_db_path, 'a')
    start_addr = file:seek("end")

    if store_end >= store_start then
        for i = store_start, store_end, 1 do
            if store_line(total_db, cursor_db, store_server_name, i) then
    ...
    --记录游标结束地址，把数据写入文件，删除后会导致start_addr一直为上次结束的位置
    file:seek("end")
    io.flush()
    io.close()
...
local function cache_logs()
    ...
    if not excluded then
        if (status_code == 500 and config["record_get_500_args"] == true) or (method == "POST" and config["record_post_args"] == true) or (status_code == 403 and config["record_get_403_args"] == true) then
            local data = ""
            local ok, err = pcall(function()
...
function _M.run_logs()
    ...
    uri = ngx_var.uri
    if uri == '/favicon.ico' or status_code == 0 or status_code == 444 then
        return true
    end
    ...
    if server_name == 'default' or server_name == '_' or server_name == 'phpinfo' then
        return true
    end
