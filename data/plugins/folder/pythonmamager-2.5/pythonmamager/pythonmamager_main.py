    ...
    def _update_conf(self):
        ...
        for conf in self._conf:
            if "log_path" not in conf:
                change = True
                conf["log_path"] = (conf["path"] + "/logs").replace("//", "/")
            if "is_supervisor" not in conf:
                change = True
                conf["is_supervisor"] = 0
            if "numprocs" not in conf:
                change = True
                conf["numprocs"] = "1"
            if "supervisor_name" not in conf:
                change = True
                conf["supervisor_name"] = ""
            conf["vpath"] = conf["vpath"].replace("//", "/")
            if conf["path"][-1] == "/":
                conf["path"] = conf["path"][:-1]
        if change:
            self.save_conf()
            for i in self._conf:
                # kill 可能未关闭的项目
                public.ExecShell('''ps aux | grep "%s" |grep -v "grep"| awk '{print $2}' | xargs kill''' % i["vpath"])
