...
class OSClient(object):
    ...
    _aes_status=os.path.join(_panel_path,'plugin/jdcloud/aes_status')
...
class OBSClient(OSClient, object):
    _title = "京东云存储"
    _name = "jdcloud"
    __error_msg = "ERROR: 无法连接到京东云存储服务器，请检查[" \
    ...
    def authorize(self):
        ...
        try:
            ...
            raise OsError("京东云存储客户端连接异常, 请检查配置参数是否正确。")
    ...
    def resumable_upload(self,
        ...
        try:
            ...
            #检测object目录是否存在，不存在则创建
            check_dir_list=[]
            if object_name != None:
                resp = jdcloudClient.list_objects_v2(
                    Bucket=self.__bucket_name,
                    MaxKeys=1000,
                    Prefix="",
                    )
                if 'Contents' in resp:
                    for dir in resp['Contents']:
                        if not dir:continue
                        check_dir_list.append(dir['Key'])
                ...
                for i in range(0,(len(dir_dir) - 1)):
                    ...
                    if dir_d not in check_dir_list:
                        self.create_dir(dir_d)
    
                        
            print("|-正在上传到 {}...".format(object_name))
            part_size = 5 * 1024 * 1024
            result = jdcloudClient.create_multipart_upload(Bucket=self.__bucket_name, Key=object_name)
            ...
            with open(local_file_name, "rb") as fp:
                while True:
                    ...
                    jdcloudClient.upload_part(Bucket=self.__bucket_name, Key=object_name, PartNumber=index, UploadId=upload_id, Body=part)
                    # print("上传成功")
            rParts = jdcloudClient.list_parts(Bucket=self.__bucket_name, Key=object_name, UploadId=upload_id)["Parts"]

            partETags = []
            for part in rParts:
                partETags.append({"PartNumber": part['PartNumber'], "ETag": part['ETag']})
            jdcloudClient.complete_multipart_upload(Bucket=self.__bucket_name, Key=object_name, UploadId=upload_id,
    ...
    def get_list(self, path="/", delimiter="/"):
        ...
        if 'Contents' in resp:
            for b in resp['Contents']:
                ...
                if b['Key'][-1] == '/':
                    tmp_key=b['Key'][:-1]
                    if tmp_key.find('/') !=-1:continue
                    tmp['download'] = 'http://bt.cn'
                    data.append(tmp)
                elif b['Key'][-1] != '/':
                    if b['Key'].find('/') !=-1:continue
                    tmp["type"] = True
                    tmp['download'] = self.generate_download_url(path+b['Key'])
                    tmp['size'] = b['Size']
                    tstr = b['LastModified']
                    tstr += datetime.timedelta(hours=8)
                    ts = int((time.mktime(tstr.timetuple()) + tstr.microsecond / 1000000.0))
                    tmp['time'] = ts
                    data.append(tmp)
    ...
    def generate_download_url(self, object_name):
        try:
            jdcloudClient = self.authorize()
            url = jdcloudClient.generate_presigned_url('get_object',Params={'Bucket': self.__bucket_name,'Key': object_name},ExpiresIn=3600)
            return url
        except Exception:
    ...
    def delete_object_by_os(self, object_name):
        ...
        # 注意：京东云存储 SDK如果删除成功，删除请求状态返回<300状态码。
        ...
        result = jdcloudClient.delete_object(
            Bucket=self.__bucket_name, 
            Key=objectKey,
            )
        if result['ResponseMetadata']['HTTPStatusCode'] in ['204',204]:
            return True
        return False
    ...
    def batch_delete(self, object_names):
        ...
        try:
            ...
            result = jdcloudClient.delete_objects(
                Delete={objects},
                )
...
class jdcloud_main(object):
    ...
    def download_file(self, filename):
        ...
        """
        jdcloudclient = OBSClient().authorize()
        object_name= 'bt_backup/'+filename
        local_file='/tmp/{}'.format(filename)
        try:
            with open(local_file,'wb') as data:
                jdcloudclient.download_fileobj(
                    OBSClient().__bucket_name,
                    object_name,
                    data
                )
        except:
    ...
    def get_lib(self):
        ...
        info = {
            ...
            "type": "计划任务",
            "ps": "将网站或数据库打包备份到京东云对象存储空间 "
                  "<a class='link' "
                  "href='https://user.jdcloud.com/register?source=jdcloud&ReturnUrl=https%3A%2F%2Fwww.jdcloud.com%2F' target='_blank'>[申请账号]</a>",
            "status": 'false',
            "opt": "jdcloud",
            "module": "jdcloud",
            "script": "jdcloud.sh",
            "help": "http://www.bt.cn/bbs/thread-1061-1-1.html",
            "AccessKeyID": "AccessKeyID|请输入AccessKeyID|京东云的AccessKeyId",
            "SecretKey": "SecretKey|请输入SecretKey|京东云的AccessKeySecret ",
            "Bucket": "存储空间名称|京东云存储中您创建的Bucket名称",
            "Domain": "京东云存储地域域名，不包括Bucket名",
            "backup_path": "备份保存路径, 默认是/bt_backup",
            "check": [
                "/www/server/panel/plugin/jdcloud",
            ]
        }
