...
class system_scan_main:
    __vul_list = '/www/server/panel/plugin/system_scan/high_risk_vul.json'
            ...
            for v in vul["affected_list"]:
                if v["manufacturer"] == sys_version:
                    ...
                    if "Up to (excluding)" in v['affected']:
                        vul_version = v['affected'].split("Up to (excluding)\n                                                ")[1]
                    elif "Up to (including)" in v['affected']:
                        vul_version = v['affected'].split("Up to (including)\n                                                ")[1]
                    else:
                        continue
                    try:
    ...
    def get_list(self, get):
        ...
        vul_list = []
        if not os.path.exists(self.__result):
            tmp_dict = {"vul_count": 1455, "risk": [], "ignore": [],
    ...
    def upgrade_soft(self, soft):
        ...
        '''
        error_message = ['Couldn\'t find', 'Error', 'Nothing to do', 'already the newest', 'Unable to correct problems']
        sys_version = self.get_sys_version()
        if "centos" in sys_version:
