...
class ftp_main:
    ...
    def set_config(self, get):
        try:
            ...
            if ftp_host.find('*') !=-1 or ftp_user.find('*') !=-1 or ftp_pass.find('*') !=-1:return public.returnMsg(False, self.__before_error_msg)
            tmp_host = ftp_host.split(':')[0]
            if not self.check_ip(tmp_host) and not public.is_domain(tmp_host):return public.returnMsg(False, '域名格式错误或IP地址格式错误，请核实!<br/>域名格式：x.x（根域名格式）<br/>域名示例：bt.cn<br/>IP地址+端口格式：x.x.x.x:x<br/>IP地址+端口示例：192.168.1.36:21');
            if not backup_path:
